...
A complete 6-man DTZ shipping set is therefore 111.5 GiB, castling included.
...
(pack layer 0), at most one extra change-point per position. Thus a `dtm50/`
pack also provides unbounded DTM without requiring a separate `dtm/` table.
A no-50MR probe reads layer 0; an `hmc = k` probe reads layer
