...
| 4 (Pawnless) |  208 KiB | 1.87 MiB |      N/A | 2.08 MiB | 7.38 MiB |
| 4 (Pawnful)  |  538 KiB |  522 KiB | 3.09 MiB | 3.69 MiB | 11.8 MiB |
| 5 (Pawnless) | 41.8 MiB |  245 MiB |      N/A |  389 MiB | 1.93 GiB |
| 5 (Pawnful)  |  156 MiB |  260 MiB | 1023 MiB |  1.3 GiB |  4.4 GiB |
| 6 (Pawnless) | 5.96 GiB | 23.4 GiB |      N/A | 46.6 GiB |  232 GiB |
| 6 (Pawnful)  | 27.9 GiB | 46.9 GiB |  173 GiB |  241 GiB |  839 GiB |
| 7 (Pawnless) |  591 GiB | 1.61 TiB |      N/A |      --- |      --- |
| 7 (Pawnful)  | 3.44 TiB | 5.63 TiB |      --- |      --- |      --- |
| 8 (Op1)      |      ... |      ... |      --- |      --- |      --- |
| Total (≤6)   | 34.1 GiB | 70.8 GiB |  174 GiB |  289 GiB | 1.05 TiB |
| Total (≤7)   | 4.05 TiB | 7.32 TiB |      --- |      --- |      --- |
...
| 7 (Pawnless) | 2.00 TiB | 5.09 TiB |      N/A |      --- |      --- |
| 7 (Pawnful)  | 12.9 TiB | 18.8 TiB |      --- |      --- |      --- |
| 8 (Op1)      |      --- |      --- |      --- |      --- |      --- |
| Total (≤6)   |  135 GiB |  234 GiB |  440 GiB |  719 GiB | 2.73 TiB |
| Total (≤7)   | 15.0 TiB | 24.1 TiB |      --- |      --- |      --- |
...
| 4          | 34.1 KiB |  127 KiB |    92.4 KiB |     310 KiB |
| 5          | 7.61 MiB | 31.1 MiB |    26.3 MiB |    83.7 MiB |
| 6          | 1.57 GiB | 4.71 GiB |    5.11 GiB |    14.1 GiB |
| Total (≤6) | 1.58 GiB | 4.74 GiB |    5.13 GiB |    14.2 GiB |

A complete 6-man DTZ shipping set therefore occupies 111.2 GiB, including
castling tables.
...
| `--cache MiB` | decoded-block cache budget shared by the sub-table probe readers (default: 64) |
...
In a castling table, the WDL frame of a side that holds a castling right is
never dropped: rebuilding it needs each quiet child's exact boundary code, which
only a kept frame supplies. That side has quiet external edges into a table
whose frame may be dropped, and a derivation cannot recover a boundary cell. If
the larger frame is protected, the smaller one is dropped instead, and an
asymmetric WDL table in which both sides hold rights keeps both frames.
...
|------------|---------------|----------------|
| `WIN` | `WIN`, by a winning capture | any of the seven codes |
| `DRAW` | `DRAW`, by a drawing capture | `LOSE`, `BOUNDARY_LOSS`, `BLESSED_LOSS`, `DRAW` |
| `LOSE` | -- | nothing: it is already the bottom |

For example, a `KQKR` win by rook capture reaches a `KQK` WIN, so its stored
cell can use any class code to improve compression. Boundary cells are never
capped because dropped-frame reconstruction reads their codes directly.

A capped cell may still store a boundary code as filler, ranked by its class:
`BOUNDARY_LOSS` as `LOSE`, `BOUNDARY_WIN` as `WIN`. The probe also applies the
bound to boundary codes, and the bound separates filler from genuine markers.
A genuine `BOUNDARY_LOSS` is a loss, so its bound is at most `LOSE` and the code
survives. A capped cell's class is at least `BLESSED_LOSS`, so the bound replaces
the filler. A genuine `BOUNDARY_WIN` has no winning capture or promotion, since
that zeroing move would give DTZ 1. A bound of `WIN` therefore marks filler, which
reads as a plain `WIN`.
...
stabilizers; 1 for castling tables, which use no symmetry), so for each color
W + D + L + illegal equals the table's weighted domain total. Since WDL is
projected from the DTZ pass, it has no `.info` file of its own; its counts are
included in the DTZ `.info` file.

Apart from en passant, decoding stays within one material: DTZ/DTC/DTM/DTM50
read classes from the material's own WDL companion, and WDL needs no companion,
so a full table decodes from its own files in O(1): one block read and one index
lookup. When the FEN carries en-passant rights, each legal en-passant capture
(at most two) adds a probe of that capture's sub-table. Sub-tables are needed
...
only W/L values, ordered by how many layers each value appears in; DRAW and
ILLEGAL use no rank, so
...
layers take about 4× the storage of one layer, or about one-twenty-fifth the
storage of 100 flat halfmove-clock tables.
...
Thirty-one rows suffice: an 8-man position gives the winner at most six pawns,
each with at most five non-converting pushes. The generator solves budgets
0 through 29; at budget 30 every possible non-converting push is allowed, so
the embedded DTZ row supplies that endpoint for clean W/L cells. Cursed/blessed
cells use row 0 only as DTZ, and DTC reports DRAW for them.
