...
constexpr unsigned IGNORE_50MR = ~0u;
...
bool check_material(const Options& opt, const std::string& name)
{
	...
	const std::filesystem::path dtm50_path = std::filesystem::path(opt.dtm50_dir) / (ps.name() + ".lzdtm50");
	...
	if (opt.check_dtm && !require_full_file(dtm_path, EGTB_Magic::DTM_MAGIC, "DTM") && !require_full_file(dtm50_path, EGTB_Magic::DTM50_MAGIC, "DTM50")) return false;
	...
	// Independent instances so the DTC check never reads DTM/DTM50 and the
	// DTM check never reads DTC. The DTM branch carries dtm + dtm50, so DTM
	// is recomputed from the dtm50 pack's layer 0 (standalone dtm fallback).
		...
		while (true)
		{
			...
			for (size_t i = lo; i < hi; ++i)
			{
				for (Color stm : { WHITE, BLACK })
				{
					...
					if (opt.check_dtc)
					{
						const Probe_Result pr = dtc_tables.probe(ps, pos, IGNORE_50MR);
						if (pr.status == Probe_Result::Status::OK)
						{
							if (!counted) { ++s.scanned; counted = true; }
							const DTC_Derived d = derive_dtc_from_children(dtc_tables, pos);
...
int main(int argc, char** argv)
{
	try {
		...
		for (int i = 1; i < argc; ++i)
		{
			...
			if (a == "--dtm50-dir" && i + 1 < argc) { opt.dtm50_dir = argv[++i]; continue; }
			...
			if (a == "-h" || a == "--help") {
				std::printf(
					...
					"  --dtm50-dir DIR  DTM50 directory (default ./dtm50/)\n"
