...
struct Options
{
	// Extra search directories; empty means the probe's own defaults only.
	std::vector<std::string> wdl_dirs;
	std::vector<std::string> dtc_dirs;
	std::vector<std::string> dtm_dirs;
	std::vector<std::string> dtm50_dirs;
...
Square ep_square_after_move(const Position& pos, Move m)
{
                   ...
                   unsigned rule50)
{
	...
	for (size_t i = 0; i < ml.size(); ++i)
	{
		...
		const Square child_ep = ep_square_after_move(root, m);
...
void probe_one(const Options& opt, Probe_Tables* tables, const Probe& probe)
{
	...
	const unsigned rule50 = fen_rule50(probe.fen);
