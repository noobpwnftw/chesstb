...
//   DTC x DTM50: the clock where DTM50 flips to DRAW is pinned by dtc.
//
...
struct Shard
{
	...
	size_t v_window_early_draw = 0;
	size_t v_window_late_win = 0;
...
bool check_material(const Options& opt, const std::string& name)
{
			...
			for (size_t i = lo; i < hi; ++i)
			{
				for (Color stm : { WHITE, BLACK })
				{
					...
					else
					{
						...
						const bool cursed = w == WDL_Entry::CURSED_WIN
							|| w == WDL_Entry::BLESSED_LOSS;
						if (pr.has_dtm50 && !cursed && dtc_v <= DTC_MAX_NON_CURSED_DTZ)
						{
							// -1 no answer, 0 DRAW at this clock, 1 still decisive.
							// A clock-expired W/L stores 0 and decodes as DRAW; the
							// mate-in-0/mate-in-1 recovery lifts back out the cells
							// that legitimately survive the tightest clock.
							auto live_at = [&](unsigned hmc) -> int {
								const Probe_Result q = (hmc == 0)
									? pr : tables.probe(ps, pos, hmc);
								if (q.status != Probe_Result::Status::OK || !q.has_dtm50)
									return -1;
								return q.dtm50_wdl != WDL_Entry::DRAW ? 1 : 0;
							};
							const unsigned last = dtc_v >= 2
								? static_cast<unsigned>(DTC_MAX_NON_CURSED_DTZ - dtc_v)
								: static_cast<unsigned>(DTM50_HMC_COUNT - 1);
							char tag[64];
							const int at_last = live_at(last);
							if (at_last < 0) {
								++s.v_missing_dtm50;
							} else if (at_last == 0) {
								++s.v_window_early_draw;
								std::snprintf(tag, sizeof(tag), "WINDOW_EARLY_DRAW@hmc=%u", last);
								push_sample(s, opt.sample_cap, ps, i, stm, pos, w, dtc_v, tag);
							}
							if (dtc_v >= 2) {
								const int past_last = live_at(last + 1);
								if (past_last < 0) {
									++s.v_missing_dtm50;
								} else if (past_last == 1) {
									++s.v_window_late_win;
									std::snprintf(tag, sizeof(tag), "WINDOW_LATE_WIN@hmc=%u", last + 1);
									push_sample(s, opt.sample_cap, ps, i, stm, pos, w, dtc_v, tag);
								}
							}
						}
	...
	for (const auto& sh : shards) {
		...
		t.v_window_early_draw        += sh.v_window_early_draw;
		t.v_window_late_win          += sh.v_window_late_win;
	...
	const size_t violations =
		...
		+ t.v_window_early_draw + t.v_window_late_win
	...
	if (t.v_dtc_noncursed_range)
		std::printf("    %zu  WIN/LOSE cells with dtc>100\n",
			t.v_dtc_noncursed_range);
	if (t.v_window_early_draw)
		std::printf("    %zu  W/L cells DTM50-drawn at hmc=100-dtc (dtc too large, or a lost DTM50 layer)\n",
			t.v_window_early_draw);
	if (t.v_window_late_win)
		std::printf("    %zu  W/L cells still DTM50-decisive at hmc=101-dtc (dtc too small, or a stale DTM50 layer)\n",
			t.v_window_late_win);
	if (v_missing_dtm)
