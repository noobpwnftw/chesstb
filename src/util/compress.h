...
// Compressed block storage with two backings, chosen at construction by source
// byte size. Small tables stay on the heap; larger ones spill to an append-only
// temp file (mmapped on first read, removed on destroy) so the full set never
// sits in RAM at once.
struct Compressed_Block_Store
{
	// At or below this many source bytes (num_blocks * block_size), keep
	// everything on the heap; larger tables spill. Bound is on source size, not
	// block count, since that's what tracks RAM pressure.
	...
	void set(size_t block_id, std::vector<uint8_t>&& block);
...
struct Compress_Helper
{
	...
	// helper consumes. Identity by default; DTZ helpers override since they
...
// Decompresses one LZMA block into dst. dst.size() is the expected output size;
// a stream error or a size mismatch throws. Holds no state across calls.
