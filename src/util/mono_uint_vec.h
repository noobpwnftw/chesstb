...
NODISCARD constexpr uint8_t mono_bit_width(uint64_t v)
{
	return v == 0 ? uint8_t{ 0 }
	              : static_cast<uint8_t>(64 - __builtin_clzll(v));
}

NODISCARD constexpr uint64_t mono_low_mask(unsigned width)
{
...
struct Mono_Uint_Vec
{
	...
	NODISCARD uint64_t get(size_t i) const
	{
	...
	NODISCARD std::array<uint64_t, 2> get2(size_t i) const
	{
...
// the end checksum).
inline constexpr size_t MONO_SECTION_WIDTH_BYTES = 4;
