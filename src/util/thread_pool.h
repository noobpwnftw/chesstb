...
struct Thread_Pool
{
	...
	void respawn_thread(size_t i)
	{
		m_workers[i].restart();
	}

	void respawn_all_threads()
	{
		for (Worker_Thread& w : m_workers)
			w.restart();
	}
...
private:
	...
	private:
		// The queue is drained before this returns, but the last task may still be
		// running when `m_no_tasks` fires — the join is what waits it out.
		void stop_and_join()
		{
			std::unique_lock<std::mutex> lock(m_mutex);
