...
// and reused for every logical<->storage conversion (gen/compress only). Holds
// Dividers so the conversions avoid hardware DIV/MOD.
struct Index_Permutation_Plan
{
	uint32_t perm = 0;                                           // raw config this plan was built from
	size_t n = 0;
	size_t within = 1;
...
NODISCARD Index_Permutation_Plan make_index_permutation_plan(
	const Config& cfg,
	uint32_t perm)
{
