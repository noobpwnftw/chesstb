...
WDL_Entry DTC_Generator::read_post_push_wdl(Position_For_Gen& pos_gen, Move move) const
{
	...
	if (m_layer > 0)
	{
		const WDL_Entry w = m_table->read<DTZ_Final_Entry>(
			opp, m_table->layer_of(m_layer - 1, post_pid), post_idx).wdl();
		if (w == WDL_Entry::LOSE) return w;
	}
	if (m_table->read<DTZ_Final_Entry>(
			opp, m_table->layer_of(m_layer, post_pid), post_idx).wdl() == WDL_Entry::WIN)
		return WDL_Entry::WIN;
	return WDL_Entry::DRAW;
}

DTZ_Any_Entry DTC_Generator::make_initial_entry(Position_For_Gen& pos_gen,
                                                bool known_legal) const
{
	...
	if (best == ValueWin) return DTZ_Final_Entry::make_win(1);

	if (!any_quiet_legal)
	{
		if (best == ValueDraw) return DTZ_Intermediate_Entry{};
		if (best == ValueLoss) return DTZ_Final_Entry::make_loss(1);
	}

	if (best == ValueDraw) return DTZ_Intermediate_Entry::make_cap_draw();
	return DTZ_Intermediate_Entry{};
}
	...
	for (size_t g : m_pair_group_ids)
	{
		...
		thread_pool->run_sync_task_on_all_threads([&](size_t) {
			...
			for (const Board_Index idx : group_it.indices())
			{
				...
				// Legality is invariant across budgets.
				bool illegal[COLOR_NB] = { false, false };
				if (m_layer > 0)
				{
					const size_t below = m_table->layer_of(m_layer - 1, pid_of_idx);
					for (Color us : { WHITE, BLACK })
					{
						illegal[us] = m_table->read<DTZ_Final_Entry>(
							us, below, idx).is_illegal();
						if (illegal[us]) write_dtc(us, idx, DTZ_Final_Entry::make_illegal());
					}
					if (illegal[WHITE] && illegal[BLACK]) continue;
				}
				else if (!pos_gen.is_legal())
				{
					write_dtc(WHITE, idx, DTZ_Final_Entry::make_illegal());
					write_dtc(BLACK, idx, DTZ_Final_Entry::make_illegal());
					continue;
				}
				if (m_layer == 0)
				{
				...
				for (Color us : { WHITE, BLACK })
				{
					if (illegal[us]) continue;
					pos_gen.set_turn(us);
					std::visit(overload{
						[&](DTZ_Final_Entry entry) {
							commit_if_new(us, idx, pid_of_idx, entry);
					...
					}, make_initial_entry(pos_gen, m_layer > 0));
				}
			}
		});
	}
...
void DTC_Generator::collect_init_layers()
{
	m_init_layers.clear();
	const auto add = [this](size_t l) {
		if (std::find(m_init_layers.begin(), m_init_layers.end(), l) == m_init_layers.end())
			m_init_layers.push_back(l);
	};
	add(m_layer);
	if (m_layer == 0) return;
	if (!m_active_pawn_slices.empty())
		add(m_table->layer_of(m_layer - 1,
		                      static_cast<size_t>(m_active_pawn_slices.front())));
	for (const int32_t pid : m_active_pawn_slices)
		for (const int32_t t : m_targets_by_pid[static_cast<size_t>(pid)])
		{
			add(m_table->layer_of(m_layer, static_cast<size_t>(t)));
			add(m_table->layer_of(m_layer - 1, static_cast<size_t>(t)));
		}
}

namespace {

// Finite DTC folds non-clean DTZ classes to draw.
NODISCARD bool dtc_cell_matches_dtz(DTZ_Final_Entry a, DTZ_Final_Entry u)
{
	switch (u.wdl())
	{
		case WDL_Entry::WIN:  return a.is_win()  && a.value() == u.value();
		case WDL_Entry::LOSE: return a.is_loss() && a.value() == u.value();
			...
			for (auto [chunk_start, chunk_end] : cell_it.chunks())
				for (Color c : { WHITE, BLACK })
				{
					...
					while (const auto row = rows.next())
					{
						...
						{
							const DTZ_Final_Entry a = row.data[static_cast<size_t>(idx) - row_lo];
                                            ...
                                            size_t g_start, size_t g_end, size_t spg) const
{
	...
	for (const int32_t pid : m_active_pawn_slices)
	{
		const auto& targets = m_targets_by_pid[static_cast<size_t>(pid)];
		...
		for (const int32_t tpid : targets)
		{
			...
			// A target's effective layers are its own, so they are the same for
			// every king slice of it.
                                    ...
                                    size_t g_start, size_t g_end, size_t spg)
{
	...
	// Every pid of a batch shares its map, so one layer below serves the whole fusion.
	...
	uint16_t ply, DTZ_Intermediate_Entry hint) const
{
	...
	for (size_t i = 0; i < ml.size(); ++i)
	{
		...
		const auto ce = read_dtc<DTZ_Final_Entry>(opp, child);
...
void DTC_Generator::retro_mark_win_in_1(Position_For_Gen& pos_gen)
{
	...
	for (size_t i = 0; i < ml.size(); ++i)
	{
		...
		const auto e = read_dtc<DTZ_Final_Entry>(opp, pred);
		if (e.is_illegal() || !e.is_draw()) continue;
		write_dtc(opp, pred, DTZ_Final_Entry::make_win(1));
	}
}
...
void DTC_Generator::retro_mark_changed(Position_For_Gen& pos_gen)
{
	...
	for (size_t i = 0; i < ml.size(); ++i)
	{
		const Board_Index pred = next_quiet_index(pos_gen, ml[i]);
		if (pred == BOARD_INDEX_NONE) continue;
		const DTZ_Final_Entry fe = read_dtc<DTZ_Final_Entry>(opp, pred);
		if (fe.is_illegal() || !fe.is_draw()) continue;
		const auto& ie = reinterpret_cast<const DTZ_Intermediate_Entry&>(fe);
		if (ie.has_change() || ie.has_cap_draw()) continue;
		// Preserve a racing Final write.
		m_table->m_dtc[opp][m_layer].lock_add_flags(pred, DTZ_FLAG_CHANGE);
		mark_iter(opp, pred, m_table->m_dtc[opp][m_layer]);
	}
}

void DTC_Generator::retro_mark_wins(Position_For_Gen& pos_gen, uint16_t target_dtc)
{
	// Wins beyond the band ceiling belong to a higher budget.
	if (target_dtc > DTZ_Final_Entry::MAX_NON_CURSED_DTZ) return;

	Position& pos = pos_gen.board_unchecked();
	const Color opp = color_opp(pos.turn());
	Move_List ml;
	pos.gen_pseudo_legal_pre_quiets(out_param(ml));
	const DTZ_Final_Entry new_e = DTZ_Final_Entry::make_win(target_dtc);
	for (size_t i = 0; i < ml.size(); ++i)
	{
		const Board_Index pred = next_quiet_index(pos_gen, ml[i]);
		if (pred == BOARD_INDEX_NONE) continue;
			...
			for (auto [chunk_start, chunk_end] : cell_it.chunks())
			{
				...
				while (const auto row = rows.next())
				{
					     ...
					     idx = static_cast<Board_Index>(static_cast<size_t>(idx) + 1))
					{
						...
						{
							const auto& ie = reinterpret_cast<const DTZ_Intermediate_Entry&>(fe);
							if (ie.has_change()) chunk.any_intermediate = true;
						...
						{
							retro_mark_wins(pos_gen, fe.value() + 1);
							local.any = true;
							break;
						}
						case Iter_Action::REVERIFY:
						{
							...
							write_dtc(stm, idx, DTZ_Final_Entry::make_loss(res.loss_dtz));
...
void DTC_Generator::iterate(In_Out_Param<Thread_Pool> thread_pool, uint16_t finished_ply)
{
	auto check_interrupt = [](uint16_t fp) {
		if (egtb_is_interrupt_requested())
			throw DTC_Interrupted{ fp };
	};

	// Retro stays inside the fusion and current layer.
	const size_t ngroups = m_table->m_dtc[WHITE][0].num_groups();
                                  ...
                                  const Resume_Point& resume)
{
	...
	for (size_t k = (resuming ? static_cast<size_t>(resume.layer) : 0); k < max_layers; ++k)
	{
		...
		// Charge fusion sizing by init's maximum resident layer count.
		set_active_fusion(psm, batch);
		collect_init_layers();
		const auto fusions = compute_fusion_groups(
			m_table->m_dtc[WHITE][0], batch, m_init_layers.size());
		...
		if (!resuming_layer)
		{
			...
			// Only DTZ saturation proves a terminal-free layer may be skipped.
		...
		for (size_t fi = 0; fi < fusions.size(); ++fi)
		{
			...
			refresh_active_metadata(m_table->m_dtc[WHITE][0]);
			// Init's marks are the frontier, unless another fusion's retro can
			// clear a shared group, or a resumed layer never ran init.
	...
	In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	DTZ_File_For_Probe dtz;
	dtz.open(paths, m_epsi);
	...
	m_table->m_batch_real.assign(batches.size() + 1, {});
	{
		...
		else
		{
			for (size_t l = 0; l < DTC_BUDGET_LAYERS; ++l)
				for (Color c : { WHITE, BLACK })
					m_table->m_dtc[c][l].remove_disk_files();
		}
	}
	remove_checkpoint(ckpt_path);

	// Pushes only reach completed earlier batches.
	m_num_layers = 1;
...
using DTC_Save_Cache = Save_Group_Cache<DTZ_Final_Entry, DTZ_Intermediate_Entry>;
using DTC_Pinned_Range = Pinned_Group_Range<DTZ_Final_Entry, DTZ_Intermediate_Entry>;
...
NODISCARD uint16_t dtc_layer_value(DTZ_Final_Entry e)
{
	if (e.is_illegal() || e.is_draw()) return DTZ_Final_Entry::ILLEGAL_VAL;
	ASSERT(static_cast<uint16_t>(e.value()) <= DTZ_Final_Entry::MAX_NON_CURSED_DTZ);
	return static_cast<uint16_t>(e.value());
}
	...
	uint16_t* chunk, size_t* logical_pos, uint8_t* cell_row, uint8_t* seen)
{
	...
	// The inherited DTZ permutation leaves a block holding only some batch rows,
	// so only the layers those rows store are worth paging in.
	...
	for (size_t layer = num_layers; layer-- > 0; )
	{
		...
		for (size_t k = 0; k < this_bp; ++k)
		{
			...
			seen[k] = 1;
			if (!at_cell_top && chunk[k] == DTZ_Final_Entry::ILLEGAL_VAL) continue;
			const auto e = table.read<DTZ_Final_Entry>(
			...
			if (at_cell_top)
			{
				if (e.is_illegal())
				{
					chunk[k] = DTZ_Final_Entry::ILLEGAL_VAL;
					v = DTZ_Final_Entry::ILLEGAL_VAL;
				}
				else
				{
					chunk[k] = dtz_pack_value(
						dtz.read(color, static_cast<Board_Index>(logical_pos[k])));
					if (chunk[k] != DTZ_Final_Entry::ILLEGAL_VAL) any_live = true;
				}
			}
			size_t b_hi = layer;
	...
	Layered_Rank_Table& ranks)
{
	...
	const auto bins = gather_egtb_info_parallel<Seen>(
		...
		[&dtz, color](Seen& s, size_t idx, const DTZ_Final_Entry& e) {
		...
		thread_pool->run_sync_task_on_multiple_threads(workers, [&](size_t) {
			for (;;)
			{
				...
				for (size_t g = 0; g < ng; ++g)
				{
					...
					// Skip groups with no owner of this stored layer.
					...
					for (size_t p = p_lo; p < p_hi; ++p)
					{
						...
						const auto e = table.read<DTZ_Final_Entry>(color, layer, static_cast<Board_Index>(p));
						if (!e.is_draw() && !(EGTB_GEN_LOSS_ONLY && e.is_win()))
	...
	Layered_Compressed_Color& out)
{
	...
	// Which physical layers a batch row stores, once for the whole save.
	...
	thread_pool->run_sync_task_on_all_threads([&](size_t thread_id) {
		...
		for (;;)
		{
			...
			if (compressed.size() > 0xFFFFFFFFu)
				throw std::runtime_error("dtc block too large for offset encoding");
...
void DTC_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	const size_t group_bytes = positions_per_group * sizeof(DTZ_Final_Entry);
	size_t cap_groups;
	size_t max_workers;
	...
	std::vector<Sliced_EGTB_File_For_Gen<DTZ_Final_Entry, DTZ_Intermediate_Entry>*> all_tables;
	...
	for (Color me : colors)
	{
		...
		for (size_t l = 0; l < DTC_BUDGET_LAYERS; ++l)
		{
			...
			m_table->m_dtc[me][l].remove_disk_files();
	...
	if (m_is_symmetric)
		for (size_t l = 0; l < DTC_BUDGET_LAYERS; ++l)
		{
			...
			m_table->m_dtc[BLACK][l].remove_disk_files();
			m_table->m_dtc[BLACK][l].close();
		}

	save_layered_table(m_epsi, index_perm, color_out, paths.dtc_save_path(m_epsi), colors,
	...
	std::printf("  save_to_disk: done in %s (%zu layers)\n",
		format_elapsed_time(t_save_start, t_save_end).c_str(), m_num_layers);
}
