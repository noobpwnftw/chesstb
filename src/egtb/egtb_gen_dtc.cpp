...
WDL_Entry DTC_Generator::effective_opp_wdl_after_dp(Position_For_Gen& pos_gen, Move dp_move) const
{
	...
	if (!has_non_ep_move)
		return (best_ep_for_opp == WDL_Entry::LOSE && m_layer == 0) ? WDL_Entry::DRAW : best_ep_for_opp;
...
uint16_t DTC_Generator::init_entries(In_Out_Param<Thread_Pool> thread_pool)
{
	...
	m_phase_pinned = pin(m_all_groups) || pin(m_fusion_groups);

	reset_paging_window();
	const size_t below = layer_below();
	auto add_init_need = [&](size_t g, uint8_t* need_w, uint8_t* need_b) {
		need_w[m_layer * ngroups + g] = 1;
		need_w[below * ngroups + g] = 1;
		const size_t g_start = g * spg;
		const size_t g_end   = std::min(g_start + spg, ntotal);
		mark_push_target_layers(need_w, ngroups, g_start, g_end, spg);
		std::copy_n(need_w, ngroups * DTC_BUDGET_LAYERS, need_b);
	};
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		...
		page_in_window(thread_pool, &m_table->m_dtc[WHITE][0], &m_table->m_dtc[BLACK][0],
		               pos, DTC_BUDGET_LAYERS, m_all_groups, add_init_need, nullptr);
                                             ...
                                             const DTZ_File_For_Probe& dtz, size_t layer)
{
	...
	m_phase_pinned = pin_layers(thread_pool, m_fusion_groups, ngroups,
	...
	auto add_need = [&](size_t group, uint8_t* need_w, uint8_t* need_b) {
		need_w[layer * ngroups + group] = 1;
		need_b[layer * ngroups + group] = 1;
	};
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		const size_t g = m_pair_group_ids[pos];
		page_in_window(thread_pool, &m_table->m_dtc[WHITE][0], &m_table->m_dtc[BLACK][0],
		               pos, DTC_BUDGET_LAYERS, m_all_groups, add_need, nullptr);
...
size_t DTC_Generator::layer_below() const
{
	return (m_layer == 0 || m_active_pawn_slices.empty())
		? m_layer
                             ...
                             Color stm, uint16_t ply)
{
	...
	reset_paging_window();
	const size_t base = m_layer * m_table->m_dtc[WHITE][0].num_groups();
	auto add_need = [&](size_t group, uint8_t* need_w, uint8_t* need_b) {
		uint8_t* need[COLOR_NB] = { need_w, need_b };
		need[stm][base + group] = 1;
		const size_t g_start = group * spg;
		const size_t g_end   = std::min(g_start + spg, m_epsi.num_slices());
		mark_king_neighbor_reach(need[color_opp(stm)] + base, g_start, g_end, spg);
	};
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		...
		page_in_window(thread_pool, &m_table->m_dtc[WHITE][0], &m_table->m_dtc[BLACK][0],
		               pos, DTC_BUDGET_LAYERS,
		               m_iter_groups[stm], add_need,
		               &m_table->m_dtc[color_opp(stm)][m_layer]);
...
// Row 0 preserves DTZ as the clean curve's terminal changepoint; the 30
...
NODISCARD Layered_Compressed_EGTB save_compress_dtc(
	In_Out_Param<Thread_Pool> thread_pool,
	DTC_Table& table,
	const DTZ_File_For_Probe& dtz,
	DTC_Save_Cache& cache,
	...
	Layered_Rank_Table ranks)
{
	...
	auto compressed_blocks = Compressed_Block_Store(
		std::move(spill_path), bcnt, static_cast<size_t>(block_size));
...
void DTC_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	{
		const size_t paging_bytes = paging_target_bytes(m_paging_budget_bytes, group_bytes);
		cap_groups = std::max<size_t>(1, paging_bytes / group_bytes);
	...
	DTC_Save_Cache cache(std::move(all_tables), m_paging_budget_bytes);
	...
	for (Color me : colors)
	{
		...
		const bool any = gather_dtc_info(
			...
			positions_per_group, thread_pool, max_workers, m_info, ranks);

		if (!any)
		{
			std::printf("save dtc %d: singular\n", static_cast<int>(me));
			color_out[me] = Layered_Compressed_EGTB::make_singular();
		}
		else
		{
