...
bool DTC_Generator::init_entries(In_Out_Param<Thread_Pool> thread_pool)
{
	...
	// Init's closure is the fusion's groups plus their push targets, both colors.
	auto pin = [&](const std::vector<uint8_t>& groups, size_t count) {
		...
		return try_pin_phase(
			thread_pool, &m_table->m_dtc[WHITE], &m_table->m_dtc[BLACK],
	...
	for (size_t g : m_pair_group_ids)
	{
		...
		const auto rets = thread_pool->run_sync_task_on_all_threads([&](size_t) -> Iter_Result {
			...
			for (auto [chunk_start, chunk_end] : cell_it.chunks())
			{
				...
				auto rows = make_in_pair_rows<DTC_Final_Entry>(
					m_epsi, pid_in_pair, m_table->m_dtc[stm], chunk_start, chunk_end);
				...
				while (const auto row = rows.next())
				{
					     ...
					     idx = static_cast<Board_Index>(static_cast<size_t>(idx) + 1))
					{
						...
						const DTC_Final_Entry fe = row.data[static_cast<size_t>(idx) - row_lo];
						...
						{
							auto ie = reinterpret_cast<const DTC_Intermediate_Entry&>(fe);
                            ...
                            Iter_Phase start_phase, uint16_t finished_ply)
{
	...
	// Past init the fusion is isolated: retro only reaches king neighbours, which
	// stay inside it, so the need-set is the whole fusion in both colors.
	auto pin = [&](const std::vector<uint8_t>& groups, size_t count) {
		m_scratch_need[WHITE] = groups;
		m_scratch_need[BLACK] = groups;
		return try_pin_phase(
	...
	m_phase_pinned = pin(m_all_groups, m_table->m_dtc[WHITE].num_groups())
	...
	In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	for (size_t bi = 0; bi < batches.size(); ++bi)
	{
		...
		const auto fusions = compute_fusion_groups(m_table->m_dtc[WHITE], batch, 1);
...
void DTC_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	for (Color me : colors)
	{
		const Singular_Probe_Result probe = singular_probe(
			thread_pool, m_epsi, m_table->m_dtc[me], cache, me, m_epsi.num_positions(), max_workers);
		...
		else
		{
			...
			wdl_index_perm[me] = choose_storage_permutation_config(
				...
				WDL_BLOCK_SIZE,
				// Dict-less though save_compress_wdl ships dict-primed: this scores
				// a ranking, not bytes, and a trained dict is perm-dependent.
