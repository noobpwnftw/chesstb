...
private:
	...
	Table_Reader_Map<DTZ_Final_Entry> m_exit_dtz_by_material;
	Table_Reader<DTZ_Final_Entry>* m_exit_reader[COLOR_NB][Castling_Group::MAX_RIGHTS + 1]{};

	uint16_t m_max_dtz = 0;
	...
	NODISCARD DTZ_Final_Entry read_exit_dtz(const Exit_Site& site) const;
	...
	NODISCARD DTZ_Final_Entry make_initial_entry(Position_For_Gen& pos_gen) const;
	struct Init_Result
	{
		bool pending_cursed = false;
		uint16_t max_classified = 0;
	};
	NODISCARD Init_Result init_entries(In_Out_Param<Thread_Pool> thread_pool);
