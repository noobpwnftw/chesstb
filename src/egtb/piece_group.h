...
struct Piece_Group
{
	...
	struct alignas(8) Placement
	{
		...
		// In-place: replace `from` with `to`, preserving ascending sort. The list
		// is already sorted and exactly one element changes, so relocate just that
		// element in a single pass -- no full sort, and no copy/return round-trip.
		INLINE void move_square(Square from, Square to)
		{
			...
			ASSERT(i < n);
			// Only one of these two loops runs: bubble `to` up or down to its slot.
...
private:
	...
	// Bounded by the non-unique space: weights[slot] * num_legal_squares <= the
	// group's non-unique size, itself capped at MAX_NON_UNIQUE_LUT_SIZE. Left
	// unfilled where that cap rejected the LUT, since nothing reads it there.
	int32_t m_diff_on_move[MAX_PIECE_GROUP_SIZE][SQUARE_NB][SQUARE_NB];
};
