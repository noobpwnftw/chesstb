...
// A sub-table has to answer every cell from storage: the generator reads it by
// index, with no move generator to derive with. Dropped, loss-only and
// relaxed frames all leave cells to the prober, so none is usable here.
...
private:
	// Index 0 is the write target and every list starts holding it, so passing
	// that same directory again would only add a redundant lookup. Compared
	// lexically normalized, since `./wdl/` and `wdl` name one directory.
...
// The storage value a rank-encoded table holds at a logical index, or nothing
// where the block is the skip sentinel and the cell illegal. DTZ and DTM share
// the wire format, so they share this: only turning the value back into an entry
// is the metric's own.
	...
	const Reader& r, Color color, Board_Index pos)
{
	...
	const uint8_t* decompressed =
			...
			Block_Cache<>&, size_t bid) -> Block_Ptr
		{
			...
			ASSERT(dsz != 0);  // the caller short-circuits the skip sentinel
...
struct WDL_File_For_Probe : public EGTB_Sub_Reader<WDL_Entry>
{
	...
	NODISCARD WDL_Entry read(Color color, Board_Index pos) const override
	{
		...
		const uint8_t* decompressed_block =
			fetch_block_cached<WDL_File_For_Probe>(m_cache[color], block_id, [&pc](
...
struct DTM_File_For_Probe : public EGTB_Sub_Reader<DTM_Final_Entry>
{
	...
	void open(const EGTB_Paths& egtb_files, const Piece_Config& ps)
	{
		...
		// DTM runs after DTZ, so the WDL companion is present.
...
// The material's own DTZ, by logical index: DTC's widest layer is that table, so
// its pack embeds it rather than solving it a second time.
...
struct DTZ_File_For_Probe : public EGTB_Sub_Reader<DTZ_Final_Entry>
{
	static constexpr size_t TL_BLOCK_SLOTS = 16;
	...
	void open(const EGTB_Paths& egtb_files, const Piece_Config& ps)
	{
		...
		// WDL is that pass's own output, so it is there whenever the DTZ file is.
		std::filesystem::path wdl_path;
		if (!egtb_files.find_wdl_file(ps, &wdl_path))
			throw std::runtime_error("Could not find a WDL file for " + ps.name());
	...
	// The layout the file was written under, which whatever embeds it shares.
...
struct DTM50_File_For_Probe : public EGTB_Sub_Reader<DTM50_Final_Entry>
{
	...
	NODISCARD DTM50_Final_Entry read(Color color, Board_Index pos) const override
	{
		...
		const uint8_t* decoded =
			fetch_block_cached<DTM50_File_For_Probe>(m_cache[color], block_id, [&pc](
				Block_Cache<>&, size_t bid) -> Block_Ptr
		{
			...
			const uint8_t* raw = scratch.data();
			const uint8_t* p = raw;
			...
			// hmc=0 is stored at PACK LAYER 1 (pack layer 0 = flat DTM). A SINGLE
			// whose transition lands on layer 1 into DRAW drops its rank there;
			// such a cell is cursed or blessed, a class that decodes DRAW whatever
			// the value carries, so the stored 0 standing in for it is a don't-care.
				...
				switch (state)
				{
					...
					case 1: {
						...
						if (h1 > 1)
							...
							stored = 0;                                // r1 omitted, class decides
