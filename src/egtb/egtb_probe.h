...
NODISCARD INLINE bool tb_file_is_full_format(const std::filesystem::path& path, EGTB_Magic expected_magic)
{
	...
	const bool egtb_table = expected_magic == EGTB_Magic::DTZ_MAGIC
	                     || expected_magic == EGTB_Magic::DTC_MAGIC
	...
	for (size_t i = 0; i < colors.size(); ++i)
	{
		...
		if (flag & (EGTB_DROPPED_FLAG | EGTB_LOSS_ONLY_FLAG | EGTB_RELAXED_FLAG)) return false;
...
struct EGTB_Paths
{
	...
	static inline const std::string DTZ_EXT = ".lzdtz";
	...
	static inline const std::string DTZ_CKPT_EXT = ".dtz.ckpt";
	...
	void add_dtz_path(std::filesystem::path s) { add_search_path(m_dtz_paths, std::move(s)); }
	void add_dtc_path(std::filesystem::path s) { add_search_path(m_dtc_paths, std::move(s)); }
	void add_wdl_path(std::filesystem::path s) { add_search_path(m_wdl_paths, std::move(s)); }
	void add_dtm_path(std::filesystem::path s) { add_search_path(m_dtm_paths, std::move(s)); }
	void add_dtm50_path(std::filesystem::path s) { add_search_path(m_dtm50_paths, std::move(s)); }
	...
	void init_directories() const
	{
		...
		for (const auto& p : m_dtz_paths) std::filesystem::create_directories(p);
	...
	NODISCARD bool find_dtz_file(const Piece_Config& ps, std::filesystem::path* tb = nullptr) const
	{
		std::filesystem::path found;
		if (!find_tb_file(ps, DTZ_EXT, m_dtz_paths, &found)) return false;
		if (!tb_file_is_full_format(found, EGTB_Magic::DTZ_MAGIC)) return false;
		if (tb) *tb = std::move(found);
		return true;
	}
	...
	NODISCARD std::filesystem::path dtz_save_path(const Piece_Config& ps) const { return path_join(m_dtz_paths[0], ps.name() + DTZ_EXT); }
	NODISCARD std::filesystem::path dtz_info_save_path(const Piece_Config& ps) const { return path_join(m_dtz_paths[0], ps.name() + INFO_EXT); }
	...
	NODISCARD std::filesystem::path dtz_checkpoint_path(const Piece_Config& ps) const
	{
		return path_join(m_tmp_path, ps.name() + DTZ_CKPT_EXT);
	}
...
private:
	static void add_search_path(std::vector<std::filesystem::path>& paths, std::filesystem::path s)
	{
		const std::filesystem::path key = s.lexically_normal();
		for (const std::filesystem::path& p : paths)
			if (p.lexically_normal() == key) return;
		paths.emplace_back(std::move(s));
	}

	std::filesystem::path m_tmp_path = "./tmp/";
	std::vector<std::filesystem::path> m_dtz_paths = { "./dtz/" };
		...
		std::filesystem::path* tb = nullptr) const
	{
		...
		for (const auto& dir : paths)
		{
			...
			if (file_exists_case_exact(path))
			{
...
struct Table_Reader
{
	...
	NODISCARD virtual EntryT read(Color, Board_Index) const
	{ ASSERT_ALWAYS(false); return {}; }

	NODISCARD virtual EntryT read_at_hmc(Color, Board_Index, uint16_t) const
	{ ASSERT_ALWAYS(false); return {}; }

	NODISCARD virtual EntryT read_at_budget(Color, Board_Index, size_t) const
	{ ASSERT_ALWAYS(false); return {}; }
};

template <typename Reader>
NODISCARD INLINE std::optional<uint16_t> rank_encoded_stored(
	const Reader& r, Color color, Board_Index pos)
{
	const auto& pc = r.m_per_color[color];
	...
	if (pair_skip[0] == pair_skip[1]) return std::nullopt;

	const uint8_t* decompressed =
		fetch_block_cached<Reader>(r.m_cache[color], block_id, [&pc](
	...
	// 1B tier: the raw byte indexes r2v, the value it lost being the class's.
	if (pc.entry_bytes == 1) return pc.rank_to_value[decompressed[in_block_pos]];
	uint16_t rank;
	std::memcpy(&rank, decompressed + in_block_pos * sizeof(uint16_t), sizeof(rank));
...
struct WDL_File_For_Probe : public Table_Reader<WDL_Entry>
{
	...
	NODISCARD WDL_Entry read(Color color, Board_Index pos) const override
	{
		...
		const uint8_t* point_index =
			fetch_block_cached<WDL_File_For_Probe, true>(m_cache[color], block_id, [&pc](
				Block_Cache&, size_t bid) -> Block_Ptr
			{
				const auto pr = pc.offsets.get2(bid);
				const size_t out_sz =
					(bid == pc.block_cnt - 1 && pc.tail_size != 0) ? pc.tail_size : pc.block_size;
				auto buf = std::make_shared<std::vector<uint8_t>>(lz4_point_index_bytes(out_sz));
				lz4_build_point_index(
					Const_Span<uint8_t>(pc.compressed_data + pr[0], pr[1] - pr[0]),
					Span<uint8_t>(buf->data(), buf->size()));
				return buf;
			});

		const Packed_WDL_Entries entry = static_cast<Packed_WDL_Entries>(lz4_point_read(
			Const_Span<uint8_t>(pc.compressed_data + pair_skip[0], pair_skip[1] - pair_skip[0]),
...
struct DTM_File_For_Probe : public Table_Reader<DTM_Final_Entry>
{
	...
	NODISCARD DTM_Final_Entry read(Color color, Board_Index pos) const override
	{
		...
		const std::optional<uint16_t> stored =
			rank_encoded_stored<DTM_File_For_Probe>(*this, color, pos);
		if (!stored) return DTM_Final_Entry::make_illegal();
...
struct DTZ_File_For_Probe;

void load_dtz_table(
	Out_Param<DTZ_File_For_Probe> dtz,
	const Piece_Config& ps,
	std::filesystem::path path,
	EGTB_Magic dtz_magic);

struct DTZ_File_For_Probe : public Table_Reader<DTZ_Final_Entry>
{
	static constexpr size_t TL_BLOCK_SLOTS = 16;

	using Per_Color = DTM_File_For_Probe::Per_Color;
	...
	DTZ_File_For_Probe(const EGTB_Paths& egtb_files, const Piece_Config& ps,
		In_Out_Param<Thread_Pool>) :
		DTZ_File_For_Probe()
	{
		open(egtb_files, ps);
	...
	DTZ_File_For_Probe(const DTZ_File_For_Probe&) = delete;
	DTZ_File_For_Probe(DTZ_File_For_Probe&&) = delete;
	DTZ_File_For_Probe& operator=(const DTZ_File_For_Probe&) = delete;
	DTZ_File_For_Probe& operator=(DTZ_File_For_Probe&&) = delete;

	void open(const EGTB_Paths& egtb_files, const Piece_Config& ps)
	{
		std::filesystem::path dtz_path;
		if (!egtb_files.find_dtz_file(ps, &dtz_path))
			throw std::runtime_error("Could not find a DTZ file for " + ps.name());
		std::filesystem::path wdl_path;
		if (!egtb_files.find_wdl_file(ps, &wdl_path))
			throw std::runtime_error("Could not find a WDL file for " + ps.name());

		load_wdl_table(out_param(m_wdl), ps, wdl_path, EGTB_Magic::WDL_MAGIC);
		load_dtz_table(out_param(*this), ps, dtz_path, EGTB_Magic::DTZ_MAGIC);
	}

	NODISCARD uint32_t index_perm(Color color) const { return m_per_color[color].plan.perm; }

	NODISCARD DTZ_Final_Entry read(Color color, Board_Index pos) const override
	{
		const WDL_Entry w = m_wdl.read(color, pos);
		...
		if (m_is_singular[color])
			return dtz_entry_from_storage(m_single_val[color], w, 1);

		const std::optional<uint16_t> stored =
			rank_encoded_stored<DTZ_File_For_Probe>(*this, color, pos);
		if (!stored) return DTZ_Final_Entry::make_illegal();
		return dtz_entry_from_storage(*stored, w, m_per_color[color].entry_bytes);
	}

	bool m_is_singular[COLOR_NB];
	uint8_t m_single_val[COLOR_NB];
	Per_Color m_per_color[COLOR_NB];
	Memory_Mapped_File m_lzdtz_file;
	WDL_File_For_Probe m_wdl;
	mutable Block_Cache m_cache[COLOR_NB];
};
...
struct DTM_Sub_File_Flat : public Table_Reader<DTM_Final_Entry>
{
	struct Per_Color
	{
...
void layered_decode_column(const uint8_t* raw, size_t positions, size_t eb,
                           const std::vector<uint16_t>& r2v,
                           uint16_t layer, uint16_t drawn, uint16_t* out)
{
	// Rank slot in force at `layer`: one less than the number of changepoints at
	// or below it. The bitmap is BM_BYTES following the k byte.
	auto multi_rank_slot = [](const uint8_t* entry, uint16_t at) -> size_t {
		constexpr size_t words = (BM_BYTES + 7) / 8;
		std::array<uint64_t, words> bits{};
		std::memcpy(bits.data(), entry + 1, BM_BYTES);
		size_t n = 0;
		for (size_t w = 0; w < words && at >= w * 64; ++w)
		{
			const size_t count = at - w * 64 + 1;
			const uint64_t mask =
				(count >= 64) ? ~uint64_t{0} : ((uint64_t{1} << count) - 1);
			n += static_cast<size_t>(popcnt(bits[w] & mask));
		}
		ASSERT(n > 0);
		return n - 1;
	};
	...
	size_t const_idx = 0, multi_idx = 0;
	size_t single_off = 0, double_off = 0;

	auto read_rank = [eb](const uint8_t* q) -> uint16_t {
		if (eb == 1) return *q;
		uint16_t r; std::memcpy(&r, q, 2); return r;
	};

	for (size_t k = 0; k < np32; ++k)
	{
		...
		{
			case 0: {
				// CONST: one rank for every layer.
			...
			case 1: {
				...
				const bool draw_end = (entry[0] & 0x80u) != 0;
				const uint16_t h = entry[0] & 0x7Fu;
				if      (layer < h) stored = r2v[read_rank(entry + 1)];
				else if (draw_end)  stored = drawn;
				else                stored = r2v[read_rank(entry + 1 + eb)];
			...
			case 2: {
				...
				const bool draw_end = (entry[1] & 0x80u) != 0;
				const uint16_t h1 = entry[0];
				const uint16_t h2 = entry[1] & 0x7Fu;
				if      (layer < h1) stored = r2v[read_rank(entry + 2)];
				else if (layer < h2) stored = r2v[read_rank(entry + 2 + eb)];
				else if (draw_end)   stored = drawn;
				else                 stored = r2v[read_rank(entry + 2 + 2 * eb)];
			...
			default: {
				...
				const bool draw_end = (entry[0] & 0x80u) != 0;
				const size_t nk = entry[0] & 0x7Fu;
				const size_t rsel = multi_rank_slot(entry, layer);
				if (rsel == nk - 1 && draw_end)
					stored = drawn;
				else
					stored = r2v[read_rank(entry + 1 + BM_BYTES + rsel * eb)];
				++multi_idx;
				break;
			}
		}
	...
	ASSERT(const_idx == num_const && multi_idx == num_multi
		&& single_off == ss_bytes32 && double_off == ds_bytes32);
}
...
struct DTM50_File_For_Probe : public Table_Reader<DTM50_Final_Entry>
{
	static constexpr size_t TL_BLOCK_SLOTS = 16;

	static constexpr size_t HMC_COUNT = 100;

	// No rank decodes to this, so it can mark a cell the curve leaves DRAW.
	static constexpr uint16_t DRAWN_AT_LAYER = 0xFFFFu;
	...
	void open(const EGTB_Paths& egtb_files, const Piece_Config& ps)
	{
		...
		if (!egtb_files.find_wdl_file(ps, &wdl_path))
			throw std::runtime_error("Could not find a WDL file for " + ps.name()
				+ " (DTM50 consumes DTZ's wdl/ output)");
	...
	// The column at `hmc`. Pack layer 0 is the flat DTM, so hmc k is layer k + 1.
	NODISCARD DTM50_Final_Entry read_at_hmc(Color color, Board_Index pos, uint16_t hmc) const override
	{
		const WDL_Entry w = m_wdl.read(color, pos);
		ASSERT(w != WDL_Entry::ILLEGAL);
		if (w == WDL_Entry::DRAW
		...
		const uint16_t layer = static_cast<uint16_t>(hmc + 1);
		...
		const size_t cache_key = block_id * (HMC_COUNT + 1) + static_cast<size_t>(layer);
		const uint8_t* decoded =
			fetch_block_cached<DTM50_File_For_Probe>(m_cache[color], cache_key, [&pc](
				Block_Cache&, size_t key) -> Block_Ptr
		{
			const size_t bid = key / (HMC_COUNT + 1);
			const uint16_t layer = static_cast<uint16_t>(key % (HMC_COUNT + 1));
			...
			const Span<uint8_t> scratch = decompress_scratch(usz);
			lzma_decompress_into(
				scratch,
				Const_Span<uint8_t>(pc.compressed_data + doff, dsz));

			layered_decode_column<DTM50_MULTI_BITMAP_BYTES>(
				scratch.data(), positions, pc.entry_bytes, pc.rank_to_value,
				layer, DRAWN_AT_LAYER, reinterpret_cast<uint16_t*>(buf->data()));
			return buf;
		});

		uint16_t stored;
		std::memcpy(&stored, decoded + pos_in_block * sizeof(uint16_t), sizeof(stored));
		if (stored == DRAWN_AT_LAYER) return DTM50_Final_Entry::make_draw();
		return dtm50_entry_from_storage(stored, w);
	}

	bool m_is_singular[COLOR_NB];
	Per_Color m_per_color[COLOR_NB];
	Memory_Mapped_File m_pack_file;
	WDL_File_For_Probe m_wdl;
	mutable Block_Cache m_cache[COLOR_NB];
};

struct DTC_File_For_Probe;

void load_dtc_table(
	Out_Param<DTC_File_For_Probe> dtc,
	const Piece_Config& ps,
	std::filesystem::path sub_dtc);

struct DTC_File_For_Probe : public Table_Reader<DTC_Final_Entry>
{
	static constexpr size_t TL_BLOCK_SLOTS = 16;

	// Pack index 0 is the widest budget -- the embedded DTZ terminal row -- and
	// the index rises as the budget falls. Mirrors budget_of_pack_index.
	NODISCARD static uint16_t pack_index_of_budget(size_t budget)
	{
		const size_t b = std::min(budget, DTC_BUDGET_LAYERS);
		return static_cast<uint16_t>(DTC_BUDGET_LAYERS - b);
	}

	// No rank decodes to this; it marks a budget that does not settle the cell.
	static constexpr uint16_t DRAWN_AT_LAYER = 0xFFFFu;

	using Per_Color = DTM50_File_For_Probe::Per_Color;

	DTC_File_For_Probe() : m_is_singular{ false, false } {}

	DTC_File_For_Probe(const EGTB_Paths& egtb_files, const Piece_Config& ps,
		In_Out_Param<Thread_Pool>) :
		DTC_File_For_Probe()
	{
		open(egtb_files, ps);
	...
	DTC_File_For_Probe(const DTC_File_For_Probe&) = delete;
	DTC_File_For_Probe(DTC_File_For_Probe&&) = delete;
	DTC_File_For_Probe& operator=(const DTC_File_For_Probe&) = delete;
	DTC_File_For_Probe& operator=(DTC_File_For_Probe&&) = delete;

	void open(const EGTB_Paths& egtb_files, const Piece_Config& ps)
	{
		std::filesystem::path dtc_path;
		if (!egtb_files.find_dtc_file(ps, &dtc_path))
			throw std::runtime_error("Could not find a DTC file for " + ps.name());
		std::filesystem::path wdl_path;
		if (!egtb_files.find_wdl_file(ps, &wdl_path))
			throw std::runtime_error("Could not find a WDL file for " + ps.name()
				+ " (DTC consumes DTZ's wdl/ output)");

		load_wdl_table(out_param(m_wdl), ps, wdl_path, EGTB_Magic::WDL_MAGIC);
		load_dtc_table(out_param(*this), ps, dtc_path);
	}

	NODISCARD DTC_Final_Entry read_at_budget(Color color, Board_Index pos, size_t budget) const override
	{
		const WDL_Entry w = m_wdl.read(color, pos);
		ASSERT(w != WDL_Entry::ILLEGAL);
		if (w == WDL_Entry::DRAW
		...
		const uint16_t layer = pack_index_of_budget(budget);
		const Per_Color& pc = m_per_color[color];
		const size_t storage_pos =
			logical_index_to_storage_index(pc.plan, static_cast<size_t>(pos));
		const size_t block_id = storage_pos / pc.block_positions;
		const size_t pos_in_block = storage_pos % pc.block_positions;
		...
		const size_t cache_key = block_id * (DTC_PACK_LAYERS + 1) + layer;
		const uint8_t* decoded =
			fetch_block_cached<DTC_File_For_Probe>(m_cache[color], cache_key, [&pc](
				Block_Cache&, size_t key) -> Block_Ptr
		{
			const size_t bid = key / (DTC_PACK_LAYERS + 1);
			const uint16_t at = static_cast<uint16_t>(key % (DTC_PACK_LAYERS + 1));
			const size_t positions =
				(bid == pc.block_cnt - 1 && pc.tail_positions != 0)
				? pc.tail_positions
				: pc.block_positions;
			...
			layered_decode_column<DTC_MULTI_BITMAP_BYTES>(
				scratch.data(), positions, pc.entry_bytes, pc.rank_to_value,
				at, DRAWN_AT_LAYER, reinterpret_cast<uint16_t*>(buf->data()));
			return buf;
		});

		uint16_t stored;
		std::memcpy(&stored, decoded + pos_in_block * sizeof(uint16_t), sizeof(stored));
		if (stored == DRAWN_AT_LAYER) return DTC_Final_Entry::make_draw();
		return dtc_entry_from_storage(stored, w);
	}
...
struct DTM50_Sub_File_Flat : public Table_Reader<DTM50_Final_Entry>
{
	struct Per_Color
	{
		Memory_Mapped_File file;
