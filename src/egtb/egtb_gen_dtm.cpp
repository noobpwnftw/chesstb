...
bool dtm_better_for_mover(DTM_Final_Entry a, DTM_Final_Entry b)
{
...
DTM_Final_Entry DTM_Generator::read_exit_dtm(const Exit_Site& site) const
{
	const auto* reader = m_exit_reader[site.giver][site.dropped];
	if (reader == nullptr) return DTM_Final_Entry::make_draw();
...
DTM_Final_Entry DTM_Generator::effective_opp_dtm_after_dp(Position_For_Gen& pos_gen, Move dp_move) const
{
	...
	const bool has_non_ep_move = any_ep && p.visit_legal_moves(
		[](Move) FORCE_INLINE_LAMBDA { return true; });
	p.undo_move(dp_move, captured_by_dp, rights_before);

	if (!any_ep) return no_ep;
	if (!has_non_ep_move) return best_ep_for_opp;
	return dtm_better_for_mover(best_ep_for_opp, no_ep) ? best_ep_for_opp : no_ep;
}

FORCE_INLINE DTM_Final_Entry DTM_Generator::make_initial_entry(
	Position_For_Gen& pos_gen, Out_Param<uint16_t> worst_loss_dtm) const
{
	auto intermediate = [](DTM_Intermediate_Entry entry) FORCE_INLINE_LAMBDA {
		return reinterpret_cast<const DTM_Final_Entry&>(entry);
	};
	...
	*worst_loss_dtm = 0;
	...
	(void)pos.visit_legal_moves([&](Move m) FORCE_INLINE_LAMBDA {
		...
		if (!is_cap && !m.is_promotion()
		    && resolve_castling(pos_gen, m, out_param(site)))
		{
			any_bound = true;
			const DTM_Final_Entry e = read_exit_dtm(site);
			if (e.is_loss())
			{
				saw_win = true;
				update_min(best_win_dtm, static_cast<uint16_t>(e.value() + 1));
			}
			else if (e.is_win())
				update_max(*worst_loss_dtm, static_cast<uint16_t>(e.value() + 1));
			else if (e.is_draw())
				saw_draw = true;
			return false;
		}
		if (!is_cap && !m.is_promotion())
		{
			...
			// Retro cannot cross pawn slices.
		...
		if (sub_e.is_illegal()) return false;
		if (sub_e.is_loss())
		{
			saw_win = true;
	...
	if (!any_legal)
	{
		if (pos.is_in_check()) return DTM_Final_Entry::make_loss(0);
		return intermediate(DTM_Intermediate_Entry{});
	}
	...
	return intermediate((any_bound && !saw_draw)
		? DTM_Intermediate_Entry::make_bound(*worst_loss_dtm)
...
uint16_t DTM_Generator::init_entries(In_Out_Param<Thread_Pool> thread_pool)
{
	...
	auto pin = [&](const std::vector<uint8_t>& groups, size_t count) {
		...
		return try_pin_phase(
			thread_pool, &m_table->m_dtm[WHITE], &m_table->m_dtm[BLACK],
	...
	reset_paging_window();
	auto add_init_need = [&](size_t g, uint8_t* need_w, uint8_t* need_b) {
		need_w[g] = 1;
		const size_t g_start = g * spg;
		const size_t g_end   = std::min(g_start + spg, ntotal);
		mark_push_target_reach(need_w, g_start, g_end, spg);
		std::copy_n(need_w, ngroups, need_b);
	};
	...
	uint16_t max_init = 0;
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		const size_t g = m_pair_group_ids[pos];
		page_in_window(thread_pool, &m_table->m_dtm[WHITE], &m_table->m_dtm[BLACK],
		               pos, 1, m_all_groups, add_init_need, nullptr);
		...
		const auto rets = thread_pool->run_sync_task_on_all_threads([&](size_t) -> uint16_t {
			...
			for (const Board_Index idx : group_it.indices())
			{
				...
				for (Color us : { WHITE, BLACK })
				{
					...
					{
						write_dtm(us, idx,
						          reinterpret_cast<const DTM_Intermediate_Entry&>(entry));
...
FORCE_INLINE DTM_Generator::Loss_Verification_Result DTM_Generator::check_loss(
	...
	uint16_t ply, DTM_Intermediate_Entry hint) const
{
	...
	const bool disqualified = pos.visit_legal_moves([&](Move m) FORCE_INLINE_LAMBDA {
		...
		Board_Index child = BOARD_INDEX_NONE;
		if (!pos.move_is_capture(m)
		    && piece_type(pos.piece_at(m.from())) != PAWN
		    && !m.is_castling())
			child = next_quiet_index(pos_gen, m);

		if (child == BOARD_INDEX_NONE)
		{
			if (hint.bound() == 0)  return true;
			if (hint.bound() > ply) return true;
			update_max(max_contribution, hint.bound());
			return false;
		}
...
FORCE_INLINE void DTM_Generator::retro_mark_win_in_1(Position_For_Gen& pos_gen)
{
	Position& pos = pos_gen.board_unchecked();
	const Color opp = color_opp(pos.turn());
	pos.visit_pseudo_legal_pre_quiets([&](Move m) FORCE_INLINE_LAMBDA {
		...
		if (e.is_loss()) return;
		if (e.is_win() && e.value() <= 1) return;
		write_dtm(opp, pred, DTM_Final_Entry::make_win(1));
	});
}

FORCE_INLINE void DTM_Generator::retro_mark_changed(Position_For_Gen& pos_gen)
{
	Position& pos = pos_gen.board_unchecked();
	const Color opp = color_opp(pos.turn());
	pos.visit_pseudo_legal_pre_quiets([&](Move m) FORCE_INLINE_LAMBDA {
		...
		if (ie.has_change()) return;
		m_table->m_dtm[opp].add_flags(pred, DTM_FLAG_CHANGE);
		mark_iter(opp, pred, m_table->m_dtm[opp]);
	});
}

FORCE_INLINE void DTM_Generator::retro_mark_wins(
	Position_For_Gen& pos_gen, uint16_t target_dtm)
{
	Position& pos = pos_gen.board_unchecked();
	const Color opp = color_opp(pos.turn());
	...
	pos.visit_pseudo_legal_pre_quiets([&](Move m) FORCE_INLINE_LAMBDA {
		...
		if (e.is_win() && e.value() <= target_dtm) return;
                                                   ...
                                                   Color stm, uint16_t ply)
{
	...
	reset_paging_window();
	auto add_need = [&](size_t group, uint8_t* need_w, uint8_t* need_b) {
		uint8_t* need[COLOR_NB] = { need_w, need_b };
		need[stm][group] = 1;
		const size_t g_start = group * spg;
		const size_t g_end   = std::min(g_start + spg, m_epsi.num_slices());
		mark_king_neighbor_reach(need[color_opp(stm)], g_start, g_end, spg);
	};
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		...
		page_in_window(thread_pool, &m_table->m_dtm[WHITE], &m_table->m_dtm[BLACK],
		               pos, 1,
		               m_iter_groups[stm], add_need, &m_table->m_dtm[color_opp(stm)]);
		...
		if (ply != 0)
		{
			...
			const auto mark_rets = thread_pool->run_sync_task_on_all_threads([&](size_t) -> bool {
				...
				for (auto [chunk_start, chunk_end] : mark_it.chunks())
				{
					...
					for (const auto [idx, fe] : make_in_pair_cells<DTM_Final_Entry>(
							m_epsi, pid_in_pair, m_table->m_dtm[stm], chunk_start, chunk_end))
					{
		...
		const auto rets = thread_pool->run_sync_task_on_all_threads([&](size_t) -> Iter_Result {
			...
			auto reverify = [&](Board_Index i, DTM_Intermediate_Entry ie) FORCE_INLINE_LAMBDA {
				pos_gen.seek(i);
				...
				write_dtm(stm, i, DTM_Final_Entry::make_loss(res.loss_dtm));
				update_max(local.max_dtm, static_cast<uint16_t>(res.loss_dtm + 1));
				retro_mark_wins(pos_gen, res.loss_dtm + 1);
				local.wrote = true;
			};
			...
			for (auto [chunk_start, chunk_end] : cell_it.chunks())
			{
						...
						m_epsi, pid_in_pair, m_table->m_dtm[stm], chunk_start, chunk_end))
				{
					...
					if (fe.is_draw())
					{
						...
						if (ie.has_change() || bound != 0) chunk.any_intermediate = true;
						if (!ie.has_change() && !(ply != 0 && bound == ply))
							continue;
						reverify(idx, ie);
					}
					else
					{
						...
						else          retro_mark_wins(pos_gen, target);
						local.wrote = true;
						update_max(local.max_dtm, target);
					}
				}
		...
		for (const Iter_Result& r : rets) {
			...
			update_max(global.max_dtm, r.max_dtm);
...
void DTM_Generator::iterate(In_Out_Param<Thread_Pool> thread_pool, uint16_t finished_ply)
{
	...
	m_phase_pinned = pin(m_all_groups, m_table->m_dtm[WHITE].num_groups())
	...
	while (finished_ply < DTM_SCORE_MAX)
	{
		...
		update_max(m_max_dtm, rw.max_dtm);
		update_max(m_max_dtm, rb.max_dtm);
		if (!rw.wrote && !rb.wrote && finished_ply > m_max_dtm) break;
		if (egtb_is_interrupt_requested(true))
			throw DTM_Interrupted{ 0, 0, finished_ply, m_max_dtm };
	}
}

void DTM_Generator::gen(
	Table_Reader_Map<DTM_Final_Entry> sub_dtm,
	Table_Reader_Map<DTM_Final_Entry> exit_dtm,
	In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	uint16_t resume_max_dtm = 0;
	{
		...
		{
			m_table->m_dtm[WHITE].remove_disk_files(thread_pool);
			m_table->m_dtm[BLACK].remove_disk_files(thread_pool);
		}
	}
	...
	for (size_t bi = 0; bi < batches.size(); ++bi)
	{
		...
		const auto fusions = compute_fusion_groups(m_table->m_dtm[WHITE], batch, 1, 1);
...
void DTM_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	{
		const size_t paging_bytes = paging_target_bytes(m_paging_budget_bytes, bytes_per_group);
		cap_groups = std::max<size_t>(1, paging_bytes / bytes_per_group);
	...
	DTM_Save_Cache cache(&m_table->m_dtm[WHITE], &m_table->m_dtm[BLACK], m_paging_budget_bytes);
	...
	for (Color me : colors)
	{
		...
		dtm_entry_bytes[me] = (dtm_rank[me].rank_to_value.size() <= 256) ? 1 : 2;
	}

	for (Color me : colors)
	{
		if (dtm_rank[me].rank_to_value.size() > 1)
		{
			dtm_index_perm[me] = choose_storage_permutation_config(
				...
				[&](uint32_t perm) {
					return make_entry_block_source(
						...
						DTM_BLOCK_SIZE, dtm_entry_bytes[me], thread_pool->num_workers());
				},
				DTM_BLOCK_SIZE,
				std::make_unique<LZMA_Rank_Compress_Helper>(
					dtm_rank[me], dtm_entry_bytes[me], EGTB_GEN_LOSS_ONLY
						? Rank_Storage_Mode::DTM_LOSS_ONLY : Rank_Storage_Mode::DTM),
				/*max_samples=*/64,
				max_workers,
				"choose_dtm_storage");
		}
		...
		dtm_save[me] = save_compress_egtb(
			thread_pool, src, me, dtm_entry_bytes[me], DTM_BLOCK_SIZE, max_workers,
			paths.block_spill_path(m_epsi, me), dtm_rank[me],
			EGTB_GEN_LOSS_ONLY ? Rank_Storage_Mode::DTM_LOSS_ONLY : Rank_Storage_Mode::DTM);
		cache.purge(me);
		m_table->m_dtm[me].remove_disk_files(thread_pool);
	...
	save_egtb_table(thread_pool, m_epsi, dtm_index_perm, dtm_save, paths.dtm_save_path(m_epsi), colors,
		EGTB_Magic::DTM_MAGIC, EGTB_GEN_LOSS_ONLY, /*relaxed=*/false);
