		...
		const auto rets = thread_pool->run_sync_task_on_all_threads([&](size_t) -> Iter_Result {
			...
			for (auto [chunk_start, chunk_end] : cell_it.chunks())
			{
				     ...
				     idx = static_cast<Board_Index>(static_cast<size_t>(idx) + 1))
				{
					...
					const DTM_Any_Entry cell = read_dtm_any(stm, idx);

					// Intermediate handles draws with flags (CHANGE / PAWN_EVAL
					// routing); Final handles classified WIN/LOSS retros. Visit fuses
					// chunk-state bookkeeping with the action decode; bare DRAW
					...
					const Iter_Action action = std::visit(overload{
						[&](DTM_Intermediate_Entry ie) -> Iter_Action {
							...
							if (ie.has_pawn_eval()
							    && ply == static_cast<uint16_t>(ie.value()))
