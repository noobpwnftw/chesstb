...
// Prefer the highest class, shortest win, or longest loss.
                                                ...
                                                Out_Param<uint16_t> worst_loss_dtm) const
{
	*worst_loss_dtm = 0;
	...
	{
		// Every move loses through a subtable.
		if (*worst_loss_dtm > 0)
			return DTM_Final_Entry::make_loss(*worst_loss_dtm);
	}
	// Keep iteration alive until the subtable loss bound is reached.
	...
	uint16_t ply, DTM_Intermediate_Entry hint) const
{
	...
	for (size_t i = 0; i < ml.size(); ++i)
	{
		...
		{
			if (!hint.has_pawn_eval()) return r;
			if (hint.value() > ply) return r;
			update_max(max_contribution, static_cast<uint16_t>(hint.value()));
			continue;
		}
	...
	for (size_t g : m_pair_group_ids)
	{
		...
		const auto rets = thread_pool->run_sync_task_on_all_threads([&](size_t) -> Iter_Result {
			...
			for (auto [chunk_start, chunk_end] : cell_it.chunks())
			{
				...
				auto rows = make_in_pair_rows<DTM_Final_Entry>(
				...
				while (const auto row = rows.next())
				{
					...
					{
						const DTM_Final_Entry fe = row.data[static_cast<size_t>(idx) - row_lo];
						...
						{
							const auto res = check_loss(
								pos_gen, ply,
								reinterpret_cast<const DTM_Intermediate_Entry&>(fe));
							if (!res.is_loss)
							{
								// A cleared CHANGE requires one more ply.
...
void DTM_Generator::iterate(In_Out_Param<Thread_Pool> thread_pool, uint16_t finished_ply)
{
	...
	while (finished_ply < DTM_SCORE_MAX)
	{
		...
		// Subtable seeds can remain dormant until their classified ply.
	...
	In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	for (size_t bi = 0; bi < batches.size(); ++bi)
	{
		...
		const auto fusions = compute_fusion_groups(m_table->m_dtm[WHITE], batch, 1);
...
void DTM_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	for (Color me : colors)
	{
		if (dtm_rank[me].rank_to_value.size() > 1)
		{
			dtm_index_perm[me] = choose_storage_permutation_config(
				...
				std::make_unique<LZMA_Rank_Compress_Helper>(
					dtm_rank[me], dtm_entry_bytes[me], &dtm_storage_fn<EGTB_GEN_LOSS_ONLY>),
				/*max_samples=*/64,
				"choose_dtm_storage");
		}
		...
		dtm_save[me] = save_compress_egtb(
			...
			max_workers, dtm_rank[me], &dtm_storage_fn<EGTB_GEN_LOSS_ONLY>);
		cache.purge(me);
	...
	save_egtb_table(m_epsi, dtm_index_perm, dtm_save, paths.dtm_save_path(m_epsi), colors,
		EGTB_Magic::DTM_MAGIC, EGTB_GEN_LOSS_ONLY);
