	...
	m_diff_on_move{}
{
	...
	int32_t weights[MAX_PIECE_GROUP_SIZE] = {};
	size_t non_unique_size = 1;
	bool build_non_unique_lut = true;
	for (size_t i = 0; i < count; ++i)
	{
		weights[i] = narrowing_static_cast<int32_t>(non_unique_size);
	...
	if (build_non_unique_lut)
	{
		...
		for (Placement_Index r = 0; r < static_cast<Placement_Index>(m_table_size); ++r)
		{
			...
			m_unique_to_non_unique[r] = static_cast<uint32_t>(canon_nu);
		...
		for (size_t slot = 0; slot < count; ++slot)
		{
			for (size_t f = 0; f < SQUARE_NB; ++f)
			{
				...
				for (size_t t = 0; t < SQUARE_NB; ++t)
				{
					...
					if (pt < 0) continue;
					m_diff_on_move[slot][f][t] = weights[slot] * (pt - pf);
				}
			}
		}
	}
