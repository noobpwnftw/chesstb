...
)
{
	...
	if (key.value() != (ps.min_material_key().value() & 0x3FFFFFFFu))
...
void load_rank_encoded_table(
	...
	const char* kind,
	size_t wide_entry_bytes
)
{
	if (!file.open_readonly(path.c_str()))
		throw std::runtime_error(std::string("Could not open ") + kind + " file trying to load " + path.string());

	const Const_Span<uint8_t> input = file.data_span();
	if ((input.size() & 63) != 8)
		throw std::runtime_error(std::string("Invalid ") + kind + " file size trying to load " + path.string());

	Serial_Memory_Reader reader(input);

	const uint32_t magic = reader.read<uint32_t>();
	if (magic != narrowing_static_cast<uint32_t>(expected_magic))
		throw std::runtime_error(std::string("Invalid ") + kind + " file magic trying to load " + path.string());

	const uint32_t key_and_table_num = reader.read<uint32_t>();
	const Material_Key key = static_cast<Material_Key>(key_and_table_num >> 2u);
	if (key.value() != (ps.min_material_key().value() & 0x3FFFFFFFu))
		throw std::runtime_error(std::string("Wrong material key in ") + kind + " file " + path.string());
	...
	for (const Color i : table_colors)
	{
		...
		{
			tb->m_is_singular[i] = true;
			tb->m_single_val[i] = reader.read<uint8_t>();
		}
		else
		{
			tb->m_is_singular[i] = false;
			const uint32_t perm = reader.read<uint32_t>();
			if (!index_permutation_config_is_valid(epsi, perm))
				throw std::runtime_error(std::string("Invalid ") + kind + " index permutation for " + path.string());
			tb->m_per_color[i].plan = make_index_permutation_plan(epsi, perm);
			auto& pc = tb->m_per_color[i];
			pc.entry_bytes = reader.read<uint8_t>();
			if (pc.entry_bytes != wide_entry_bytes && pc.entry_bytes != 1)
				throw std::runtime_error(std::string("Bad ") + kind + " entry_bytes in " + path.string());
	...
	for (const Color i : table_colors)
	{
		if (tb->m_is_singular[i]) continue;
		auto& pc = tb->m_per_color[i];
	...
	for (const Color i : table_colors)
	{
		if (tb->m_is_singular[i]) continue;
		reader.align(64);
		tb->m_per_color[i].compressed_data = reader.caret();
		reader.advance(data_size[i]);
	}
...
{
	load_rank_encoded_table<DTM_File_For_Probe>(dtm, dtm->m_lzdtm_file, ps, sub_dtm,
		dtm_magic, "DTM", sizeof(DTM_Final_Entry));
}
...
{
	load_rank_encoded_table<DTZ_File_For_Probe>(dtz, dtz->m_lzdtz_file, ps, path,
		dtz_magic, "DTZ", sizeof(DTZ_Final_Entry));
}

// Wire format: see save_layered_table.
template <typename Reader>
static void load_layered_pack(
	Reader& dtm50,
	const Piece_Config& ps,
	const std::filesystem::path& sub_dtm50,
	EGTB_Magic pack_magic,
	const char* kind
)
{
	if (!dtm50.m_pack_file.open_readonly(sub_dtm50.c_str()))
		throw std::runtime_error(std::string("Could not open ") + kind + " file trying to load " + sub_dtm50.string());

	const Const_Span<uint8_t> input = dtm50.m_pack_file.data_span();
	if ((input.size() & 63) != 8)
		throw std::runtime_error(std::string("Invalid ") + kind + " file size trying to load " + sub_dtm50.string());

	Serial_Memory_Reader reader(input);

	const uint32_t magic = reader.read<uint32_t>();
	if (magic != narrowing_static_cast<uint32_t>(pack_magic))
		throw std::runtime_error(std::string("Invalid ") + kind + " file magic trying to load " + sub_dtm50.string());

	const uint32_t key_and_table_num = reader.read<uint32_t>();
	const Material_Key key = static_cast<Material_Key>(key_and_table_num >> 2u);
	if (key.value() != (ps.min_material_key().value() & 0x3FFFFFFFu))
		throw std::runtime_error(std::string("Wrong material key in ") + kind + " file " + sub_dtm50.string());
	...
	for (const Color i : table_colors)
	{
		...
		if (flag & EGTB_SINGULAR_FLAG)
		{
			...
			if (reader.read<uint8_t>() != 0)
				throw std::runtime_error(std::string(kind) + " singular value must be DRAW in " + sub_dtm50.string());
		}
		else
		{
			...
			const uint32_t perm = reader.read<uint32_t>();
			if (!index_permutation_config_is_valid(epsi, perm))
				throw std::runtime_error(std::string("Invalid ") + kind + " index permutation for " + sub_dtm50.string());
			...
			if (pc.entry_bytes != 1 && pc.entry_bytes != 2)
				throw std::runtime_error(std::string("Bad ") + kind + " entry_bytes in " + sub_dtm50.string());
...
{
	load_layered_pack(*dtm50, ps, sub_dtm50, EGTB_Magic::DTM50_MAGIC, "DTM50");
}
...
{
	load_layered_pack(*dtc, ps, sub_dtc, EGTB_Magic::DTC_MAGIC, "DTC");
}
...
)
{
	...
	for (const Color i : table_colors)
	{
		...
		if (!out_file.create(tmp_path.c_str()))
		...
		if (is_singular[i])
		{
			...
			thread_pool->run_sync_task_on_all_threads([&](size_t) {
				std::vector<DTM_Final_Entry> out(FILL_CHUNK);
				for (;;)
				{
					...
					for (size_t p = start; p < stop; ++p)
					{
						...
						out[p - start] = dtm_entry_from_storage(sv, w);
					}
					const size_t bytes = (stop - start) * sizeof(DTM_Final_Entry);
					if (!out_file.write_at(start * sizeof(DTM_Final_Entry),
							Const_Span<uint8_t>(reinterpret_cast<const uint8_t*>(out.data()), bytes)))
						print_and_abort("Could not write flat sub-DTM\n");
				}
			});
		}
		else
		{
			...
			thread_pool->run_sync_task_on_all_threads([&](size_t) {
				std::vector<DTM_Final_Entry> out(positions_per_block);
				for (;;)
				{
					...
					if (dsz == 0)
					{
						std::memset(out.data(), 0, positions * sizeof(DTM_Final_Entry));
					}
					else
					{
						const Span<uint8_t> scratch = decompress_scratch(decode_sz);
						lzma_decompress_into(
							scratch, Const_Span<uint8_t>(data[i] + doff, dsz));
						...
						if (entry_bytes[i] == 1)
						{
							for (size_t k = 0; k < positions; ++k)
							{
								...
								out[k] = dtm_entry_from_storage(stored, w);
							}
						}
						else
						{
					...
					const size_t bytes = positions * sizeof(DTM_Final_Entry);
					if (!out_file.write_at(pos * sizeof(DTM_Final_Entry),
							Const_Span<uint8_t>(reinterpret_cast<const uint8_t*>(out.data()), bytes)))
						print_and_abort("Could not write flat sub-DTM\n");
				}
			});
		...
		if (!out_map.open_readonly(tmp_path.c_str()))
			throw std::runtime_error("Could not reopen flat sub-DTM at " + tmp_path.string());
...
)
{
	...
	if (!egtb_files.find_wdl_file(ps, &wdl_path))
		throw std::runtime_error("Could not find a WDL file for " + ps.name()
			+ " (DTM50 consumes DTZ's wdl/ output for class disambiguation)");
	...
	for (const Color i : table_colors)
	{
		...
		if (is_singular[i])
		{
			...
			constexpr size_t ZERO_CHUNK = 1u << 20;
			const std::vector<uint8_t> zeros(std::min(file_bytes, ZERO_CHUNK));
			for (size_t off = 0; off < file_bytes; off += zeros.size())
				if (!out_file.write_at(off, Const_Span<uint8_t>(
						zeros.data(), std::min(zeros.size(), file_bytes - off))))
					print_and_abort("Could not write flat sub-DTM50\n");
			out_file.close_file();
			Memory_Mapped_File out_map(Memory_Mapped_File::Access_Advice::RANDOM);
			if (!out_map.open_readonly(tmp_path.c_str()))
				throw std::runtime_error("Could not reopen flat sub-DTM50 at " + tmp_path.string());
			flat->m_per_color[i].file = std::move(out_map);
			continue;
		}
		...
		thread_pool->run_sync_task_on_all_threads([&](size_t) {
			std::vector<DTM50_Final_Entry> out(ppb);
			for (;;)
			{
				...
				for (size_t k = 0; k < this_bp; ++k)
				{
					...
					const WDL_Entry w = wdl.read(i, static_cast<Board_Index>(logical));
					out[k] = dtm50_entry_from_storage(stored, w);
				}
				ASSERT(const_idx == num_const && multi_idx == num_multi
				    && single_off == ss_bytes32 && double_off == ds_bytes32);
				}
				const size_t bytes = this_bp * sizeof(DTM50_Final_Entry);
				if (!out_file.write_at(base_pos * sizeof(DTM50_Final_Entry),
						Const_Span<uint8_t>(reinterpret_cast<const uint8_t*>(out.data()), bytes)))
					print_and_abort("Could not write flat sub-DTM50\n");
			}
		});
