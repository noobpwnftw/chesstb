...
// Wire format: see save_egtb_table. DTZ and DTM write the same one, so the load
// is shared and the caller names the metric for the errors.
namespace {
	...
	for (const Color i : table_colors)
	{
		...
		thread_pool->run_sync_task_on_all_threads([&](size_t) {
			for (;;)
			{
				...
				// transition lands on layer 1 into DRAW); such a cell is cursed or
				// blessed, a class that decodes DRAW whatever the value carries,
				// so the stored 0 standing in for the dropped rank is a don't-care.
