...
)
{
	...
	if (!reader.is_end_checksum_ok(static_cast<uint64_t>(EGTB_CHECKSUM_INIT_VALUE)))
		throw std::runtime_error(std::string("Invalid ") + kind + " file checksum trying to load " + path.string());
...
)
{
	...
	Serial_Memory_Reader reader(input);
	if (!reader.is_end_checksum_ok(static_cast<uint64_t>(EGTB_CHECKSUM_INIT_VALUE)))
		throw std::runtime_error(std::string("Invalid ") + kind + " file checksum trying to load " + sub_dtm50.string());
