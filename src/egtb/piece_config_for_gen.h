...
enum struct Symmetry_Group : uint8_t {
	NONE,          // castling: the right names a side of the board, so nothing folds
...
inline constexpr Square ANCHOR_TRIANGLE_SQUARES[] = {
	SQ_A1, SQ_B1, SQ_C1, SQ_D1,
	SQ_B2, SQ_C2, SQ_D2,
	SQ_C3, SQ_D3,
	SQ_D4,
...
public:
	...
	NODISCARD bool has_castling() const { return m_has_castling; }
	...
	NODISCARD static Board_Index shift_digit(Board_Index pos, ptrdiff_t old_digit,
	                                         ptrdiff_t new_digit, size_t weight)
	{
		return pos + (new_digit - old_digit) * static_cast<ptrdiff_t>(weight);
	}
	...
	NODISCARD Board_Index change_single_group_index(
		Board_Index pos, Piece_Group::Placement_Index old_index,
		Piece_Group::Placement_Index new_index, Piece_Class set) const
	{
		return shift_digit(pos, old_index, new_index, m_weights[set]);
	}

	NODISCARD Board_Index change_king_slice(Board_Index pos, int32_t old_id, int32_t new_id) const
	{
		return shift_digit(pos, old_id, new_id, m_within_slice_size);
	}

	NODISCARD Board_Index change_pawn_slice(Board_Index pos, int32_t old_id, int32_t new_id) const
	{
		return shift_digit(pos, old_id, new_id, m_pawn_slice_stride);
	}
	...
	NODISCARD bool same_slice(Board_Index a, Board_Index b) const
	{
		if (m_within_slice_size <= 1) return false;
		return static_cast<size_t>(a) / m_within_slice_size_div
		    == static_cast<size_t>(b) / m_within_slice_size_div;
	}

	// Caller must pass a board holding a complete, conflict-free cell of the same
	// slice, with `placements` describing it.
	template <bool ASSUME_LEGAL>
	bool patch_board(const Decomposed_Board_Index& index, In_Out_Param<Position> board,
