...
struct DTM_Table
{
	...
	NODISCARD INLINE DTM_Any_Entry read_any(Color stm, Board_Index pos) const
	{
		auto& tbl = m_dtm[stm];
		const DTM_Final_Entry& fe = tbl.template view_at<DTM_Final_Entry>(pos);
		if (fe.is_illegal() || !fe.is_draw())
			return fe;
		return reinterpret_cast<const DTM_Intermediate_Entry&>(fe);
	}
	template <typename EntryT>
...
private:
	...
	NODISCARD INLINE DTM_Any_Entry read_dtm_any(Color stm, Board_Index pos) const
	{
		return m_table->read_any(stm, pos);
	}
