...
Layered_Compressed_EGTB::Layered_Compressed_EGTB(
	Compressed_Block_Store&& compressed_blocks,
	std::vector<uint64_t> usizes,
	size_t block_positions,
	size_t tail_positions,
	Layered_Rank_Table ranks
) :
	m_is_singular(false),
	m_is_dropped(false),
	m_block_positions(narrowing_static_cast<uint32_t>(block_positions)),
	m_tail_positions(narrowing_static_cast<uint32_t>(tail_positions)),
	m_compressed_blocks(std::move(compressed_blocks)),
	m_usizes(std::move(usizes)),
	m_total_compressed_size(0),
                           ...
                           Const_Span<uint8_t> slack)
{
	...
	while (p < nbytes)
	{
		...
		// Overlapping copies repeat the period p - src, as LZ4 matches do.
		auto extend = [&](size_t src) {
			const auto period = static_cast<uint32_t>(p - src);
			size_t len = 0;
			while (len < MAX_MATCH && p + len < nbytes
				&& byte_admits(p + len, byte_at(src + static_cast<uint32_t>(len) % period)))
				++len;
			return len;
		};
		...
		const auto best_period = static_cast<uint32_t>(p - best_q);
		for (size_t k = 0; k < best_len; ++k)
		{
			const uint8_t want = byte_at(best_q + static_cast<uint32_t>(k) % best_period);
	...
	Span<Packed_WDL_Entries> data, Const_Span<WDL_Stored> caps)
{
	...
	for (size_t begin = 0, end = 0; begin < size; begin = end)
	{
		...
		for (size_t i = begin; i < end; ++i)
		{
			...
			cells[i] = nibble_admits(slack[i], cap, cells[i], fill) ? fill : cap;
		}
	}
...
)
{
	...
	const size_t workers = std::max<size_t>(1, std::min(capped_workers, num_blocks_to_use));
	thread_pool->run_sync_task_on_multiple_threads(workers, [&](size_t worker_id) {
		...
		for (;;)
		{
			...
			if (i >= num_blocks_to_use) break;
			const size_t bid = i * split;
			const auto blk = src.get(worker_id, bid, Span<uint8_t>(scratch.get(), block_size));
			// An all-don't-care block comes back empty; it samples as zeros.
			ASSERT(blk.empty() || blk.size() == block_size);
			if (blk.empty())
				std::memset(dist_buf.get() + i * block_size, 0, block_size);
			else
	...
	const char* task_name)
{
	...
	for (uint32_t perm = 0; perm < candidates; ++perm)
	{
		...
		const size_t capped = (max_workers == 0)
			? pool_workers
			: std::min(max_workers, pool_workers);
		const size_t workers = std::min<size_t>(capped, sample_cnt);

		const auto partial = thread_pool->run_sync_task_on_multiple_threads(workers, [&](size_t worker_id) -> uint64_t {
			...
			for (;;)
			{
				...
				const Const_Span<uint8_t> block = src.get(
					worker_id, block_id, Span<uint8_t>(scratch.get(), source_block_size));
...
)
{
	...
	auto dict = make_dict_for_wdl(thread_pool, dict_src, block_size, max_workers);
...
size_t LZMA_Rank_Compress_Helper::compress_impl(
	Span<uint8_t> dest, Const_Span<uint8_t> src)
{
	...
	const uint16_t illegal_v =
		rank_storage_value<Mode>(DTZ_Final_Entry::ILLEGAL_VAL, m_entry_bytes);
	for (size_t i = 0; i < entries; ++i)
		work[i] = rank_storage_value<Mode>(in[i], m_entry_bytes);
...
size_t LZMA_Rank_Compress_Helper::compress(Span<uint8_t> dest, Const_Span<uint8_t> src)
{
	...
	case Rank_Storage_Mode::DTZ:
		return compress_impl<Rank_Storage_Mode::DTZ>(dest, src);
	case Rank_Storage_Mode::DTZ_LOSS_ONLY:
		return compress_impl<Rank_Storage_Mode::DTZ_LOSS_ONLY>(dest, src);
	case Rank_Storage_Mode::DTM:
		return compress_impl<Rank_Storage_Mode::DTM>(dest, src);
	case Rank_Storage_Mode::DTM_LOSS_ONLY:
		return compress_impl<Rank_Storage_Mode::DTM_LOSS_ONLY>(dest, src);
	}
	ASSUME(false);
	return 0;
}
...
)
{
	...
	if (rank_table.rank_to_value.empty()
	    || (rank_table.rank_to_value.size() == 1 && rank_table.rank_to_value[0] <= 0xFFu))
	{
		printf("%s: singular\n", task_name.c_str());
	...
	auto compressed_blocks = compress_blocks(
		...
		src,
		block_size,
		std::make_unique<LZMA_Rank_Compress_Helper>(rank_table, entry_bytes, storage_mode),
		max_workers,
		task_name,
		std::move(spill_path)
	);
...
)
{
	...
	thread_pool->run_sync_task_on_all_threads([&](size_t) {
		...
		for (;;)
		{
			...
			const Const_Span<uint8_t> b = blocks.block(k);
			scratch.resize(b.size());
			std::memcpy(scratch.data(), b.data(), b.size());
			if (!writer->write_at(base_offset + dst_offsets[k], Const_Span<uint8_t>(scratch)))
...
)
{
	...
	if (!writer.create(tmp.c_str(), EGTB_CHECKSUM_INIT_VALUE))
		print_and_abort("Failed to create %s\n", tmp.c_str());

	writer.write<uint32_t>(narrowing_static_cast<uint32_t>(magic));
	writer.write<uint32_t>(static_cast<uint32_t>((ps.min_material_key().value() << 2ull) + table_colors.size()));
	...
	for (const Color i : table_colors)
	{
		...
		write_blocks_concurrent(thread_pool, In_Out_Param(writer), t.compressed_blocks());
		writer.zero_align(64);
	}
...
)
{
	...
	const uint8_t mod_flags =
		static_cast<uint8_t>((loss_only ? EGTB_LOSS_ONLY_FLAG : uint8_t{0})
		                   | (relaxed ? EGTB_RELAXED_FLAG : uint8_t{0}));
...
void save_layered_table(
	...
	const Piece_Config& ps,
	const uint32_t index_perm[COLOR_NB],
	const Layered_Compressed_EGTB color_out[COLOR_NB],
	...
	EGTB_Magic magic)
{
	...
	for (Color c : colors)
	{
		const Layered_Compressed_EGTB& co = color_out[c];
		if (!co.has_payload()) continue;
		const size_t n = co.num_blocks();
		std::vector<uint64_t> off(n + 1);
		size_t cur = 0;
		for (size_t k = 0; k < n; ++k)
		{
		...
		for (size_t k = 0; k < n; ++k) usz[k] = co.usize(k);
	...
	for (Color c : colors)
	{
		...
		if (color_out[c].is_singular()) file_size += 1;
			...
			file_size += color_out[c].rank_table().rank_to_value.size() * 2;
		}
	}
	for (Color c : colors)
	...
	const auto tmp = file_path.string() + ".tmp";
	Serial_File_Writer w;
	if (!w.create(tmp.c_str(), EGTB_CHECKSUM_INIT_VALUE))
		print_and_abort("Failed to create %s\n", tmp.c_str());

	w.write<uint32_t>(narrowing_static_cast<uint32_t>(magic));
	w.write<uint32_t>(static_cast<uint32_t>(
	...
	for (Color c : colors)
	{
		...
		else
		{
			...
			w.write<uint8_t>(co.entry_bytes());
			w.write<uint32_t>(co.block_positions());
			w.write<uint64_t>(co.num_blocks());
			w.write<uint32_t>(co.tail_positions());
			w.write<uint64_t>(static_cast<uint64_t>(co.total_compressed_size()));
			w.write<uint16_t>(static_cast<uint16_t>(co.rank_table().rank_to_value.size()));
			for (uint16_t v : co.rank_table().rank_to_value) w.write<uint16_t>(v);
		}
	}
	for (Color c : colors)
	{
	...
	for (Color c : colors)
	{
		const Layered_Compressed_EGTB& co = color_out[c];
		if (!co.has_payload()) continue;
		write_blocks_concurrent(thread_pool, In_Out_Param(w), co.compressed_blocks());
		w.zero_align(64);
	}
	if (w.num_bytes_written() != file_size)
		print_and_abort("file size is wrong.\n");
	w.write_end_checksum();
	w.close_file();
