...
bool Piece_Config_For_Gen::try_init()
{
	...
	const bool has_pawns =
		counts[WHITE_PAWN] > 0 || counts[BLACK_PAWN] > 0 || has_frozen_pair();
	if (has_pawns)
	{
	...
	// The board carries the pair's two pawns even though they are not free-pawn
	// classes, so reflect them in the cached physical piece counts.
	if (has_frozen_pair())
	{
...
uint8_t Piece_Config_For_Gen::orbit_weight(const Decomposed_Board_Index& d) const
{
	...
	if (m_pawn_slice_manager->has_pawns())
	{
		...
		// The frozen pair's two pawns can also collide with a king or a piece.
		if (m_pair_group)
		{
	...
	Out_Param<std::array<Piece_Group::Placement, PIECE_CLASS_NB>> placements) const
{
	...
	if (psm.has_pawns())
	{
		...
		// The frozen pair's two pawns are ordinary pawns; place them alongside the
		// free pawns and fold them into the same all-pawns placement (the pair is
		// split back out by Pair_Group::find_canonical at board->index time).
		if (m_pair_group)
		{
			...
			white_occ |= pwbb;
			black_occ |= pbbb;
			// place_pawns assigns the placement only for a populated free class;
			// when there is none, the slot still holds the previous call's pawns,
			// so reset it before folding in the pair member (placements is reused).
