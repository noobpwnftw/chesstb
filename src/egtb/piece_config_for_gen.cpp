...
bool Piece_Config_For_Gen::try_init()
{
	...
	m_has_castling = Piece_Config::has_castling();
	...
	ASSERT(m_has_castling == (m_symmetry == Symmetry_Group::NONE));
	m_king_slice_manager =
		m_has_castling ? &castling_slice_mgr(castling_rights(WHITE), castling_rights(BLACK))
		: m_symmetry == Symmetry_Group::DIHEDRAL_8 ? &dihedral_8
		: &file_mirror;
...
Const_Span<Square> Piece_Config_For_Gen::castling_rook_squares(int32_t king_slice_id, Color c) const
{
	ASSERT(m_has_castling);
...
uint8_t Piece_Config_For_Gen::orbit_weight(const Decomposed_Board_Index& d) const
{
	...
	ASSERT(d.king_slice_id >= 0
	       && static_cast<size_t>(d.king_slice_id) < ksm.kings_of_slice.size());
	const auto [wk, bk] = ksm.kings_of_slice[d.king_slice_id];
	...
	if (m_has_castling)
		for (const Color c : { WHITE, BLACK })
			for (const Square r : castling_rook_squares(d.king_slice_id, c))
				occupied |= square_bb(r);
	...
	if (m_pawn_slice_manager->has_pawns())
	{
		...
		if (m_pair_group)
		{
			for (const Square sq : { m_pair_group->white_square(pd.pair_idx),
			                         m_pair_group->black_square(pd.pair_idx) })
			{
	...
	if (m_symmetry == Symmetry_Group::NONE) return 1;
	if (m_symmetry == Symmetry_Group::FILE_MIRROR) return 2;

	ASSERT(m_symmetry == Symmetry_Group::DIHEDRAL_8);
	ASSERT(static_cast<size_t>(d.king_slice_id) < ksm.slice_has_stabilizer.size());
	if (!ksm.slice_has_stabilizer[d.king_slice_id]) return 8;
	...
	Out_Param<std::array<Piece_Group::Placement, PIECE_CLASS_NB>> placements) const
{
	...
	if (psm.has_pawns())
	{
		...
		if (m_pair_group)
		{
			...
			white_occ |= pwbb;
			black_occ |= pbbb;
			if (!is_populated(WHITE_PAWNS)) (*placements)[WHITE_PAWNS].clear();
			if (!is_populated(BLACK_PAWNS)) (*placements)[BLACK_PAWNS].clear();
			(*placements)[WHITE_PAWNS].add(pw);
			(*placements)[BLACK_PAWNS].add(pb);
		}
	}
	...
	if (m_has_castling)
	{
		for (const Color cc : { WHITE, BLACK })
		{
			...
			for (const Square rook_sq : castling_rook_squares(index.king_slice_id, cc))
			{
				...
				board->set_castling_right(cc, sq_file(rook_sq) > sq_file(castling_king), rook_sq);
			}
		}
	}
...
template <bool ASSUME_LEGAL>
bool Piece_Config_For_Gen::patch_board(
	const Decomposed_Board_Index& index, In_Out_Param<Position> board,
	In_Out_Param<std::array<Piece_Group::Placement, PIECE_CLASS_NB>> placements) const
{
	const Piece_Group::Placement* new_placements[PIECE_CLASS_NB];
	Piece_Class changed[PIECE_CLASS_NB];
	...
	for (size_t i = 0; i < m_num_populated_classes; ++i)
	{
		const Piece_Class c = m_populated_classes[i];
		const Piece_Group& g = group(c);
		const Piece_Group::Placement& np = g.squares(index.within[c]);
		const Piece_Group::Placement& op = (*placements)[c];

		if (np.size() == op.size())
		{
			size_t k = 0;
			while (k < op.size() && np[k] == op[k]) ++k;
			if (k == op.size()) continue;
		}

		new_placements[num_changed] = &np;
		changed[num_changed] = c;
		++num_changed;

		Bitboard bb = Bitboard::make_empty();
		for (size_t k = 0; k < op.size(); ++k)
		{
			board->m_squares[op[k]] = PIECE_NONE;
			bb |= square_bb(op[k]);
		}
		const Piece pc = g.piece();
		board->m_pieces[pc] ^= bb;
		board->m_pieces[piece_occupy(piece_color(pc))] ^= bb;
		board->m_occupied ^= bb;
	}

	for (size_t i = 0; i < num_changed; ++i)
	{
		const Piece_Class c = changed[i];
		const Piece_Group::Placement& np = *new_placements[i];
		const Piece pc = group(c).piece();
		Bitboard bb = Bitboard::make_empty();
		for (size_t k = 0; k < np.size(); ++k)
		{
			const Square sq = np[k];
			if constexpr (!ASSUME_LEGAL)
				if (!board->is_empty(sq)) return false;
			board->m_squares[sq] = pc;
			bb |= square_bb(sq);
...
template bool Piece_Config_For_Gen::patch_board<false>(
	const Decomposed_Board_Index&, In_Out_Param<Position>,
	In_Out_Param<std::array<Piece_Group::Placement, PIECE_CLASS_NB>>) const;
template bool Piece_Config_For_Gen::patch_board<true>(
	const Decomposed_Board_Index&, In_Out_Param<Position>,
	In_Out_Param<std::array<Piece_Group::Placement, PIECE_CLASS_NB>>) const;
