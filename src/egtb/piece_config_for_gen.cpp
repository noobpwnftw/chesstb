...
bool Piece_Config_For_Gen::try_init()
{
	...
	if (has_opposing_pair())
	{
		...
		// The board carries the pair's two pawns even though they are not
		// free-pawn classes, so reflect them in the cached physical counts.
...
uint8_t Piece_Config_For_Gen::orbit_weight(const Decomposed_Board_Index& d) const
{
	...
	if (m_pawn_slice_manager->has_pawns())
	{
		...
		// The opposing pair's two pawns can also collide with a king or a piece.
		if (m_pair_group)
		{
	...
	Out_Param<std::array<Piece_Group::Placement, PIECE_CLASS_NB>> placements) const
{
	...
	if (psm.has_pawns())
	{
		...
		// The pair's two pawns are ordinary pawns; place them alongside the
