	...
	m_is_symmetric(false)
{
	...
	// Castling materials route every capture through the castling tables.
	for (size_t i = 0; i < ps.num_pieces() && !m_epsi.has_castling(); ++i)
	{
		const Piece pawn = ps.pieces()[i];
		if (piece_type(pawn) != PAWN) continue;
		const Color mover = piece_color(pawn);
...
void EGTB_Generator::refresh_active_metadata(const Sliced_EGTB_File_For_Gen<EntryT, OtherEntryTs...>& tbl)
{
	...
	const auto mark_slice_run = [&](std::vector<uint8_t>& groups, size_t base) {
		const size_t g_lo = tbl.group_id_of(base);
		const size_t g_hi = tbl.group_id_of(base + nks - 1);
		std::fill(groups.begin() + g_lo, groups.begin() + g_hi + 1, uint8_t{1});
	};

	m_fusion_groups.assign(ngroups, 0);
	for (int32_t pid : m_active_pawn_slices)
		mark_slice_run(m_fusion_groups, static_cast<size_t>(pid) * nks);
	...
	for (const auto& targets : m_targets_by_active)
		for (int32_t tpid : targets)
			mark_slice_run(m_fusion_init_groups, static_cast<size_t>(tpid) * nks);
                                      ...
                                      size_t own_layers, size_t target_layers) const
{
	...
	auto add_pid_range = [&](Group_Set& dst, int32_t pid) {
		...
		const size_t first_g = tbl.group_id_of(base);
		const size_t last_g  = tbl.group_id_of(base + nks - 1);
...
void EGTB_Generator::apply_working_set(
	...
	const void* protected_dirty_tbl)
{
		...
		for (size_t layer = 0; layer < num_layers; ++layer)
		{
			...
			for (size_t group = 0; group < ng; ++group)
			{
				if (need_layer[group])
				{
					...
					if (!tbl.is_group_resident(group))
						loads.push_back({ &tbl, group });
					ram_bytes += tbl.group_bytes(group);  // resident once loaded
				}
				else
				{
					ram_bytes += tbl.group_ram_bytes(group);
				}
			}
		}
	}
	...
	if (m_paging_budget_bytes > 0)
	{
		for (Color c : { WHITE, BLACK })
		{
			...
			for (size_t layer = 0; layer < num_layers; ++layer)
			{
				...
				const uint8_t* need_layer = need + layer * ng;
				for (size_t group = 0; group < ng; ++group)
					if (!need_layer[group] && tbl.group_ram_bytes(group) != 0)
					{
						const bool dirty = tbl.is_group_dirty(group);
						const bool exempt =
							(std::is_same_v<EntryT, DTM50_Final_Entry> && layer == 0)
							|| (dirty && &tbl == protected_dirty_tbl);
						victims.push_back({ &tbl, group, dirty, exempt });
					}
			}
		}
		std::sort(victims.begin(), victims.end(),
			[](const Candidate& lhs, const Candidate& rhs) {
				...
				if (lhs.dirty != rhs.dirty)
					return !lhs.dirty;
				if (lhs.dirty && !lhs.exempt)
					return lhs.tbl->last_used(lhs.group) > rhs.tbl->last_used(rhs.group);
				return lhs.tbl->last_used(lhs.group) < rhs.tbl->last_used(rhs.group);
			});
	}

	const size_t target = paging_target_bytes(m_paging_budget_bytes, w_tbl->group_bytes(0));

	std::vector<Group_Ref> packs;
	{
		size_t projected = ram_bytes;
		for (const Candidate& v : victims)
		{
			if (projected <= target) break;
			if (!v.tbl->is_group_resident(v.group)) continue;
			packs.push_back({ v.tbl, v.group });
			projected -= v.tbl->group_bytes(v.group) / 2;
		}
	}
	...
	if (!packs.empty())
	{
		...
		thread_pool->run_sync_task_on_multiple_threads(
			...
			for (;;)
			{
				...
				if (idx >= packs.size()) return;
				packs[idx].tbl->evict_group(packs[idx].group_id);
			}
		});

		for (const Group_Ref& r : packs)
			ram_bytes = ram_bytes - r.tbl->group_bytes(r.group_id)
			          + r.tbl->group_ram_bytes(r.group_id);
	}

	std::vector<Group_Ref> evicts;
	for (const Candidate& v : victims)
	{
		if (ram_bytes <= target) break;
		evicts.push_back({ v.tbl, v.group });
		ram_bytes -= v.tbl->group_ram_bytes(v.group);
	}
	...
	if (!evicts.empty())
	{
		...
		thread_pool->run_sync_task_on_multiple_threads(
			...
			{
				const size_t idx = next.fetch_add(1);
				if (idx >= evicts.size()) return;
				evicts[idx].tbl->flush_group(evicts[idx].group_id);
			}
		});
	}
	...
	size_t group_charge)
{
	...
	apply_working_set(thread_pool, w_tbl, b_tbl, needed_w, needed_b, nullptr);
	return true;
}
	...
	size_t num_workers)
{
	...
	const size_t total_blocks = ceil_div(output_total_bytes, block_size);
	auto worker_pins = std::make_shared<Worker_Group_Pins<EntryT, OtherEntryTs...>>(
		cache, color, num_workers);
	return Block_Source{
		output_total_bytes,
		[within, spg, entry_bytes, total_blocks, num_workers](size_t first_block, size_t block_size) {
			if (total_blocks - first_block <= num_workers)
				return first_block + 1;

			const size_t first_entry = first_block * block_size / entry_bytes;
			const size_t group = (first_entry / within) / spg;
			const size_t group_end_entries = (group + 1) * spg * within;
			return ceil_div(group_end_entries * entry_bytes, block_size);
		},
		[&src, worker_pins, within, spg, source_block_bytes, source_total_bytes, perm_plan](size_t worker_id, size_t block_id, Span<uint8_t> scratch) -> Const_Span<uint8_t> {
			...
			worker_pins->acquire(worker_id, first_g, last_g);
		...
		[worker_pins](size_t worker_id) { worker_pins->release(worker_id); }
	};
}
...
template void EGTB_Generator::apply_working_set<DTZ_Final_Entry, DTZ_Intermediate_Entry>(In_Out_Param<Thread_Pool>, Sliced_EGTB_File_For_Gen<DTZ_Final_Entry, DTZ_Intermediate_Entry>*, Sliced_EGTB_File_For_Gen<DTZ_Final_Entry, DTZ_Intermediate_Entry>*, const std::vector<uint8_t>&, const std::vector<uint8_t>&, const void*);
template void EGTB_Generator::apply_working_set<DTC_Final_Entry, DTC_Intermediate_Entry>(In_Out_Param<Thread_Pool>, Sliced_EGTB_File_For_Gen<DTC_Final_Entry, DTC_Intermediate_Entry>*, Sliced_EGTB_File_For_Gen<DTC_Final_Entry, DTC_Intermediate_Entry>*, const std::vector<uint8_t>&, const std::vector<uint8_t>&, const void*);
template void EGTB_Generator::apply_working_set<DTM_Final_Entry, DTM_Intermediate_Entry>(In_Out_Param<Thread_Pool>, Sliced_EGTB_File_For_Gen<DTM_Final_Entry, DTM_Intermediate_Entry>*, Sliced_EGTB_File_For_Gen<DTM_Final_Entry, DTM_Intermediate_Entry>*, const std::vector<uint8_t>&, const std::vector<uint8_t>&, const void*);
template void EGTB_Generator::apply_working_set<DTM50_Final_Entry, DTM50_Intermediate_Entry>(In_Out_Param<Thread_Pool>, Sliced_EGTB_File_For_Gen<DTM50_Final_Entry, DTM50_Intermediate_Entry>*, Sliced_EGTB_File_For_Gen<DTM50_Final_Entry, DTM50_Intermediate_Entry>*, const std::vector<uint8_t>&, const std::vector<uint8_t>&, const void*);
...
template Block_Source make_entry_block_source<DTZ_Final_Entry, DTZ_Intermediate_Entry>(Sliced_EGTB_File_For_Gen<DTZ_Final_Entry, DTZ_Intermediate_Entry>&, Save_Group_Cache<DTZ_Final_Entry, DTZ_Intermediate_Entry>&, Color, Index_Permutation_Plan, size_t, size_t, size_t);
template Block_Source make_entry_block_source<DTC_Final_Entry, DTC_Intermediate_Entry>(Sliced_EGTB_File_For_Gen<DTC_Final_Entry, DTC_Intermediate_Entry>&, Save_Group_Cache<DTC_Final_Entry, DTC_Intermediate_Entry>&, Color, Index_Permutation_Plan, size_t, size_t, size_t);
template Block_Source make_entry_block_source<DTM_Final_Entry, DTM_Intermediate_Entry>(Sliced_EGTB_File_For_Gen<DTM_Final_Entry, DTM_Intermediate_Entry>&, Save_Group_Cache<DTM_Final_Entry, DTM_Intermediate_Entry>&, Color, Index_Permutation_Plan, size_t, size_t, size_t);
template Block_Source make_entry_block_source<DTM50_Final_Entry, DTM50_Intermediate_Entry>(Sliced_EGTB_File_For_Gen<DTM50_Final_Entry, DTM50_Intermediate_Entry>&, Save_Group_Cache<DTM50_Final_Entry, DTM50_Intermediate_Entry>&, Color, Index_Permutation_Plan, size_t, size_t, size_t);
                    ...
                    const std::filesystem::path& path, uint64_t expected_magic)
{
	...
	Huge_Array<uint8_t> buf(For_Overwrite_Tag{}, spill_scratch_bytes(bytes));
	size_t off = 0;
	while (off < bytes)
	{
		Spill_Header hdr;
		...
		if (hdr.magic != expected_magic)
			print_and_abort("Spill magic mismatch: %s\n", name.c_str());
		if (hdr.uncompressed_size > bytes - off)
			print_and_abort("Spill size mismatch: %s\n", name.c_str());
		if (hdr.compressed_size > buf.size())
			buf = Huge_Array<uint8_t>(For_Overwrite_Tag{}, static_cast<size_t>(hdr.compressed_size));
		fp.read(reinterpret_cast<char*>(buf.data()),
			narrowing_static_cast<std::streamsize>(hdr.compressed_size));
		if (!fp) print_and_abort("Spill file truncated payload: %s\n", name.c_str());

		decompress_bytes_into(buf.data(), static_cast<size_t>(hdr.compressed_size),
		                      data + off, hdr.uncompressed_size);
		off += hdr.uncompressed_size;
	}
}

Packed_Group pack_group_raw(const uint8_t* data, size_t bytes)
{
	const size_t num_chunks = (bytes + SPILL_CHUNK_BYTES - 1) / SPILL_CHUNK_BYTES;
	...
	for (size_t i = 0; i < num_chunks; ++i)
	{
		...
		if (n <= 0) print_and_abort("LZ4 compress failed for a packed group\n");

		Packed_Group::Chunk& chunk = packed.chunks[i];
...
void unpack_group_raw(uint8_t* data, size_t bytes, Packed_Group& packed)
{
	size_t off = 0;
	for (Packed_Group::Chunk& chunk : packed.chunks)
	{
		if (chunk.uncompressed_size > bytes - off)
			print_and_abort("Packed group chunk size mismatch\n");

		decompress_bytes_into(chunk.data.data(), chunk.data.size(),
		                      data + off, chunk.uncompressed_size);
		off += chunk.uncompressed_size;
		chunk.data.clear();
	}
	if (off != bytes) print_and_abort("Packed group is short of the group size\n");
}

void save_packed_group(const Packed_Group& packed, const std::filesystem::path& path,
                       uint64_t magic)
{
	std::ofstream fp(path, std::ios::binary | std::ios::trunc);
	if (!fp) print_and_abort("Could not open spill file for write: %s\n", path.string().c_str());

	for (const Packed_Group::Chunk& chunk : packed.chunks)
	{
		const Spill_Header hdr{
			magic,
