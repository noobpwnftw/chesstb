...
placements_from_position(const Piece_Config_For_Gen& epsi, const Position& pos)
{
	...
	// Pawn placements determine orientation even though slices store them.
	...
	std::array<Piece_Group::Placement, PIECE_CLASS_NB>& placements)
{
	...
	if (epsi.pawn_slice_manager().has_pawns())
	{
		...
		if (epsi.pair_group())
		{
			// Exclude the canonical opposing pair from the free pawns.
...
// Capture-promotion sub-config, including pair-to-free-pawn transitions.
	...
	m_is_symmetric(false)
{
	...
	// Map the literal child to its canonical material orientation.
	auto resolve_sub = [&](Color mover, Const_Span<Piece> literal_post_pieces,
		...
		// The excluded pair still contributes to the material key.
	...
	for (size_t i = 0; i < ps.num_pieces(); ++i)
	{
		...
		// Capturing a free piece dissolves an opposing pair into free pawns.
...
std::map<Material_Key, Piece_Config> EGTB_Generator::enumerate_sub_materials(const Piece_Config& ps)
{
	...
	// Pair-member captures need separate pair-to-single-pawn children.
...
Working_Set_Estimate compute_working_set(const Piece_Config& ps)
{
	...
	// Match Sliced_EGTB_File_For_Gen grouping.
	...
	// Reuse reach storage across sampled pairs.
	...
	for (const auto& batch : psm.pair_topo_batches())
	{
		...
		// Sample with the same helpers used by paging.
		...
		// Restrict clearing and counting to groups this pair can reach.
                              ...
                              Slice_Reach_Scratch& scratch)
{
	...
	for (int32_t pid : active_pids)
	{
		...
		for (size_t s = s_lo; s < s_hi; ++s)
		{
			...
			// Expand owned kids once; neighbors do not recursively expand.
...
void EGTB_Generator::refresh_active_metadata(const Sliced_EGTB_File_For_Gen<EntryT, OtherEntryTs...>& tbl)
{
	...
	// The fusion union covers each push target's full pid range.
...
template std::vector<std::vector<int32_t>> EGTB_Generator::compute_fusion_groups<DTZ_Final_Entry, DTZ_Intermediate_Entry>(const Sliced_EGTB_File_For_Gen<DTZ_Final_Entry, DTZ_Intermediate_Entry>&, const std::vector<int32_t>&, size_t) const;
...
// Bound each worker's transient spill buffer.
