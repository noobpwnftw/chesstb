...
struct Layered_Compressed_EGTB
{
	static Layered_Compressed_EGTB make_singular()
	{
		Layered_Compressed_EGTB info{};
		info.m_is_singular = true;
		return info;
	}

	static Layered_Compressed_EGTB make_dropped()
	{
		Layered_Compressed_EGTB info{};
	...
	NODISCARD uint32_t block_positions() const
	{
		return m_block_positions;
	}

	NODISCARD uint32_t tail_positions() const
	{
		return m_tail_positions;
	}
	...
	NODISCARD uint8_t entry_bytes() const
	{
		return m_ranks.entry_bytes;
	}
	...
	NODISCARD uint64_t usize(size_t block_id) const
	{
		return m_usizes[block_id];
	}

	NODISCARD const Layered_Rank_Table& rank_table() const
	{
		return m_ranks;
	}
...
private:
	...
	uint32_t m_block_positions;
	uint32_t m_tail_positions;

	Compressed_Block_Store m_compressed_blocks;
	std::vector<uint64_t> m_usizes;
	size_t m_total_compressed_size;

	Layered_Rank_Table m_ranks;
};
