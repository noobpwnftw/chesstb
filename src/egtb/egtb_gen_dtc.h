...
struct DTC_Table
{
	...
	NODISCARD INLINE DTC_Any_Entry read_any(Color stm, Board_Index pos) const
	{
		auto& tbl = m_dtc[stm];
		const DTC_Final_Entry& fe = tbl.template view_at<DTC_Final_Entry>(pos);
		if (fe.is_illegal() || !fe.is_draw())
			return fe;
		return reinterpret_cast<const DTC_Intermediate_Entry&>(fe);
	}
	template <typename EntryT>
...
private:
	...
	NODISCARD INLINE DTC_Any_Entry read_dtc_any(Color stm, Board_Index pos) const
	{
