...
DTZ_Generator::Init_Result DTZ_Generator::init_entries(In_Out_Param<Thread_Pool> thread_pool)
{
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		...
		page_in_window(thread_pool, &m_table->m_dtz[WHITE], &m_table->m_dtz[BLACK],
	...
	Position_For_Gen& pos_gen, uint16_t target_dtz)
{
	...
	pos.visit_pseudo_legal_pre_quiets([&](Move m) FORCE_INLINE_LAMBDA {
		...
		else
		{
			...
			else if (reinterpret_cast<const DTZ_Intermediate_Entry&>(e).has_cap_cwin())
				return;
		}
                                  ...
                                  Color stm, uint16_t ply)
{
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		...
		page_in_window(thread_pool, &m_table->m_dtz[WHITE], &m_table->m_dtz[BLACK],
		               pos, 1,
		               m_iter_groups[stm], add_need, &m_table->m_dtz[color_opp(stm)]);
                            ...
                            Iter_Phase start_phase, uint16_t finished_ply)
{
	...
	if (start_phase == Iter_Phase::CURSED && finished_ply < DTZ_Final_Entry::MAX_NON_CURSED_DTZ)
	{
		...
		if (!pending_cursed && finished)
			return;
		update_max<uint16_t>(finished_ply, DTZ_Final_Entry::MAX_NON_CURSED_DTZ - 1);
	}
...
struct Gather_Sink
{
	...
	void merge_into(EGTB_Info& info, Value_Histogram& hist) const
	{
		for (const Shard& shard : shards)
		{
			info.consolidate_from(&shard.info, &shard.info + 1, color);
			for (size_t k = 0; k < Value_Histogram::HIST_BINS; ++k)
			{
				hist.hist_1b[k] += shard.hist.hist_1b[k];
				hist.hist_2b[k] += shard.hist.hist_2b[k];
			}
		}
	}
};
			...
			for (;;)
			{
				...
				for (size_t s = s_begin; s < s_end; ++s)
				{
					...
					for (size_t i = 0; i < end_in_slice; ++i)
					{
						...
						const WDL_Stored stored = wdl_for_storage(e);
						if (stored == WDL_Stored::BOUNDARY_WIN
						    || stored == WDL_Stored::BOUNDARY_LOSS)
						{
...
{
	ASSERT(sink == nullptr || sink->color == color);
	auto& src = table.m_dtz[color];
	const size_t within = src.within_slice_size();
	const size_t spg = src.slices_per_group();
	const size_t total_packed_bytes = ceil_div(num_positions, WDL_ENTRY_PACK_RATIO);
	const size_t total_blocks = ceil_div(total_packed_bytes, WDL_BLOCK_SIZE);
	const auto perm_plan = make_index_permutation_plan(epsi, index_perm);
	auto worker_pins = std::make_shared<Worker_Group_Pins<DTZ_Final_Entry, DTZ_Intermediate_Entry>>(
		cache, color, num_workers);
	return Block_Source{
		total_packed_bytes,
		[within, spg, total_blocks, num_workers](size_t first_block, size_t block_size) {
			if (total_blocks - first_block <= num_workers)
				return first_block + 1;

			const size_t first_storage = first_block * block_size * WDL_ENTRY_PACK_RATIO;
			const size_t group = (first_storage / within) / spg;
			const size_t group_end_storage = (group + 1) * spg * within;
			const size_t group_end_bytes = ceil_div(group_end_storage, WDL_ENTRY_PACK_RATIO);
			return ceil_div(group_end_bytes, block_size);
		},
		[&epsi, &src, worker_pins, sink, color, within, spg, num_positions, total_packed_bytes,
		 perm_plan, relax](
			size_t worker_id, size_t block_id, Span<uint8_t> scratch) -> Const_Span<uint8_t>
		{
			...
			Gather_Sink::Shard* shard = sink ? &sink->shards[worker_id] : nullptr;
			...
			if (relax) pos_gen.emplace(epsi, BOARD_INDEX_ZERO, color);
			...
			for (size_t storage = first_storage; storage < end_storage; ++storage)
			{
				...
				if (shard)
				{
					...
					shard->info.add_result(color, e.wdl(), ow);
					if (e.is_win())
						shard->info.maybe_update_longest_win(color, logical, e.value());
					if (!e.is_draw() && !(EGTB_GEN_LOSS_ONLY && e.is_win()))
					{
						++shard->hist.hist_1b[static_cast<size_t>(dtz_value_for_storage(e))];
						++shard->hist.hist_2b[static_cast<size_t>(static_cast<uint16_t>(e.value()))];
					}
				}
			}
...
void DTZ_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	DTZ_Save_Cache cache(&m_table->m_dtz[WHITE], &m_table->m_dtz[BLACK], m_paging_budget_bytes);
	...
	for (Color me : colors)
	{
		...
		else
		{
			...
			wdl_index_perm[me] = choose_storage_permutation_config(
				...
				[&](uint32_t perm) {
					return make_wdl_block_source(
						m_epsi, *m_table, me, cache, num_positions,
						thread_pool->num_workers(), nullptr,
						perm, relax ? &*relax : nullptr);
				},
				WDL_BLOCK_SIZE,
			...
			Gather_Sink sink{me, std::vector<Gather_Sink::Shard>(thread_pool->num_workers())};
			Block_Source dict_src = make_wdl_block_source(
				m_epsi, *m_table, me, cache, num_positions,
				thread_pool->num_workers(), nullptr,
				wdl_index_perm[me], relax ? &*relax : nullptr);
			Block_Source src = make_wdl_block_source(
				m_epsi, *m_table, me, cache, num_positions,
				thread_pool->num_workers(), &sink,
				wdl_index_perm[me],
				relax ? &*relax : nullptr);
			wdl_save[me] = save_compress_wdl(
				thread_pool, dict_src, src, me, WDL_BLOCK_SIZE, max_workers,
				paths.block_spill_path(m_epsi, me));
			sink.merge_into(m_info, dtz_hist[me]);
		}
	...
	for (Color me : colors)
	{
			...
			dtz_index_perm[me] = choose_storage_permutation_config(
				...
				[&](uint32_t perm) {
					return make_entry_block_source(
						...
						DTZ_BLOCK_SIZE, dtz_entry_bytes[me], thread_pool->num_workers());
				},
				DTZ_BLOCK_SIZE,
		...
		dtz_save[me] = save_compress_egtb(
			thread_pool, src, me, dtz_entry_bytes[me], DTZ_BLOCK_SIZE, max_workers,
			paths.block_spill_path(m_epsi, me), chosen,
			EGTB_GEN_LOSS_ONLY ? Rank_Storage_Mode::DTZ_LOSS_ONLY : Rank_Storage_Mode::DTZ);
		cache.purge(me);
