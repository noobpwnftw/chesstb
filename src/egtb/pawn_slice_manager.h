...
struct Pawn_Slice_Manager
{
    ...
    // A slice and its file mirror, or only the slice when it is self-mirrored.
    struct Pair_Members
    {
        int32_t ids[2] = { 0, 0 };
        size_t n = 0;

        NODISCARD const int32_t* begin() const { return ids; }
        NODISCARD const int32_t* end()   const { return ids + n; }
        NODISCARD size_t size()          const { return n; }
        NODISCARD int32_t operator[](size_t i) const { return ids[i]; }
    };

    NODISCARD Pair_Members pair_members(int32_t slice_id) const
    {
        const int32_t mir = mirror_slice_of(slice_id);
        Pair_Members m;
        m.ids[m.n++] = slice_id;
        if (mir != slice_id) m.ids[m.n++] = mir;
        return m;
    }

    void push_target_slices(int32_t slice_id, std::vector<int32_t>& out) const;
