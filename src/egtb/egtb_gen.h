...
NODISCARD inline size_t paging_target_bytes(size_t budget_bytes, size_t group_bytes)
{
	const size_t reserve = MAX_PAGING_THREADS * group_bytes;
	return budget_bytes - std::min(reserve, budget_bytes);
}

// A group compressed chunk by chunk, each chunk holding the very bytes its
// spill file record is made of, so spilling it later is a byte copy.
struct Packed_Group
{
	struct Chunk
	{
...
NODISCARD Packed_Group pack_group_raw(const uint8_t* data, size_t bytes);
...
struct Sliced_EGTB_File_For_Gen
{
	...
	NODISCARD bool is_group_packed(size_t g) const { return !m_packed[g].chunks.empty(); }
	// Set iff the group's contents are not in its spill file.
	NODISCARD bool is_group_dirty(size_t g) const { return m_dirty[g] != 0; }
	NODISCARD size_t group_ram_bytes(size_t g) const
	{
		size_t bytes = is_group_resident(g) ? group_bytes(g) : 0;
		for (const Packed_Group::Chunk& chunk : m_packed[g].chunks)
			bytes += large_page_bytes(chunk.data.size());
		return bytes;
	}
	...
	void load_group(size_t g)
	{
		...
		if (is_group_packed(g))
		{
			alloc_group(g, false);  // unpacking writes every byte
			unpack_group_raw(
				reinterpret_cast<uint8_t*>(m_groups[g].data()),
				group_bytes(g), m_packed[g]);
			m_packed[g] = Packed_Group{};  // m_dirty carries over to the array
			return;
		}
	...
	// One step out of RAM: a resident group is compressed where it lies, and an
	// already compressed one is either saved if dirty or dropped if clean.
	void evict_group(size_t g)
	{
		if (is_group_resident(g))
		{
			m_packed[g] = pack_group_raw(
				reinterpret_cast<const uint8_t*>(m_groups[g].data()),
				group_bytes(g));
			m_groups[g] = Huge_Array<Underlying_Entry_Type>{};  // free
			return;
		}
		if (!is_group_packed(g)) return;
		if (m_dirty[g])
		{
			save_packed_group(m_packed[g], m_disk_paths[g], m_disk_magic);
			m_dirty[g] = false;
		}
		m_packed[g] = Packed_Group{};
	}

	// Every step, until the group holds no RAM at all. A clean group is in its
	// spill file already, so it is dropped rather than compressed on the way.
	void flush_group(size_t g)
	{
		if (is_group_resident(g) && !m_dirty[g])
		{
			m_groups[g] = Huge_Array<Underlying_Entry_Type>{};  // free
			return;
		}
		while (is_group_resident(g) || is_group_packed(g))
			evict_group(g);
	}
...
private:
	...
	std::vector<Packed_Group> m_packed;
...
struct Save_Group_Cache
{
	...
	using Pos = std::list<Key>::iterator;

	// PACKED is a group this cache compressed in place: still a hit, but it has
	// to be unpacked before it can be handed out.
	enum struct State : uint8_t { LOADING, RESIDENT, PACKED, EVICTING };
	struct Entry { int pins; State state; bool cold; Pos pos; };

	std::vector<Table*> tables;
	size_t budget_bytes;           // 0 = unbounded
	size_t ram_bytes = 0;
	size_t loads_in_flight = 0;
	size_t evictions_in_flight = 0;
	std::mutex mu;
	std::condition_variable cv;    // an entry left LOADING/EVICTING
	std::list<Key> hot;            // load order, front = oldest; LOADING + RESIDENT
	std::list<Key> cold;           // compressed, front = oldest
	std::map<Key, Entry> entries;

	Save_Group_Cache(Table* w_tbl, Table* b_tbl, size_t budget)
		: tables{ w_tbl, b_tbl }, budget_bytes(budget) { reserve_paging(); init_resident(); }

	Save_Group_Cache(std::vector<Table*> tbls, size_t budget)
		: tables(std::move(tbls)), budget_bytes(budget) { reserve_paging(); init_resident(); }
	...
	void acquire(size_t table_idx, size_t g)
	{
		...
		for (;;)
		{
			...
			{
				hot.push_back(k);
				entries.emplace(k, Entry{ 1, State::LOADING, false, std::prev(hot.end()) });
				ram_bytes += tables[table_idx]->group_bytes(g);
				sweep(lk);
				load_locked(lk, k);
				return;
			}
			...
			if (it->second.state == State::PACKED)
			{
				...
				ram_bytes -= tables[table_idx]->group_ram_bytes(g);
				ram_bytes += tables[table_idx]->group_bytes(g);
				sweep(lk);
				load_locked(lk, k);
				return;
	...
	void purge(size_t table_idx)
	{
		...
		{
			if (it->first.first != table_idx) { ++it; continue; }
			ram_bytes -= tables[table_idx]->group_ram_bytes(it->first.second);
			(it->second.cold ? cold : hot).erase(it->second.pos);
			it = entries.erase(it);
		}
...
private:
	void reserve_paging()
	{
		...
		budget_bytes = paging_target_bytes(budget_bytes, tables.front()->group_bytes(0));
		if (budget_bytes == 0) budget_bytes = 1;
	}

	// Hands out a group whose entry is already LOADING, releasing the lock
	// across the load itself.
	void load_locked(std::unique_lock<std::mutex>& lk, const Key& k)
	{
		Table& tbl = *tables[k.first];
		if (!tbl.is_group_resident(k.second))
		{
			while (evictions_in_flight != 0
			       || loads_in_flight + evictions_in_flight >= MAX_PAGING_THREADS)
				cv.wait(lk);
			++loads_in_flight;
			lk.unlock();
			tbl.load_group(k.second);
			lk.lock();
			--loads_in_flight;
		}
		Entry& e = entries.find(k)->second;
		if (e.cold)
		{
			hot.splice(hot.end(), cold, e.pos);
			e.cold = false;
		}
	...
	// Sheds one level per pass. Compressing costs no IO, so the oldest unpinned
	// resident group goes first, and a group already compressed only goes to
	// disk once there is no resident one left to shrink.
	void sweep(std::unique_lock<std::mutex>& lk)
	{
		while (budget_bytes != 0 && ram_bytes > budget_bytes)
		{
			if (evictions_in_flight >= MAX_PAGING_THREADS)
			{
				cv.wait(lk);
				continue;
			}

			// An entry another thread is already evicting stays in its list
			// until that finishes, so match on the state, not the position.
			auto victim = entries.end();
			for (const Key& k : hot)
			{
				auto p = entries.find(k);
			...
			if (victim == entries.end())
				for (const Key& k : cold)
				{
					auto p = entries.find(k);
					if (p != entries.end() && p->second.state == State::PACKED)
					{
						victim = p;
						break;
					}
				}
			if (victim == entries.end()) return;  // all pinned or in flight

			const Key k = victim->first;
			Table& tbl = *tables[k.first];
			const size_t before = tbl.group_ram_bytes(k.second);

			victim->second.state = State::EVICTING;
			++evictions_in_flight;
			lk.unlock();
			tbl.evict_group(k.second);
			lk.lock();
			--evictions_in_flight;

			const size_t after = tbl.group_ram_bytes(k.second);
			ram_bytes = ram_bytes - before + after;
			auto p = entries.find(k);
			if (p != entries.end())
			{
				if (after == 0)  // it left RAM entirely
				{
					(p->second.cold ? cold : hot).erase(p->second.pos);
					entries.erase(p);
				}
				else           // compressed, and kept
				{
					cold.splice(cold.end(), hot, p->second.pos);
					p->second.cold = true;
					p->second.state = State::PACKED;
				}
			}
			cv.notify_all();
		}
	...
	void init_resident()
	{
		for (size_t ti = 0; ti < tables.size(); ++ti)
		{
			...
			{
				// Whatever the generator's pager left behind, in either form.
				const size_t bytes = tbl.group_ram_bytes(g);
				if (bytes == 0) continue;
				const bool packed = !tbl.is_group_resident(g);
				std::list<Key>& list = packed ? cold : hot;
				list.push_back({ ti, g });
				entries[{ ti, g }] = Entry{
					0, packed ? State::PACKED : State::RESIDENT, packed, std::prev(list.end())
				};
				ram_bytes += bytes;
			}
		}
...
template <typename EntryT, typename... OtherEntryTs>
struct Worker_Group_Pins
{
	using Cache = Save_Group_Cache<EntryT, OtherEntryTs...>;
	using Pin = Pinned_Group_Range<EntryT, OtherEntryTs...>;

	struct Slot
	{
		size_t first_g = SIZE_MAX;
		size_t last_g = SIZE_MAX;
	...
	Worker_Group_Pins(Cache& c, size_t ti, size_t workers)
		: cache(&c), table_idx(ti), slots(workers) {}

	void acquire(size_t worker_id, size_t first_g, size_t last_g)
	{
		ASSERT(worker_id < slots.size());
		Slot& slot = slots[worker_id];
		if (slot.pin && slot.first_g <= first_g && last_g <= slot.last_g) return;
		slot.pin.reset();
		slot.pin = std::make_unique<Pin>(*cache, table_idx, first_g, last_g);
		slot.first_g = first_g;
		slot.last_g = last_g;
	}
	...
	void release(size_t worker_id)
	{
		...
		Slot& slot = slots[worker_id];
		slot.pin.reset();
		slot.first_g = SIZE_MAX;
		slot.last_g = SIZE_MAX;
	}
};
...
inline constexpr size_t CHUNK_SIZE = CACHE_LINE_SIZE * CHAR_BIT * 64;
...
protected:
	...
	std::vector<uint8_t>  m_window_need[COLOR_NB];
	size_t m_window_end;

	void reset_paging_window() { m_window_end = 0; }

	template <typename EntryT, typename... OtherEntryTs, typename Need_Fn>
	void page_in_window(
		In_Out_Param<Thread_Pool> thread_pool,
		Sliced_EGTB_File_For_Gen<EntryT, OtherEntryTs...>* w_tbl,
		Sliced_EGTB_File_For_Gen<EntryT, OtherEntryTs...>* b_tbl,
		size_t pos, size_t num_layers,
		const std::vector<uint8_t>& live_groups, Need_Fn add_need,
		const void* protected_dirty_tbl)
	{
		if (m_phase_pinned || pos < m_window_end) return;

		using Table = Sliced_EGTB_File_For_Gen<EntryT, OtherEntryTs...>;
		Table* tbls[COLOR_NB] = { w_tbl, b_tbl };
		const size_t ng = w_tbl->num_groups();
		const size_t need_len = num_layers * ng;

		for (Color c : { WHITE, BLACK })
			m_window_need[c].assign(need_len, 0);

		const size_t fill_budget = paging_target_bytes(m_paging_budget_bytes, w_tbl->group_bytes(0));
		size_t charge = 0;
		size_t window_loads = 0;
		size_t covered = pos;
		for (size_t k = pos; k < m_pair_group_ids.size(); ++k)
		{
			const size_t g = m_pair_group_ids[k];
			if (!live_groups[g]) continue;

			for (Color c : { WHITE, BLACK })
				m_scratch_need[c].assign(need_len, 0);
			add_need(g, m_scratch_need[WHITE].data(), m_scratch_need[BLACK].data());

			size_t added = 0, added_loads = 0;
			for (Color c : { WHITE, BLACK })
				for (size_t i = 0; i < need_len; ++i)
				{
					if (!m_scratch_need[c][i] || m_window_need[c][i]) continue;
					Table& tbl = tbls[c][i / ng];
					const size_t group = i % ng;
					added += tbl.group_bytes(group);
					if (!tbl.is_group_resident(group)) ++added_loads;
				}

			if (covered != pos
			    && (window_loads + added_loads > MAX_PAGING_THREADS
			        || (m_paging_budget_bytes != 0 && charge + added > fill_budget)))
				break;

			for (Color c : { WHITE, BLACK })
				for (size_t i = 0; i < need_len; ++i)
					if (m_scratch_need[c][i])
						m_window_need[c][i] = 1;
			charge += added;
			window_loads += added_loads;
			covered = k + 1;
		}

		m_window_end = covered;

		apply_working_set(thread_pool, w_tbl, b_tbl,
		                  m_window_need[WHITE], m_window_need[BLACK],
		                  protected_dirty_tbl);
	}
