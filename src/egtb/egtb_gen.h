...
struct Sliced_EGTB_File_For_Gen
{
	...
	// Board_Index -> (group, offset within the group's buffer), in one division.
	// Going via the slice divides twice for no reason: the slice term cancels,
	// since (p / within) / spg == p / (within * spg) and the in-group byte offset
	// is just p - g * entries_per_group. Every entry read/write takes this path,
	// so the saved mulh-shift is on the innermost loop of every retro.
	NODISCARD INLINE std::pair<size_t, size_t> locate(size_t p) const
	{
		if (m_entries_per_group > 1)
		{
	...
	// Same answer as group_id_of(slice_id_of(p)), one division instead of two.
	...
	// One spill per worker, each holding a chunk-sized LZ4 buffer (see
	// save_group_raw); distinct groups touch disjoint per-group state.
...
// The part of one slice that a chunk covers, as a flat row.
template <typename EntryT>
struct In_Pair_Row
{
	Board_Index lo{};              // first index of the run
	Board_Index hi{};              // one past the last
	const EntryT* data = nullptr;  // data[i - lo] is the entry at index i
	NODISCARD explicit operator bool() const { return data != nullptr; }
};

// Splits a chunk of a group scan into in-pair rows, leaving the per-cell loop no
// slice bookkeeping at all -- not even a pid test. Per cell, pawn_slice_of +
// view_at cost three Divider mulh-shifts plus the pid_in_pair load to fetch two
// sequential bytes, though neither the pid nor the slice base changes except at
// a slice boundary; here each is paid once per slice, and an out-of-pair slice
// is skipped whole. Callers loop `while (const auto row = rows.next())`.
...
struct In_Pair_Row_Iterator
{
	...
	// The next in-pair run, or a falsy row once the chunk is exhausted.
	NODISCARD In_Pair_Row<EntryT> next()
	{
		while (m_pos < m_chunk_end)
		{
			// A chunk boundary can fall mid-slice at either end, hence row_base
			// <= lo and the clamp on hi.
...
// Deduces TableT; only the entry type has to be named at the call site.
template <typename EntryT, typename TableT>
...
// mark_king_neighbor_reach's `kids` list plus two [num_king_slices] bitmaps:
// `kid_seen` is membership in `kids`, `kid_expanded` marks the kids whose
// neighbors have already been pulled in.
...
// Working-set closures over the slices [g_start, g_end) one group covers,
// marking `need` (a [num_groups] bitmap, or a color/hmc-offset window into one)
// by group id. Shared by every page_in_for_group and by compute_working_set, so
// the estimate and the paging it sizes cannot drift apart.

// Retro/quiet reach: for every active pid pp, the groups holding (pp, k) over
// each in-range slice's own kid plus its king neighbors. Deduping the kid set
// first keeps a kid two pids both reach from costing two passes over the pids.
...
// Pawn-push reach: the groups holding (push_target(pid), kid). Targets are
// pid-specific, so this one cannot dedupe across pids. Pawnless is a no-op.
...
protected:
	...
	// Pin candidates, largest first: full residency, then the fusion closures --
	// the union over the fusion of what the per-dispatch page_in helpers mark.
	// Iterate stays inside m_fusion_groups (the king-neighbour reach is crossed
	// with every active pid and kids stay in [0, nks)); init adds push targets.
	...
	// A pinned phase has an invariant need-set, so page_in_for_group can skip the
	// reach walk and the residency rescan.
	bool m_phase_pinned = false;

	Slice_Reach_Scratch m_reach_scratch;

	// The reach helpers, bound to this generator's active pids.
	INLINE void mark_king_neighbor_reach(uint8_t* need, size_t g_start, size_t g_end, size_t spg)
	{
		::mark_king_neighbor_reach(need, m_epsi, m_active_pawn_slices,
		                           g_start, g_end, spg, m_reach_scratch);
	}
	INLINE void mark_push_target_reach(uint8_t* need, size_t g_start, size_t g_end, size_t spg)
	{
	...
	// resident_layers: concurrent layer-copies per group, per color (2 for DTM50,
	// else 1). Both colors are charged; callers pass the per-color layer count.
	template <typename EntryT, typename... OtherEntryTs>
	...
	// Pin a phase-wide need-set. `group_charge` is the total needed group slots
	// over both colors and every layer; over budget it touches nothing and returns
	// false, leaving the caller to offer a smaller candidate. An unbounded budget
