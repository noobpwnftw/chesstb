...
void DTM50_Generator::init_entries(In_Out_Param<Thread_Pool> thread_pool)
{
	...
	// [hmc] is the fusion's groups, [0] adds the push-target closure, both colors.
	...
	// Working set per group: [hmc] holds group g as a write target only (quiet moves
	// target virtual hmc=100). [0] holds g (illegal + intermediate writes) plus the
	// pawn-push read closure -- a push reads opp[0] at (target_pid, same kid).
	// needs[B] == needs[W].
...
void DTM50_Generator::build_layer(In_Out_Param<Thread_Pool> thread_pool, uint16_t hmc)
{
	...
	// Past init the fusion is isolated: stm[0], stm[hmc] and opp[hmc+1] are read
	// over the king-neighbour reach, which stays inside it.
	...
	auto pin = [&](const std::vector<uint8_t>& groups, size_t count) {
		...
		return try_pin_phase(
			thread_pool, &m_table->m_dtm[WHITE][0], &m_table->m_dtm[BLACK][0],
			m_scratch_need[WHITE], m_scratch_need[BLACK], 3 * count);
	};
	...
	for (size_t g : m_pair_group_ids)
	{
		...
		const auto rets = thread_pool->run_sync_task_on_all_threads([&](size_t) -> bool {
			...
			for (auto [chunk_start, chunk_end] : group_it.chunks())
			{
				...
				bool any_legal = false;
				auto rows = make_in_pair_rows<DTM50_Final_Entry>(
				...
				while (const auto row = rows.next())
				{
					     ...
					     idx = static_cast<Board_Index>(static_cast<size_t>(idx) + 1))
					{
						...
						const DTM50_Final_Entry fe = row.data[static_cast<size_t>(idx) - row_lo];
	...
	In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	for (size_t bi = 0; bi < batches.size(); ++bi)
	{
		...
		const auto fusions = compute_fusion_groups(m_table->m_dtm[WHITE][0], batch, 2);
...
void DTM50_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	for (Color me : colors)
	{
		...
		{
			// Inherited from DTM, not searched: a permutation only decides which
			// positions are neighbours, so its quality belongs to the index
			// geometry and the value function, not the alphabet emitted downstream.
			// Every DTM50 field is a function of DTM's value function. Share a
			// permutation iff you share a value function.
