...
void DTM50_Generator::init_entries(In_Out_Param<Thread_Pool> thread_pool)
{
	...
	m_phase_pinned = pin(m_all_groups, m_all_groups, 2 * ngroups)
	...
	auto add_init_need = [&](size_t g, uint8_t* need_w, uint8_t* need_b) {
		const size_t hmc_base = static_cast<size_t>(hmc) * ngroups;
		need_w[hmc_base + g] = 1;
		need_b[hmc_base + g] = 1;
		need_w[g] = 1;
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		...
		page_in_window(thread_pool,
			&m_table->m_dtm[WHITE][0], &m_table->m_dtm[BLACK][0],
			pos, DTM50_HMC_COUNT, m_all_groups, add_init_need, nullptr);
...
void DTM50_Generator::build_layer(In_Out_Param<Thread_Pool> thread_pool, uint16_t hmc)
{
	...
	auto add_need = [&](size_t group, uint8_t* need_w, uint8_t* need_b) {
		...
		need[stm][group] = 1;
		need[stm][static_cast<size_t>(hmc) * ngroups + group] = 1;
		const size_t g_start = group * spg;
		const size_t g_end   = std::min(g_start + spg, m_epsi.num_slices());
		mark_king_neighbor_reach(
			need[pin_opp] + static_cast<size_t>(hmc + 1) * ngroups,
			g_start, g_end, spg);
	};
	...
	for (size_t pos = 0; pos < m_pair_group_ids.size(); ++pos)
	{
		...
		page_in_window(thread_pool,
			&m_table->m_dtm[WHITE][0], &m_table->m_dtm[BLACK][0],
			pos, DTM50_HMC_COUNT,
			m_iter_groups[stm], add_need, &m_table->m_dtm[pin_opp][hmc + 1]);
...
NODISCARD Layered_Compressed_EGTB save_compress_dtm50(
	In_Out_Param<Thread_Pool> thread_pool,
	DTM50_Table& table,
	DTM50_Save_Cache& cache,
	const DTM_File_For_Probe& dtm,
...
void DTM50_Generator::save_to_disk(In_Out_Param<Thread_Pool> thread_pool, const EGTB_Paths& paths)
{
	...
	DTM50_Save_Cache cache(std::move(all_tables), m_paging_budget_bytes);
	...
	for (Color me : colors)
	{
		...
		const bool any = gather_dtm50_info(
			...
			thread_pool, max_workers, m_info, ranks);
		...
		else
		{
			...
			color_out[me] = save_compress_dtm50(
