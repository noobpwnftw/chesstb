		...
		if (opt.estimate_only)
		{
			...
			for (const auto& name : opt.materials)
			{
				...
				std::printf("  iter per-dispatch     : %zu groups = %s  (minimum)\n",
					w.peak_dispatch_iter_groups,
					fmt_bytes(w.peak_dispatch_iter_groups * w.bytes_per_group).c_str());
				std::printf("  iter per-mirror-pair  : %zu groups = %s  (cache fits one mirror pair's trajectory)\n",
				...
				std::printf("  init per-dispatch     : %zu groups = %s\n",
					w.peak_dispatch_init_groups,
					fmt_bytes(w.peak_dispatch_init_groups * w.bytes_per_group).c_str());
				std::printf("  init per-mirror-pair  : %zu groups = %s\n",
				...
				if (opt.build_dtm50)
				{
					...
					std::printf("  dtm50 per-dispatch    : %zu groups = %s  (%s binds)\n",
						floor_g, fmt_bytes(floor_g * w.bytes_per_group).c_str(),
