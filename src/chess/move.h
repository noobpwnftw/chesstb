...
struct Move_List
{
	...
	void swap_with_last_and_pop(size_t idx)
	{
		ASSERT(idx < m_size);
	...
	NODISCARD Move   operator[](size_t pos) const { return m_moves[pos]; }
	NODISCARD const Move* begin() const { return m_moves; }
	NODISCARD Move* begin()       { return m_moves; }
	NODISCARD const Move* end()   const { return m_moves + m_size; }
	NODISCARD Move* end()         { return m_moves + m_size; }
