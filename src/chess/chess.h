...
public:
	...
	// Opposing pawn pair ('p'): distinguishes a 'p'-table from the free-pieces
	// table with the same `key` (KQpK vs KQK). Not serialized -- the disk header
	// stores value() only; files are opened by name.
	...
	NODISCARD constexpr friend bool operator==(Material_Key lhs, Material_Key rhs) noexcept { return lhs.key == rhs.key && lhs.pair_flag == rhs.pair_flag; }
	NODISCARD constexpr friend bool operator!=(Material_Key lhs, Material_Key rhs) noexcept { return !(lhs == rhs); }
	NODISCARD constexpr friend bool operator< (Material_Key lhs, Material_Key rhs) noexcept { return lhs.key != rhs.key ? lhs.key < rhs.key : (!lhs.pair_flag && rhs.pair_flag); }
...
template <>
struct std::hash<Material_Key>
{
	// A 'p'-material and its free-pieces twin share value(), so the flag rides
	// past the multiply.
	NODISCARD size_t operator()(Material_Key k) const noexcept
	{
		return static_cast<size_t>(k.value()) * 1099511628211ull + k.has_pair();
	}
};
...
NODISCARD constexpr bool is_piece_free_attacker(Piece piece)
{
