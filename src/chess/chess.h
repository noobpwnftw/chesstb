...
public:
	...
	// Frozen opposing pawn pair ('p'): distinguishes a 'p'-table from the
	// free-pieces table with the same `key` (KQpK vs KQK). Not serialized -- the
	// disk header stores value() only; files are opened by name.
	...
	// In-memory map/cache key separating a 'p'-material from its free-pieces twin
	// (same value()). Not for disk: canonical min-keys are < 2^31, top bit free.
	NODISCARD constexpr uint32_t cache_key() const { return key | (pair_flag ? 0x80000000u : 0u); }
