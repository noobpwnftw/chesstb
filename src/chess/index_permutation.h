...
inline constexpr std::array<uint32_t, 9> FACTORIAL = {
	1u, 1u, 2u, 6u, 24u, 120u, 720u, 5040u, 40320u
};

template <typename Config>
NODISCARD bool index_permutation_config_is_valid(
	const Config& cfg,
	uint32_t perm)
{
...
template <typename Config>
NODISCARD std::array<Piece_Class, PIECE_CLASS_NB> storage_within_class_order(
	const Config& cfg,
	uint32_t perm)
{
	...
	for (size_t i = 0; i < n; ++i)
	{
		const uint32_t f = FACTORIAL[n - 1 - i];
