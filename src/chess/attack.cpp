...
// 5248 bishop entries + 102400 rook entries = ~107K * 8 bytes = ~840 KB. Too
// large to fold into the binary as a constexpr table, so it is built once by
// the constructor of the single global below, before main() runs. Nothing
// outside this file touches it, and nothing in this program reads attacks
// during static initialization, so no ordering hazard exists — the debug flag
// below asserts that if it ever changes.
struct Slider_Tables
{
	// Indexed by [square][pext(occupancy, mask)]. Per-square offsets into the
	// flat data arrays keep memory tight.
	struct Entry
	{
		uint64_t mask;
