...
struct Position
{
	...
	NODISCARD bool is_attacked_by(Square sq, Color attacker_color) const
	{
	...
	NODISCARD Legality legality_context() const
	{
	...
	NODISCARD bool is_pseudo_legal_move_legal(Move m, const Legality& ctx) const
	{
	...
	// Rank-mirrored, color-swapped copy, with the side to move flipped: the same
	...
	// True iff `m` is a pawn crossing two ranks, the move that leaves an ep square.
	NODISCARD bool move_is_pawn_double_push(Move m) const
	{
		return piece_type(m_squares[m.from()]) == PAWN && is_pawn_double_push(m);
	}

	// The square such a push skips over, or SQ_END if `m` leaves none. do_move
	// can't record it (see above), so probe-time callers re-derive it per move.
	NODISCARD Square ep_square_after_move(Move m) const
	{
		if (!move_is_pawn_double_push(m))
			return SQ_END;
