...
struct Piece_Config
{
		...
		m_num_pieces(0)
	{
		...
		for (const char c : s)
		{
			// Lowercase 'p' is the opposing pair, not a board piece: it never
	...
	NODISCARD std::string name() const
	{
		...
		// opposing pair contributes one pawn to each side, so emit a 'p' at the end
	...
	{
		return m_num_pieces <= 2 && !m_has_opposing_pair;
	}
	...
	NODISCARD Piece_Config with_removed_piece(size_t idx) const
	{
		...
		// A non-pair piece was removed; the opposing pair (if any) rides along.
		if (m_has_opposing_pair)
	...
	NODISCARD Piece_Config with_replaced_piece(size_t idx, Piece replacement) const
	{
		...
		// Promoting a free pawn leaves the opposing pair (if any) intact.
		if (m_has_opposing_pair)
			res.mark_opposing_pair();
		return res;
	}
	...
	// Sets the opposing-pair flag on an already-built config and stamps it into
