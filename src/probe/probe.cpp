...
NODISCARD DTM50_Result recover_mate_at_hmc(const Position& pos, WDL_Entry wdl)
{
	if (wdl == WDL_Entry::WIN)
	{
		...
		for (size_t i = 0; i < ml.size(); ++i)
		{
			...
			Position child = p;
			const Square child_ep = move_is_pawn_double_push(p, ml[i])
				? ep_square_after_double_push(ml[i]) : SQ_END;
			(void)child.do_move(ml[i]);
			if (is_checkmate(child, child_ep))
...
struct Probe_Tables::Impl
{
	...
	// Keyed by material-key int to avoid per-probe string hashing. One lock covers
	// all four maps; the thread-local front in cached_open keeps it off the hit path.
	std::mutex tables_mu;
	std::unordered_map<uint32_t, std::shared_ptr<WDL_File>>   wdl_cache;
	std::unordered_map<uint32_t, std::shared_ptr<DTC_File>>   dtc_cache;
	std::unordered_map<uint32_t, std::shared_ptr<DTM_File>>   dtm_cache;
	std::unordered_map<uint32_t, std::shared_ptr<DTM50_File>> dtm50_cache;
	...
	NODISCARD File* open_table(
		std::unordered_map<uint32_t, std::shared_ptr<File>>& cache,
		const std::vector<std::filesystem::path>& dirs, const char* ext, const Piece_Config& ps)
	{
		...
		return cached_open(epoch, tables_mu, cache, k,
			[&]() -> std::shared_ptr<File> {
...
// Every list starts holding its default directory, and init() appends that same
// one again whenever a table subdirectory is missing, so a repeat would only add
// a redundant lookup and another scan_paths pass. Compared lexically normalized,
// since `./wdl/` and `wdl` name one directory.
