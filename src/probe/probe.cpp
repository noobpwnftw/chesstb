...
constexpr int MAX_DERIVE_DEPTH = 64;
...
void fill_pawnless_dtc_from_dtz(In_Out_Param<Probe_Result> r, unsigned rule50)
{
	...
	const bool busted = r->dtz > dtc_budget_plies(rule50);
	r->has_dtc = true;
...
WDL_Entry Probe_Tables::Impl::relax_bound_wdl(const Position& pos, int depth)
{
	...
	(void)pos.visit_legal_moves([&](Move m) FORCE_INLINE_LAMBDA {
		...
		if (cw == WDL_Entry::ILLEGAL) { best = WDL_Entry::ILLEGAL; return false; }

		const WDL_Entry mine = invert_wdl(cw);
		if (mine == WDL_Entry::WIN) { best = WDL_Entry::WIN; return true; }
		if (best != WDL_Entry::ILLEGAL && wdl_rank(mine) > wdl_rank(best)) best = mine;
		return false;
	});
	return best;
}
...
{
	if (!w.is_relaxed[frame] || stored == WDL_Entry::ILLEGAL || stored == WDL_Entry::WIN)
		return stored;
	const WDL_Entry bound = relax_bound_wdl(pos, depth);
	if (bound == WDL_Entry::ILLEGAL) return WDL_Entry::ILLEGAL;
...
WDL_Stored Probe_Tables::Impl::read_wdl_stored(WDL_File* w, const Position& pos, int depth)
{
	...
	if (!w->is_relaxed[stm] || s == WDL_Stored::ILLEGAL || s == WDL_Stored::WIN)
		return s;
	const WDL_Entry bound = relax_bound_wdl(pos, depth);
	if (bound == WDL_Entry::ILLEGAL) return WDL_Stored::ILLEGAL;
	if (s == WDL_Stored::BOUNDARY_WIN)
		return bound == WDL_Entry::WIN ? WDL_Stored::WIN : s;
	const WDL_Entry stored = wdl_from_storage(s);
	...
	const bool pinned = pos.visit_legal_moves([&](Move m) FORCE_INLINE_LAMBDA {
		...
		else
		{
			const unsigned child_rule50 = push ? 0u
				: (rule50 == IGNORE_50MR ? 1u : rule50 + 1u);
	...
	const Position& pos, WDL_Entry wdl, int depth)
{
	...
	const bool pinned = pos.visit_legal_moves([&](Move m) FORCE_INLINE_LAMBDA {
		...
		// A blessed loss counts only the moves that keep it blessed: into a
		// cursed win, or quietly into a win already at the horizon.
		if (!winning && cw != WDL_Entry::CURSED_WIN && (conversion || push)) return false;
		...
		const auto cell = probe_dtc_internal(open_dtc(c.ps), c.ps, c.pos, cw,
		                                     IGNORE_50MR, depth + 1);
		if (!cell) { skipped.unknown(); return false; }
		if (!winning && cw == WDL_Entry::WIN && cell->dtz < DTZ_MAX_NON_CURSED) return false;
...
Probe_Result Probe_Tables::Impl::probe_impl(const Piece_Config& ps, const Position& pos, unsigned rule50, Square ep_square, int depth)
{
	...
	if (ep_square == SQ_END || r.wdl == WDL_Entry::ILLEGAL)
		return r;
	...
	bool any_ep = false;
	std::optional<bool> has_non_ep_move;
	...
	const bool unknown = pos.visit_legal_ep_captures(ep_square, [&](Move m) FORCE_INLINE_LAMBDA {
		...
		if (!has_non_ep_move)
		{
			// The table entry has no en-passant state.  Its terminal result is not
			// an option when en-passant is the only move in the real position.
			has_non_ep_move = pos.visit_legal_moves(
				[](Move) FORCE_INLINE_LAMBDA { return true; });
		}
		if (!*has_non_ep_move && !any_ep)
		{
			best.status = Probe_Result::Status::OK;
			best.wdl = my_wdl;
			...
			best_dtm_wdl = fold_dtm_wdl(my_wdl);
			best_dtm = cr.has_dtm ? static_cast<uint16_t>(1u + cr.dtm) : 0;
			best.has_dtm = cr.has_dtm;
			best.dtm = cr.has_dtm
				&& (best_dtm_wdl == WDL_Entry::WIN || best_dtm_wdl == WDL_Entry::LOSE)
				? best_dtm : 0;

			best_dtc_wdl = fold_50mr_wdl(my_wdl);
			best_dtc_order = 0;
			best_dtc = 1;
			best.has_dtc = true;
			best.dtc_wdl = best_dtc_wdl;
			best.dtc_order = 0;
			best.dtc = best_dtc_wdl == WDL_Entry::DRAW ? 0 : 1;

			best.has_dtm50 = cr.has_dtm50;
			best_dtm50_wdl = cr.has_dtm50 ? invert_wdl(cr.dtm50_wdl) : WDL_Entry::ILLEGAL;
			best_dtm50 = cr.has_dtm50 ? static_cast<uint16_t>(1u + cr.dtm50) : 0;
			best.dtm50_wdl = best_dtm50_wdl;
			best.dtm50 = cr.has_dtm50
				&& (best_dtm50_wdl == WDL_Entry::WIN || best_dtm50_wdl == WDL_Entry::LOSE)
				? best_dtm50 : 0;
	...
	if (rule50_drawn)
	{
		// Even an EP conversion costs a ply; the claim is already available.
		best.has_dtc = true;
		best.dtc_wdl = WDL_Entry::DRAW;
		best.dtc_order = 0;
		best.dtc = 0;
		best.has_dtm50 = true;
		best.dtm50_wdl = WDL_Entry::DRAW;
		best.dtm50 = 0;
	}
	return best;
}
...
WDL_Entry Probe_Tables::Impl::probe_wdl_impl(const Piece_Config& ps, const Position& pos, Square ep_square, int depth)
{
	...
	(void)pos.visit_legal_ep_captures(ep_square, [&](Move m) FORCE_INLINE_LAMBDA {
		...
		if (!has_non_ep_move)
			has_non_ep_move = pos.visit_legal_moves(
				[](Move) FORCE_INLINE_LAMBDA { return true; });
		if ((!*has_non_ep_move && !any_ep) || wdl_rank(mine) > wdl_rank(best))
			best = mine;
		any_ep = true;
		return false;
	});
	if (unresolved_ep && best != WDL_Entry::WIN)
		return WDL_Entry::ILLEGAL;
	return best;
}
...
// An exact DTZ wins while v + cnt50 <= 100. A cursed win or blessed loss past
// 900 plies keeps its side of the draw rank; equal ranks sort by dtz.
int fathom_dtz_rank(int v, unsigned cnt50, bool has_repeated)
{
	if (v > 0) return (static_cast<int>(v + cnt50) <= 100 && !has_repeated)
		? 1000 : std::max(1, 1000 - static_cast<int>(v + cnt50));
	if (v < 0) return (-v * 2 + static_cast<int>(cnt50) < 100)
		? -1000 : std::min(-1, -1000 + static_cast<int>(-v + cnt50));
	return 0;
}
	...
	unsigned rule50, bool use_rule50, bool has_repeated)
{
	...
	auto visit = [&](Move m) FORCE_INLINE_LAMBDA {
		...
		// A quiet root move spends one ply even when the root rank ignores 50MR.
		const unsigned child_rule50 = !use_rule50
			? (c.is_zeroing ? IGNORE_50MR : 1u)
		...
		WDL_Entry my_wdl = invert_wdl(cr.wdl);
		int v;
		if (c.is_zeroing)
		{
		...
		else
		{
			...
			else if (const int child = -signed_dtz_of(cr); child != 0)
			{
				const int d = (child > 0 ? child : -child) + 1;
				const bool cursed = cr.wdl == WDL_Entry::CURSED_WIN
					|| cr.wdl == WDL_Entry::BLESSED_LOSS || d > 100;
				if (cursed)
					my_wdl = child > 0 ? WDL_Entry::CURSED_WIN : WDL_Entry::BLESSED_LOSS;
				v = (child > 0 ? 1 : -1) * (cursed ? 100 + d : d);
			}
			else
				v = 0;
		}

		int rank = fathom_dtz_rank(v, use_rule50 ? rule50 : 0u, has_repeated);
	...
	const Position& pos, Square ep_square, unsigned rule50, bool use_rule50)
{
	...
	auto visit = [&](Move m) FORCE_INLINE_LAMBDA {
		...
		{
			// A root at clock 100 or more is already drawn.
			const WDL_Entry rule_wdl = rule50 >= 100 ? WDL_Entry::DRAW : invert_wdl(cr.dtm50_wdl);
