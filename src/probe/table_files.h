...
constexpr size_t DTM50_PACK_LAYERS = DTM50_HMC_COUNT + 1;
...
struct DTM50_Rank_Per_Color : Block_Cache<>
{
	using Block_Cache::Block_Cache;
...
struct Table_File
{
	...
	auto read(Color c, const Position& pos, Args... args)
	{
		ASSERT(!is_dropped[c]);
		if (is_singular[c])
			return Traits::read(per_color[c], true, BOARD_INDEX_ZERO, args...);

		const Board_Index storage_pos =
			board_index_of_position(*index_cfg, per_color[c].layout, pos);
		return Traits::read(per_color[c], false, storage_pos, args...);
	}
};
