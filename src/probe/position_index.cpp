	...
	Symmetry_Transform t)
{
	...
	// Transform whatever pawns are present, including a frozen pair with no
	// free-pawn class -- key off the placement contents, not is_populated.
...
placements_from_position(const Position_Index_Config& cfg, const Position& pos)
{
	...
	// pawns with no free-pawn class (e.g. KpKp). The pair members are folded in
	// here and split back out by Pair_Group::find_canonical at lookup time.
	...
	const Piece_Group* black_pawns)
{
	...
	{
		const auto pair_idx = static_cast<Pair_Group::Index>(cart % m_pair_table_size);
		const size_t rem = cart / m_pair_table_size;
		const auto w_idx = static_cast<Piece_Group::Placement_Index>(rem % m_white_table_size);
		const auto b_idx = static_cast<Piece_Group::Placement_Index>(rem / m_white_table_size);

		size_t n = 0;
		if (m_pair_group)
		{
			sqbuf[n++] = m_pair_group->white_square(pair_idx);
			sqbuf[n++] = m_pair_group->black_square(pair_idx);
		}
		if (m_white_group)
		{
			const auto pl = m_white_group->squares(w_idx);
			for (size_t k = 0; k < pl.size(); ++k) sqbuf[n++] = pl[k];
		}
		if (m_black_group)
		{
			const auto pl = m_black_group->squares(b_idx);
			for (size_t k = 0; k < pl.size(); ++k) sqbuf[n++] = pl[k];
		}
		...
		if (m_pair_group)
		{
			const Square pair_w = m_pair_group->white_square(pair_idx);
			const Square pair_b = m_pair_group->black_square(pair_idx);
			...
			{
				const auto pl = m_white_group->squares(w_idx);
				for (size_t k = 0; k < pl.size(); ++k) wsq[nw++] = pl[k];
			}
			if (m_black_group)
			{
				const auto pl = m_black_group->squares(b_idx);
				for (size_t k = 0; k < pl.size(); ++k) bsq[nb++] = pl[k];
			}
			Square cw = SQ_END, cb = SQ_END;
			Pair_Group::canonical_pair(Const_Span<Square>(wsq, nw),
			                           Const_Span<Square>(bsq, nb), cw, cb);
			if (cw != pair_w || cb != pair_b) continue;
		}
...
bool Position_Index_Config::try_init()
{
	...
	// The pair's two pawns are physical board pawns but not free-pawn classes.
	if (has_frozen_pair())
	{
	...
	const Position& pos)
{
	...
	const Square wk = placements[WHITE_KINGS][0];
	const Square bk = placements[BLACK_KINGS][0];
	const int32_t king_slice_id = cfg.king_slice_manager().lookup(wk, bk).slice_id;
