...
Block_Ptr dtc_get_block(LZMA_Rank_Per_Color& pc, size_t block_id)
{
	...
	lzma_decompress_into(
		scratch, Const_Span<uint8_t>(pc.compressed_data + doff, dsz));
...
void DTC_Traits::on_singular(Serial_Memory_Reader& reader, Per_Color& pc)
{
...
uint16_t DTC_Traits::read(Per_Color& pc, bool is_singular, Board_Index pos, WDL_Entry wdl)
{
	...
	if (is_singular) return dtc_value_from_storage(pc.single_val, wdl, 1);
	...
	const uint8_t* data = fetch_block_cached<DTC_Traits>(pc, block_id, dtc_get_block);
	uint16_t stored;
