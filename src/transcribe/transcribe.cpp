...
NODISCARD Stored_Bitmap build_legal_bitmap(In_Out_Param<Thread_Pool> thread_pool,
                                           const Piece_Config_For_Gen& epsi, Color color,
                                           KeepFn&& keep)
{
	const size_t num_positions = epsi.num_positions();
	Stored_Bitmap out;
	out.bits = Huge_Array<uint8_t>((num_positions + 7) / 8);
	...
	thread_pool->run_sync_task_on_multiple_threads(workers, [&](size_t) {
		...
		for (;;)
		{
			...
			for (size_t i = lo; i < hi; ++i)
			{
				...
				if (keep(legal.position(), idx)) out.set(i);
			}
		}
	});
	return out;
...
{
	return build_legal_bitmap(thread_pool, epsi, color,
		[relax, color](Position_For_Gen& pos, Board_Index idx) {
			return relax == nullptr || !relax->can_omit(pos, color, idx);
		});
}
...
{
	return build_legal_bitmap(thread_pool, epsi, color,
		[&src, color](Position_For_Gen&, Board_Index idx) {
			const WDL_Entry w = wdl_from_storage(src.m_wdl.read(color, idx));
			ASSERT(w != WDL_Entry::ILLEGAL);
			return w != WDL_Entry::DRAW;
		});
}
	...
	Stored_Bitmap& priced)
{
	...
	priced.bits = Huge_Array<uint8_t>((num_positions + 7) / 8);
...
template <typename Has_Payload, typename Is_Singular, typename Payload_Size>
	...
	Payload_Size payload_size)
{
	...
	const bool w = has_payload(WHITE);
	const bool b = has_payload(BLACK);
	if (w && b)
		return payload_size(WHITE) >= payload_size(BLACK) ? WHITE : BLACK;
	if (w && is_singular(BLACK)) return WHITE;
	if (b && is_singular(WHITE)) return BLACK;
	return COLOR_NB;
}
...
{
	if (!enabled) return;
	const Color drop = shrink_color(
		colors, loss_only,
		[&](Color c) { return save[c].has_payload(); },
		[&](Color c) { return save[c].is_singular(); },
		[&](Color c) { return save[c].total_compressed_size(); });
	if (drop == COLOR_NB) return;
	std::printf("  shrink: drop %s STM payload\n", drop == WHITE ? "WHITE" : "BLACK");
	save[drop] = Compressed_EGTB::make_dropped();
}

void shrink_before_write(
	Layered_Compressed_Color save[COLOR_NB],
	const Fixed_Vector<Color, 2>& colors,
	bool enabled,
	bool loss_only)
{
	...
	if (drop == COLOR_NB) return;
	std::printf("  shrink: drop %s STM payload\n", drop == WHITE ? "WHITE" : "BLACK");
	save[drop] = Layered_Compressed_Color{};
	save[drop].is_dropped = true;
}
                    ...
                    const Transcribe_Options& opt)
{
	...
	shrink_before_write(save, src.m_colors, opt.shrink, /*loss_only=*/false);
                    ...
                    const Transcribe_Options& opt)
{
	...
	for (const Color c : src.m_colors)
	{
		...
		else
		{
			...
			priced = build_priced_bitmap(thread_pool, src, epsi, c);
		}
