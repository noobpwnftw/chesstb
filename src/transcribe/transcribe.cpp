	...
	size_t entry_bytes)
{
	...
	return Block_Source{
		...
		[](size_t first_block, size_t) { return first_block + 1; },
		[&src, &epsi, relax, color, plan, source_block_bytes, source_total_bytes](
			size_t, size_t block_id, Span<uint8_t> scratch) -> Const_Span<uint8_t>
		{
			...
			for (size_t i = 0; i < cnt; ++i)
			{
				...
				const Board_Index idx = static_cast<Board_Index>(logical);
				// Illegal and omitted cells go to the compressor as don't-cares.
				const bool stored = legal(logical)
					&& (relax == nullptr || !relax->can_omit(legal.position(), color, idx));
				const Entry e = stored ? src.read(color, idx) : Entry::make_illegal();
	...
	Rank_Storage_Mode storage_mode)
{
	...
	auto locals = thread_pool->run_sync_task_on_multiple_threads(
		workers, [&](size_t worker_id) -> Value_Histogram {
			Value_Histogram h;
			...
			for (;;)
			{
				...
				const Const_Span<uint8_t> block =
					src.get(worker_id, b, Span<uint8_t>(scratch.data(), scratch.size()));
				switch (storage_mode)
				{
	...
	const Relax_Bound* relax)
{
	...
	return Block_Source{
		total_packed_bytes,
		[](size_t first_block, size_t) { return first_block + 1; },
		[&src, &epsi, color, plan, num_positions, total_packed_bytes, block_size, relax](
			size_t, size_t block_id, Span<uint8_t> scratch) -> Const_Span<uint8_t>
		{
	...
	const Relax_DTZ* relax)
{
	...
	else
	{
		const Block_Source hist_src = make_source_block_source(
			src, epsi, relax, color, make_index_permutation_plan(epsi, 0),
	...
	*out_perm = 0;
		...
		*out_perm = choose_storage_permutation_config(
			...
			[&](uint32_t perm) {
				return make_source_block_source(
					src, epsi, relax, color, make_index_permutation_plan(epsi, perm),
	...
	Block_Source out_src = make_source_block_source(
		src, epsi, relax, color, make_index_permutation_plan(epsi, *out_perm),
		num_positions, block_size, entry_bytes);
	return save_compress_egtb(thread_pool, out_src, color, entry_bytes, block_size, /*max_workers=*/0,
	                          spill_path, chosen, storage_mode);
}
...
// A cell is priced when it is legal and its class is packed. Under loss-only a
// cell whose row 0 reads ILLEGAL does not count, as in the rank gathering; the
// encoder ignores the rest of such a column.
template <typename Source>
NODISCARD bool fill_pack_chunk(const Source& src, Legal_Slots& legal, Color color,
                               const Index_Permutation_Plan& perm_plan, bool loss_only,
                               size_t p_base, size_t this_bp, uint16_t* chunk)
{
	...
	for (size_t k = 0; k < this_bp; ++k)
	{
		...
		const Board_Index idx = static_cast<Board_Index>(logical);
		if (!legal(logical)
		 || !pack_prices_class(wdl_from_storage(src.m_wdl.read(color, idx)), loss_only))
		{
			chunk[k] = DTM_Final_Entry::ILLEGAL_VAL;
			continue;
		}
		src.read_column(color, idx, chunk + k, this_bp);
		if (loss_only && chunk[k] == DTM_Final_Entry::ILLEGAL_VAL) continue;
		any_priced = true;
	}
	return any_priced;
}
...
template <typename Source>
NODISCARD Layered_Compressed_EGTB compress_layered_color(
	In_Out_Param<Thread_Pool> thread_pool,
	const Source& src,
	const Piece_Config_For_Gen& epsi,
	Color color,
...
{
	const size_t bp = block_size / ranks.entry_bytes;
	const size_t bcnt = ceil_div(num_positions, bp);
	const size_t last = num_positions - (bcnt - 1) * bp;
	const size_t tail = (last == bp) ? 0 : last;
	auto compressed_blocks = Compressed_Block_Store(std::move(spill_path), bcnt, block_size);
	std::vector<uint64_t> usizes(bcnt);
	...
	thread_pool->run_sync_task_on_multiple_threads(workers, [&](size_t) {
		...
		for (;;)
		{
			const size_t b = next.fetch_add(1, std::memory_order_relaxed);
			if (b >= bcnt) return;
			const size_t this_bp = (b == bcnt - 1 && tail != 0) ? tail : bp;

			if (!fill_pack_chunk(src, legal, color, perm_plan, loss_only, b * bp, this_bp,
			                     chunk.data()))
			{
				// usz 0 in the offset table is the skip sentinel.
				usizes[b] = 0;
				compressed_blocks.clear(b);
				progress_bar += 1;
				continue;
			}
			...
			usizes[b] = payload.size();
			compressed_blocks.set(b, Const_Span<uint8_t>(compressed));
			progress_bar += 1;
		}
	});

	progress_bar.set_finished();
	return Layered_Compressed_EGTB(
		std::move(compressed_blocks), std::move(usizes), bp, tail, std::move(ranks));
}
...
NODISCARD std::filesystem::path wdl_source_path(
	const std::vector<std::filesystem::path>& dirs, const Piece_Config& ps)
{
	for (const auto& dir : dirs)
	{
		const auto path = path_join(dir, ps.name() + EGTB_Paths::WDL_EXT);
		if (file_exists_case_exact(path)) return path;
	}
	throw std::runtime_error("No source WDL table for " + ps.name());
}
...
inline constexpr bool ALL_DROPPABLE[COLOR_NB] = { true, true };

template <typename Saved>
NODISCARD Color shrink_color(
	const Saved save[COLOR_NB],
	const Fixed_Vector<Color, 2>& colors,
	bool loss_only,
	const bool droppable[COLOR_NB])
{
	if (loss_only || colors.size() != COLOR_NB) return COLOR_NB;
	const bool w = save[WHITE].has_payload() && droppable[WHITE];
	const bool b = save[BLACK].has_payload() && droppable[BLACK];
	if (w && b)
		return save[WHITE].total_compressed_size() >= save[BLACK].total_compressed_size()
			? WHITE : BLACK;
	if (w && (save[BLACK].is_singular() || save[BLACK].has_payload())) return WHITE;
	if (b && (save[WHITE].is_singular() || save[WHITE].has_payload())) return BLACK;
	return COLOR_NB;
}

template <typename Saved>
void shrink_before_write(
	Saved save[COLOR_NB],
	const Fixed_Vector<Color, 2>& colors,
	bool enabled,
	bool loss_only,
	const bool droppable[COLOR_NB] = ALL_DROPPABLE)
{
	if (!enabled) return;
	const Color drop = shrink_color(save, colors, loss_only, droppable);
	if (drop == COLOR_NB) return;
	std::printf("  shrink: drop %s STM payload\n", drop == WHITE ? "WHITE" : "BLACK");
	save[drop] = Saved::make_dropped();
}
	...
	const Relax_DTZ* relax)
{
	...
	shrink_before_write(save, src.m_colors, opt.shrink, opt.loss_only);
...
void transcribe_dtz(In_Out_Param<Thread_Pool> thread_pool, const Piece_Config& ps,
                  ...
                  const std::vector<std::filesystem::path>& wdl_dirs,
                  const Transcribe_Options& opt)
{
	...
	src.open(ps, source_path(dtz_dir, ps, EGTB_Paths::DTZ_EXT),
	         wdl_source_path(wdl_dirs, ps), opt.loss_only,
	...
	{
		EGTB_Paths paths;
		for (const auto& dir : wdl_dirs) paths.add_wdl_path(dir);
...
void transcribe_wdl(In_Out_Param<Thread_Pool> thread_pool, const Piece_Config& ps,
                    ...
                    const Transcribe_Options& opt)
{
	const auto out = dest_path(opt, wdl_source_path(wdl_dirs, ps).parent_path(), ps, EGTB_Paths::WDL_EXT);
	Source_WDL src;
	...
	src.open(ps, wdl_source_path(wdl_dirs, ps),
	...
	for (const Color c : src.m_colors)
	{
		...
		perm[c] = choose_storage_permutation_config(
			thread_pool, epsi,
			[&](uint32_t p) {
				return make_wdl_block_source(src, epsi, c,
		...
		Block_Source out_src = make_wdl_block_source(
			src, epsi, c, make_index_permutation_plan(epsi, perm[c]),
			num_positions, block_size, bound ? &*bound : nullptr);
		save[c] = save_compress_wdl(thread_pool, out_src, out_src, c, block_size, /*max_workers=*/0,
		                            path_join(opt.tmp_dir, ps.name() + EGTB_Paths::BLOCK_SPILL_EXT[c]));
	}
	// A side holding a castling right keeps its WDL frame: giving the right up
	// leaves the material, so the frame could not be rebuilt from the other.
	const bool droppable[COLOR_NB] = {
		ps.castling_rights(WHITE) == 0,
		ps.castling_rights(BLACK) == 0,
	};
	shrink_before_write(save, src.m_colors, opt.shrink, /*loss_only=*/false, droppable);
                    ...
                    const Transcribe_Options& opt)
{
	...
	Layered_Compressed_EGTB color_out[COLOR_NB]{};
	uint32_t perm[COLOR_NB] = { 0, 0 };
	for (const Color c : src.m_colors)
	{
		if (src.m_per_color[c].is_dropped)
		{
			color_out[c] = Layered_Compressed_EGTB::make_dropped();
			continue;
		}
		if (src.m_per_color[c].is_singular)
		{
			color_out[c] = Layered_Compressed_EGTB::make_singular();
			continue;
		}
		...
		if (opt.loss_only)
		{
			if (!gather_layered_ranks(thread_pool, src, epsi, c, num_positions, true, ranks))
			{
				color_out[c] = Layered_Compressed_EGTB::make_singular();
				continue;
			}
		...
		color_out[c] = compress_layered_color(
			thread_pool, src, epsi, c, num_positions, block_size, perm[c], opt.loss_only,
			path_join(opt.tmp_dir, ps.name() + EGTB_Paths::BLOCK_SPILL_EXT[c]),
			std::move(ranks));
	}
	shrink_before_write(color_out, src.m_colors, opt.shrink, opt.loss_only);
