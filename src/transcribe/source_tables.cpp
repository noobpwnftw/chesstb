...
// A transcription re-encodes cell for cell, so a frame that leaves cells to a
// probe-time derive is not a source it can work from. Loss-only is the exception
// when the output is loss-only too: the same cells are don't-cares on both sides.
NODISCARD uint8_t read_frame_flag(Serial_Memory_Reader& r, const std::filesystem::path& path,
                                  const char* kind, bool allow_loss_only = false)
{
	...
	if ((flag & EGTB_LOSS_ONLY_FLAG) && !allow_loss_only)
		throw std::runtime_error(std::string(kind) + " is loss-only; only --loss-only takes it as a source "
			+ path.string());
	if (flag & EGTB_RELAXED_FLAG)
		throw std::runtime_error(std::string(kind) + " is relaxed, which is not a "
			"transcription source; start from the full table " + path.string());
	return flag;
}
	...
	bool allow_loss_only)
{
	...
	for (const Color c : colors)
	{
		...
		// One value over the whole frame, so the writer skipped the payload. It
		// only ever emits that from the 1-byte tier, a rank table of one value
		// being far inside it, and the header carries no entry_bytes to say so.
...
// Rank stream cell read. `Tag` picks the thread-local block FIFO, so the two
// rank sources do not evict each other's entries.
                     ...
                     const std::filesystem::path& path, bool allow_loss_only)
{
	load_rank_source(dtz->m_per_color, dtz->m_colors, dtz->m_file, ps, path,
	                 EGTB_Magic::DTZ_MAGIC, "DTZ", allow_loss_only);
}
...
WDL_Stored Source_WDL::read(Color color, Board_Index pos) const
{
	...
	const uint8_t* decoded = fetch_block_cached<Source_WDL>(m_cache[color], block_id, [&pc](
		Block_Cache<LZ4_Decompress_Helper>& cache, size_t bid) -> Block_Ptr
	{
		...
		const size_t out_sz =
			(bid == pc.block_cnt - 1 && pc.tail_size != 0) ? pc.tail_size : pc.block_size;
		const auto out = dc.decompress(
			Const_Span<uint8_t>(pc.compressed_data + pr[0], pr[1] - pr[0]), out_sz);
		return std::make_shared<const std::vector<uint8_t>>(out.begin(), out.end());
	});

	Packed_WDL_Entries packed;
	std::memcpy(&packed, decoded + in_block, sizeof(packed));
...
// Locate a cell's record. False when the frame or block holds no W/L cell,
// which makes every layer DRAW.
                                         ...
                                         uint16_t* out, size_t stride) const
{
	...
	switch (state)
	{
		...
		default:
		{
			...
			// The flip is the highest changepoint, and nothing past it is read.
			size_t end = layers;
			if (draw_end)
			{
				end = 0;
