	...
	const char* kind)
{
	...
	if ((key_and_table_num >> 2u) != (ps.min_material_key().value() & 0x3FFFFFFFu))
...
// One decoded rs-pack block. The payload keeps its packed 2-bit state map; a
// sparse prefix index and two short-record bitmaps make its variable-width
// streams randomly addressable without a per-position directory.
namespace {

constexpr size_t RS_PREFIX_STRIDE = 256;

struct RS_State_Prefix
{
	uint32_t index;
	uint8_t state;
};

struct RS_Prefix_Entry
{
	uint32_t n_const;
	uint32_t n_single;
	uint32_t n_double;
	uint32_t n_multi;
	uint32_t n_single_short;
	uint32_t n_double_short;
};
static_assert(sizeof(RS_Prefix_Entry) == 24);

struct RS_Cached_Block
{
...
static_assert(sizeof(RS_Cached_Block) % 4 == 0);

NODISCARD INLINE size_t popcount_bit_range(
	const uint8_t* bitmap, size_t lo, size_t hi)
{
	if (lo >= hi) return 0;
	size_t count = 0;
	size_t byte = lo >> 3;
	size_t remaining = hi - lo;
	const size_t head = lo & 7;
	if (head)
	{
		const uint8_t b = static_cast<uint8_t>(bitmap[byte] >> head);
		const size_t available = 8 - head;
		if (remaining <= available)
			return __builtin_popcount(
				static_cast<uint8_t>(b & ((1u << remaining) - 1u)));
		count += __builtin_popcount(b);
		remaining -= available;
		++byte;
	}
	while (remaining >= 64)
	{
		uint64_t word;
		std::memcpy(&word, bitmap + byte, sizeof(word));
	...
	{
		uint64_t word = 0;
		std::memcpy(&word, bitmap + byte, (remaining + 7) / 8);
...
INLINE void popcount_state_range(
	const uint8_t* states, size_t bit_lo, size_t bit_hi,
	uint32_t& num_single, uint32_t& num_double, uint32_t& num_multi)
{
	...
	{
		uint64_t word;
		std::memcpy(&word, states + bit / 8, sizeof(word));
		const uint64_t lo = word & EVEN_BITS;
		const uint64_t hi = (word >> 1) & EVEN_BITS;
		num_single += __builtin_popcountll(lo & ~hi);
		num_double += __builtin_popcountll(hi & ~lo);
		num_multi += __builtin_popcountll(lo & hi);
		bit += 64;
	}
	if (bit < bit_hi)
	{
		const size_t remaining = bit_hi - bit;
		uint64_t word = 0;
		std::memcpy(&word, states + bit / 8, (remaining + 7) / 8);
		word &= (uint64_t{ 1 } << remaining) - 1;
		const uint64_t lo = word & EVEN_BITS;
		const uint64_t hi = (word >> 1) & EVEN_BITS;
		num_single += __builtin_popcountll(lo & ~hi);
...
INLINE void build_rs_prefix_index(
	const uint8_t* states, const uint8_t* single_hints, const uint8_t* double_hints,
	size_t positions, RS_Prefix_Entry* out, size_t num_strides)
{
	uint32_t nc = 0, ns = 0, nd = 0, nm = 0;
	uint32_t ns_short = 0, nd_short = 0;
	for (size_t s = 0; s < num_strides; ++s)
	{
		out[s] = { nc, ns, nd, nm, ns_short, nd_short };
		const size_t pos_lo = s * RS_PREFIX_STRIDE;
		const size_t pos_hi = std::min(pos_lo + RS_PREFIX_STRIDE, positions);
		const uint32_t ns_before = ns;
		const uint32_t nd_before = nd;
		const uint32_t nm_before = nm;
		popcount_state_range(states, pos_lo * 2, pos_hi * 2, ns, nd, nm);
		const uint32_t added_nonconst =
			(ns - ns_before) + (nd - nd_before) + (nm - nm_before);
		nc += static_cast<uint32_t>(pos_hi - pos_lo) - added_nonconst;
		ns_short += static_cast<uint32_t>(
			popcount_bit_range(single_hints, ns_before, ns));
		nd_short += static_cast<uint32_t>(
			popcount_bit_range(double_hints, nd_before, nd));
	}
}

enum class RS_State_Class : uint8_t { SINGLE, DOUBLE, MULTI, NONCONST };

template <RS_State_Class State>
NODISCARD INLINE uint32_t tally_state_word(uint64_t word)
{
	constexpr uint64_t EVEN_BITS = 0x5555555555555555ull;
	const uint64_t lo = word & EVEN_BITS;
	const uint64_t hi = (word >> 1) & EVEN_BITS;
	if constexpr (State == RS_State_Class::SINGLE)      return __builtin_popcountll(lo & ~hi);
	else if constexpr (State == RS_State_Class::DOUBLE) return __builtin_popcountll(hi & ~lo);
	else if constexpr (State == RS_State_Class::MULTI)  return __builtin_popcountll(lo & hi);
	else                                                return __builtin_popcountll(lo | hi);
}

template <RS_State_Class State>
NODISCARD uint32_t popcount_state_class(
	const uint8_t* states, size_t bit_lo, size_t bit_hi)
{
	uint32_t count = 0;
	size_t bit = bit_lo;
	while (bit + 64 <= bit_hi)
	{
		uint64_t word;
		std::memcpy(&word, states + bit / 8, sizeof(word));
		count += tally_state_word<State>(word);
		bit += 64;
	}
	if (bit < bit_hi)
	{
...
NODISCARD INLINE RS_State_Prefix rs_state_prefix(
	const uint8_t* states, const RS_Prefix_Entry* prefix, size_t pos)
{
	const size_t bit_pos = pos * 2;
	const uint8_t state = (states[bit_pos / 8] >> (bit_pos % 8)) & 3u;
	const size_t stride = pos / RS_PREFIX_STRIDE;
	const RS_Prefix_Entry snap = prefix[stride];
	const size_t bit_lo = stride * RS_PREFIX_STRIDE * 2;
	...
	switch (state)
	{
		case 1:
			index = snap.n_single
			      + popcount_state_class<RS_State_Class::SINGLE>(states, bit_lo, bit_pos);
			break;
		case 2:
			index = snap.n_double
			      + popcount_state_class<RS_State_Class::DOUBLE>(states, bit_lo, bit_pos);
			break;
		case 3:
			index = snap.n_multi
			      + popcount_state_class<RS_State_Class::MULTI>(states, bit_lo, bit_pos);
			break;
		default:
		{
			const uint32_t walked =
				static_cast<uint32_t>(pos - stride * RS_PREFIX_STRIDE);
			const uint32_t nonconst =
				popcount_state_class<RS_State_Class::NONCONST>(states, bit_lo, bit_pos);
			index = snap.n_const + walked - nonconst;
			break;
		}
	}
	return RS_State_Prefix{ index, state };
}

NODISCARD INLINE size_t single_record_width(const uint8_t* entry, size_t entry_bytes)
{
	return (entry[0] & 0x80u) ? 1 + entry_bytes : 1 + 2 * entry_bytes;
}

NODISCARD INLINE size_t double_record_width(const uint8_t* entry, size_t entry_bytes)
{
	return (entry[1] & 0x80u) ? 2 + 2 * entry_bytes : 2 + 3 * entry_bytes;
}
...
NODISCARD Block_Ptr decode_rs_block(const Source_Layered_Per_Color& pc, size_t block_id)
{
	...
	const size_t payload_off = sizeof(RS_Cached_Block);
	const size_t num_strides = ceil_div(positions, RS_PREFIX_STRIDE);
	const size_t prefix_bytes = num_strides * sizeof(RS_Prefix_Entry);
	const size_t prefix_off = payload_off + usz;
	const size_t hints_off = prefix_off + prefix_bytes;
	auto blob = std::make_shared<std::vector<uint8_t>>(hints_off, 0);
	uint8_t* const raw = blob->data() + payload_off;
	...
	ASSERT(hdr[0] == positions);

	const size_t eb = pc.entry_bytes;
	const size_t num_const = positions - num_single - num_double - num_multi;
	const size_t state_bytes = (positions * 2 + 7) / 8;
	const size_t single_hint_bytes = (num_single + 7) / 8;
	const size_t double_hint_bytes = (num_double + 7) / 8;

	RS_Cached_Block meta{};
	size_t p = 24;
	meta.state_bits_off = narrowing_static_cast<uint32_t>(p); p += state_bytes;
	meta.const_stream_off = narrowing_static_cast<uint32_t>(p); p += num_const * eb;
	meta.single_stream_off = narrowing_static_cast<uint32_t>(p); p += ss_bytes;
	meta.double_stream_off = narrowing_static_cast<uint32_t>(p); p += ds_bytes;
	p += (4 - (p & 3)) & 3;
	meta.multi_dir_off = narrowing_static_cast<uint32_t>(p); p += (num_multi + 1) * 4u;
	meta.multi_data_off = narrowing_static_cast<uint32_t>(p);
	meta.prefix_off = narrowing_static_cast<uint32_t>(prefix_off);
	meta.single_hints_off = narrowing_static_cast<uint32_t>(hints_off - payload_off);
	meta.double_hints_off = narrowing_static_cast<uint32_t>(
		hints_off - payload_off + single_hint_bytes);

	const size_t cached_size = hints_off + single_hint_bytes + double_hint_bytes;
	// resize() may grow geometrically and leave nearly a second payload's worth
	// of uncounted capacity in the block cache. reserve() requests the exact
	// final extent before appending the hint maps.
	blob->reserve(cached_size);
	blob->resize(cached_size, 0);
	std::memcpy(blob->data(), &meta, sizeof(meta));
	uint8_t* const single_hints = blob->data() + payload_off + meta.single_hints_off;
	uint8_t* const double_hints = blob->data() + payload_off + meta.double_hints_off;
	const uint8_t* entry = blob->data() + payload_off + meta.single_stream_off;
	for (size_t i = 0; i < num_single; ++i)
	{
		if (entry[0] & 0x80u)
			single_hints[i >> 3] |= static_cast<uint8_t>(1u << (i & 7));
		entry += single_record_width(entry, eb);
	}
	entry = blob->data() + payload_off + meta.double_stream_off;
	for (size_t i = 0; i < num_double; ++i)
	{
		if (entry[1] & 0x80u)
			double_hints[i >> 3] |= static_cast<uint8_t>(1u << (i & 7));
		entry += double_record_width(entry, eb);
	}

	auto* const prefix = reinterpret_cast<RS_Prefix_Entry*>(
		blob->data() + meta.prefix_off);
	build_rs_prefix_index(blob->data() + payload_off + meta.state_bits_off,
	                      single_hints, double_hints, positions, prefix, num_strides);
	return blob;
}
                                     ...
                                     Board_Index pos, const uint8_t** entry, uint8_t* state)
{
	...
	const auto* meta = reinterpret_cast<const RS_Cached_Block*>(blob);
	const uint8_t* const payload = blob + sizeof(RS_Cached_Block);
	const auto* prefix = reinterpret_cast<const RS_Prefix_Entry*>(
		blob + meta->prefix_off);
	const RS_State_Prefix sp = rs_state_prefix(
		payload + meta->state_bits_off, prefix, in_block);
	const RS_Prefix_Entry& snap = prefix[in_block / RS_PREFIX_STRIDE];
	...
	*state = sp.state;
	...
	{
		case 0:
			*entry = payload + meta->const_stream_off + idx * eb;
			break;
		case 1:
		{
			const uint8_t* hints = payload + meta->single_hints_off;
			const size_t num_short = snap.n_single_short
				+ popcount_bit_range(hints, snap.n_single, idx);
			const size_t byte_off =
				num_short * (1 + eb) + (idx - num_short) * (1 + 2 * eb);
			*entry = payload + meta->single_stream_off + byte_off;
			break;
		}
		case 2:
		{
			const uint8_t* hints = payload + meta->double_hints_off;
			const size_t num_short = snap.n_double_short
				+ popcount_bit_range(hints, snap.n_double, idx);
			const size_t byte_off =
				num_short * (2 + 2 * eb) + (idx - num_short) * (2 + 3 * eb);
			*entry = payload + meta->double_stream_off + byte_off;
			break;
		}
		default:
		{
			uint32_t off;
			std::memcpy(&off, payload + meta->multi_dir_off + idx * 4u, sizeof(off));
			*entry = payload + meta->multi_data_off + off;
			break;
		}
	}
	return true;
