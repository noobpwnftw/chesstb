...
void print_usage()
{
	std::cerr <<
		...
		"  --shrink       fully transcribe both sides, then drop one STM payload\n"
		"                 when the other side can reconstruct it\n"
		"  --relaxed\n"
...
NODISCARD bool parse_args(int argc, char** argv, Options& out)
{
	for (int i = 1; i < argc; ++i)
	{
		...
		else if (a == "--shrink") out.to.shrink = true;
	                          ...
	                          const Transcribe_Options& material_opt)
{
	const auto has_wdl = [&](const Piece_Config& material) {
		return std::any_of(opt.wdl_dirs.begin(), opt.wdl_dirs.end(),
			[&](const auto& dir) { return table_exists(dir, material, EGTB_Paths::WDL_EXT); });
	};
...
int main(int argc, char** argv)
{
	try
	{
		...
		if (opt.to.block_size > size_t{ UINT32_MAX })
		{
			std::cerr << "--block " << opt.to.block_size / 1024 << " exceeds the 32-bit block field\n";
			return 1;
		}
		// A decoded layered block is at most about 124 bytes per block byte, and its
		// stream offsets are 32-bit.
		const bool writes_dtc = !opt.dtc_dir.empty() && (!opt.to.extract_dtz || opt.to.loss_only || opt.to.relaxed);
		const bool writes_dtm50 = !opt.dtm50_dir.empty() && (!opt.to.extract_dtm || opt.to.loss_only || opt.to.relaxed);
		if (!opt.do_wdl && (writes_dtc || writes_dtm50)
		    && opt.to.block_size > size_t{ 16 } << 20)
		{
			std::cerr << "--block " << opt.to.block_size / 1024 << " exceeds 16384 for layered tables\n";
			return 1;
		}
		...
		if ((opt.do_wdl && std::any_of(opt.wdl_dirs.begin(), opt.wdl_dirs.end(), same_as_output))
		...
		auto transcribe_material = [&](const Piece_Config& ps,
			...
			if (opt.do_wdl)
			{
				...
				transcribe_wdl(inout_param(pool), ps, opt.wdl_dirs, material_opt);
				return;
			}
			if (!opt.dtz_dir.empty())
			{
				std::cout << "  " << name << ": transcribing DTZ...\n";
				transcribe_dtz(inout_param(pool), ps, opt.dtz_dir, opt.wdl_dirs, material_opt);
			}
			// DTC is pawnful-only, so a pawnless material has no file to read.
			if (!opt.dtc_dir.empty() && ps.has_pawns())
			{
				std::cout << "  " << name << ": transcribing DTC...\n";
				transcribe_dtc(inout_param(pool), ps, opt.dtc_dir, opt.wdl_dirs, material_opt);
			}
			if (!opt.dtm_dir.empty())
			{
				std::cout << "  " << name << ": transcribing DTM...\n";
				transcribe_dtm(inout_param(pool), ps, opt.dtm_dir, opt.wdl_dirs, material_opt);
			}
			if (!opt.dtm50_dir.empty())
			{
				std::cout << "  " << name << ": transcribing DTM50...\n";
				transcribe_dtm50(inout_param(pool), ps, opt.dtm50_dir, opt.wdl_dirs, material_opt);
			}
		};

		// The settings follow from the material, so they are derived where they
		// are needed rather than carried beside it.
		...
		for (const std::string& name : opt.materials)
		{
			...
			const auto [material_key, mirror_key] = ps.material_keys();
			if (opt.to.relaxed && !opt.do_wdl && material_key != mirror_key
			    && ((!opt.dtc_dir.empty() && ps.has_pawns())
			        || !opt.dtm_dir.empty() || !opt.dtm50_dir.empty()))
			{
				std::cerr << "--relaxed supports only DTZ for asymmetric materials\n";
				return 1;
			}
			...
			for (const Piece_Config& ps : materials)
			{
				...
				try
				{
					...
					if (output_complete(opt, ps, material_opt)
					 || !sources_ready(opt, ps, material_opt)) continue;
					Fleet_Lock lock;
					if (opt.fleet)
					{
