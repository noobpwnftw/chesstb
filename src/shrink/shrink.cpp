...
//
// Without --out each file is rewritten in place: the tool writes
// "<path>.shrink_tmp" and renames it over the original. With --out DIR the
// inputs are left alone and results land in DIR as one flat directory keyed by
// filename only, since the argument list is usually a shell glob spanning
// several table directories. Tables that cannot shrink are copied there
// unchanged so DIR holds a complete table set; non-table files are never
// copied.
//
// With -n/--dry-run no files are written or copied; each file is parsed and the
// size it would shrink to is reported instead. A grand total over all processed
// files is printed at the end.
...
// Where a result lands: over the input in in-place mode, or under --out,
// which is one flat directory, so only the filename carries over.
...
// A table that cannot shrink stays as it is: in-place mode leaves it alone,
// output mode copies it so the output directory is a complete table set.
...
// One-decimal size reduction. Formatted off to the side so the sticky
// float manipulators never leak into the caller's stream.
...
// Either changepoint pack: a drop is container-level, so nothing here needs to
// know what a layer counts or how many of them a pack has.
...
int main(int argc, char** argv)
{
	try {
		...
		// Dry runs write nothing, so the directory is not created either.
