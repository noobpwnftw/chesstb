...
NODISCARD size_t castling_rights_in_name(const std::filesystem::path& path, Color c)
{
	const std::string name = path.stem().string();
	const size_t second_k = name.find('K', 1);
	if (second_k == std::string::npos) return 0;
	const auto first = (c == WHITE) ? name.begin() : name.begin() + second_k;
	const auto last  = (c == WHITE) ? name.begin() + second_k : name.end();
	return static_cast<size_t>(std::count(first, last, 'r'));
}
...
//   - loss-only frames
//   - colors the caller marks undroppable (a WDL side holding a castling right,
//     whose quiet moves leave the material)
...
NODISCARD Color pick_drop_color(const ColorInfo info[COLOR_NB],
                                ...
                                const bool droppable[COLOR_NB] = nullptr)
{
	...
	auto allowed = [&](Color c) { return !droppable || droppable[c]; };

	if (normals.size() == 2)
	{
		const Color larger = (info[normals[0]].data_size >= info[normals[1]].data_size)
			? normals[0] : normals[1];
		const Color smaller = (larger == normals[0]) ? normals[1] : normals[0];
		if (allowed(larger)) return larger;
		if (allowed(smaller)) return smaller;
		return COLOR_NB;
	}
	if (normals.size() == 1 && table_colors.size() == 2 && allowed(normals[0]))
	{
                         ...
                         EGTB_Magic magic, const char* label)
{
	...
	if (!w.create(tmp.c_str(), CHECKSUM_INIT))
	{
...
bool shrink_wdl(const std::filesystem::path& path)
{
	...
	const bool droppable[COLOR_NB] = {
		castling_rights_in_name(path, WHITE) == 0,
		castling_rights_in_name(path, BLACK) == 0,
	};
	const Color drop = pick_drop_color(info, table_colors, droppable);
	if (drop == COLOR_NB)
	{
		account(bytes.size(), bytes.size());
		return keep_unshrinkable(path, "nothing droppable");
