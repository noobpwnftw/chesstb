CXX      ?= g++
CXXFLAGS ?= -std=c++17 -O3 -DNDEBUG -flto=auto -march=native -Wall -Wno-strict-aliasing -Isrc -Ilib -Wno-interference-size -Wno-format-security -Wno-class-memaccess
LDFLAGS  ?= -pthread

ifeq ($(shell uname -m),aarch64)
CXXFLAGS += -mno-outline-atomics
endif

COMMON_C := \
  $(wildcard lib/lz4/*.c) \
  $(wildcard lib/LZMA/*.c) \
  $(wildcard lib/zstd/common/*.c) \
  $(wildcard lib/zstd/compress/*.c) \
  $(wildcard lib/zstd/dictBuilder/*.c)

COMMON_CXX := \
  $(wildcard src/chess/*.cpp) \
  $(wildcard src/egtb/*.cpp) \
  $(wildcard src/util/*.cpp)

TRANSCRIBE_CXX := \
  $(wildcard src/transcribe/*.cpp)

SHRINK_C := lib/zstd/common/xxhash.c
SHRINK_CXX := \
  src/util/filesystem.cpp \
  src/util/intrin.cpp \
  src/shrink/shrink.cpp

HDRS := $(shell find src lib -name '*.h' -o -name '*.hpp' 2>/dev/null)

.PHONY: all tools clean
all: chesstb shrink transcribe

chesstb: src/main.cpp $(COMMON_CXX) $(COMMON_C) $(HDRS) Makefile
	$(CXX) $(CXXFLAGS) \
	    -x c++ src/main.cpp $(COMMON_CXX) \
	    -x c $(COMMON_C) \
	    $(LDFLAGS) -o $@

transcribe: $(TRANSCRIBE_CXX) $(COMMON_CXX) $(COMMON_C) $(HDRS) Makefile
	$(CXX) $(CXXFLAGS) \
	    -x c++ $(TRANSCRIBE_CXX) $(COMMON_CXX) \
	    -x c $(COMMON_C) \
	    $(LDFLAGS) -o $@

shrink: $(SHRINK_CXX) $(SHRINK_C) $(HDRS) Makefile
	$(CXX) $(CXXFLAGS) \
	    -x c++ $(SHRINK_CXX) \
	    -x c $(SHRINK_C) \
	    $(LDFLAGS) -o $@

tools:
	$(MAKE) -C tools

clean:
	rm -f chesstb shrink transcribe
	$(MAKE) -C tools clean
