...
bool check_material(const Options& opt, const std::string& name)
{
	...
	// Only pawnful material ships a DTC pack, and dtc_tables can read nothing else.
	const bool check_dtc = ps.has_pawns();
			...
			for (size_t i = lo; i < hi; ++i)
			{
				for (Color stm : { WHITE, BLACK })
				{
					...
					else
					{
						...
						if (is_lose_class(w) && dtz_v == 0 && !pos.is_checkmate()) {
					...
					const Probe_Result pr = check_dtc
						? dtc_tables.probe(ps, pos, /*rule50=*/0)
						: Probe_Result{};
					...
					if (pr.has_dtc)
					{
						...
						if (clean && pr.dtc_wdl == w)
						{
							...
							if (w == WDL_Entry::LOSE && value == 0 && !pos.is_checkmate()) {
						...
						if (!pr.has_dtz) {
							...
							// The one-byte tier rounds cursed values, so a derived one may be a ply off.
							&& !((w == WDL_Entry::CURSED_WIN || w == WDL_Entry::BLESSED_LOSS)
							     && std::abs(static_cast<int>(pr.dtz) - static_cast<int>(cr.dtz)) <= 1)) {
					...
					if (is_lose_class(w) && dtm_v == 0 && !pos.is_checkmate()) {
