...
Child_Pos make_child(const Position& parent, Move m)
{
	...
	return { std::move(pos), std::move(cps), zeroing };
}
...
Child_DTM effective_child_dtm_after_move(Probe_Tables& tables, const Position& parent, Move m)
{
	...
	if (!parent.move_is_pawn_double_push(m)) return best;
...
{
	Child_Pos no_ep_child = make_child(parent, m);
	Child_DTZ best = probe_child_dtz(tables, no_ep_child);
	if (!parent.move_is_pawn_double_push(m)) return best;
	...
	for (int df : { -1, +1 })
	{
		...
		const Child_Pos ep_child = make_child(pre_ep, ep);
		const Child_DTZ after_ep = probe_child_dtz(tables, ep_child);
		if (after_ep.missing)
		{
			best.missing = true;
			continue;
...
Unique_Piece_Configs one_move_sub_configs(const Piece_Config& ps)
{
	...
	for (const auto& [_captured, sub] : ps.sub_configs_by_capture())
		out.add_unique(sub);
	if (ps.has_opposing_pair())
		for (const Color survivor : { WHITE, BLACK })
			out.add_unique(ps.pair_broken_survivor(survivor));
	...
	for (size_t i = 0; i < ps.num_pieces(); ++i)
	{
		...
		for (const Piece_Type promo : { QUEEN, ROOK, BISHOP, KNIGHT })
			out.add_unique(ps.with_replaced_piece(i, piece_make(mover, promo)));
...
bool check_material(const Options& opt, const std::string& name)
{
	...
	// Independent instances so the DTZ check never reads DTM/DTM50 and the
	// DTM check never reads DTZ. The DTM branch carries dtm + dtm50, so DTM
