...
//
//   ./tools/compare_syzygy -r KQK,KRK
//   ./tools/compare_syzygy --list FILE
//   ./tools/compare_syzygy --enumerate N
...
static std::vector<std::string> split_materials(const char* csv)
{
	std::vector<std::string> out;
	...
	try {
		...
		for (int i = 1; i < argc; ++i) {
			...
			if (a == "-r" && i + 1 < argc) {
				...
				g_num_threads = std::max<size_t>(1, std::strtoull(argv[++i], nullptr, 10));
		...
		if (!tb_init(opt.syzygy_dir.c_str())) {
			std::cerr << "tb_init failed (need " << opt.syzygy_dir << "/ with .rtbw files)\n";
			return 1;
		}
		// tb_init accepts an empty directory; fail early so the run is meaningful.
		if (TB_LARGEST == 0) {
			std::cerr << opt.syzygy_dir
				<< "/ contains no usable tablebase files — every probe will "
